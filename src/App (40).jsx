...
function ATrip({ go, tid, initTab, trips, clients, crm, sT, sC, sCrm }) {
  ...
  const [passDocNum,setPassDocNum]=useState("");
          ...
          return (
            <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":A.border+"44"}` }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
                <div onClick={()=>c.passportPhoto?(setPassModal({src:c.passportPhoto,cid:c.id,name:c.nombre}),setPassDocNum(c.docNumero||""),setPassExpiry(c.passportExpiry||"")):(setUpIdx(c.id),passRef.current.value="",passRef.current.click())}
                ...
                <div style={{ display:"flex",gap:12,padding:"12px" }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    ...
                    <input
                      ...
                      defaultValue={c.passportExpiry||""}
                      onBlur={e=>updClient(c.id,x=>({...x,passportExpiry:e.target.value,passportExpiryDismissed:false}))}
        ...
        {passModal && (
          <AModal title={passModal.name} onClose={()=>setPassModal(null)}>
            ...
            <div style={{ borderRadius:12,overflow:"hidden",marginBottom:14,position:"relative" }}>
              <img src={passModal.src} style={{ width:"100%",maxHeight:260,objectFit:"cover",display:"block" }} alt="" />
              <div style={{ position:"absolute",top:8,right:8,display:"flex",gap:6 }}>
                <button onClick={()=>setViewPhoto(passModal.src)} style={{ background:"rgba(0,0,0,0.7)",border:"none",borderRadius:8,padding:"6px 10px",color:"#fff",fontSize:12,fontFamily:BF,cursor:"pointer" }}>🔍 Ampliar</button>
                <a href={passModal.src} download="pasaporte.jpg" target="_blank" rel="noreferrer" style={{ background:"rgba(255,215,0,0.85)",border:"none",borderRadius:8,padding:"6px 10px",color:A.bg,fontSize:12,fontFamily:BF,cursor:"pointer",textDecoration:"none" }}>📲</a>
              </div>
            </div>
            {/* Editable fields while photo is visible */}
            <div style={{ background:A.card2,borderRadius:12,padding:"12px",marginBottom:12,border:`1px solid ${A.border}` }}>
              <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Nº Pasaporte / DNI</div>
              <input
                value={passDocNum}
                onChange={e=>setPassDocNum(e.target.value.toUpperCase())}
                placeholder="ABC123456"
                style={{ ...ais,fontFamily:ANTON,fontSize:22,letterSpacing:3,textAlign:"center",color:A.cyan,marginBottom:10,background:A.bg }}
              />
              <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Caducidad (AAAA-MM-DD)</div>
              <input
                value={passExpiry}
                ...
                inputMode="numeric"
                type="text"
                style={{ ...ais,fontFamily:ANTON,fontSize:18,textAlign:"center",color:passportWarn(passExpiry)?A.orange:A.text,background:A.bg,border:`1px solid ${passportWarn(passExpiry)?A.orange+"66":A.border}` }}
              />
            </div>
            <div style={{ display:"flex",gap:8,marginBottom:10 }}>
              <button onClick={()=>{ updClient(passModal.cid,c=>({...c,docNumero:passDocNum,passportExpiry:passExpiry,docVerified:false})); setPassModal(null); }} style={{ ...ab(A.cyan,A.bg),flex:2 }}>💾 Guardar</button>
              <button onClick={()=>{ chPassRef.current.value=""; chPassRef.current.click(); }} style={{ ...ab(A.purple+"22",A.purple),flex:1,border:`1px solid ${A.purple}44` }}>Cambiar foto</button>
              <button onClick={()=>{ updClient(passModal.cid,c=>({...c,passportPhoto:null})); setPassModal(null); }} style={{ ...ab(A.red+"22",A.red),flex:1,border:`1px solid ${A.red}44` }}>Borrar</button>
            </div>
  ...
  const AIDocsTab=()=>{
    ...
    const [manualTarget,setManualTarget]=useState(allTargets[0]?.id||"");
    const [manualTipo,setManualTipo]=useState("vuelo_ida");
    const [manualNombre,setManualNombre]=useState("");
    const [manualFiles,setManualFiles]=useState([]);
    const [manualSaving,setManualSaving]=useState(false);
    const [manualDone,setManualDone]=useState(false);
    const TIPOS_MANUAL=[
      {k:"vuelo_ida",l:"✈️ Vuelo ida"},{k:"vuelo_vuelta",l:"✈️ Vuelo vuelta"},
      {k:"seguro",l:"🛡️ Seguro"},{k:"voucher",l:"🎫 Voucher"},
      {k:"visado",l:"🛂 Visado"},{k:"hotel",l:"🏨 Hotel"},
      {k:"otro",l:"📄 Otro"}
    ];
    ...
    const addManualFiles=e=>{ const files=Array.from(e.target.files||[]); setManualFiles(prev=>[...prev,...files.map(f=>({id:uid(),file:f,name:f.name}))]); e.target.value=""; };
    ...
    const assignManual=async()=>{
      if(!manualFiles.length||!manualTarget) return;
      const target=allTargets.find(t=>t.id===manualTarget); if(!target) return;
      setManualSaving(true);
      for(const mf of manualFiles){
        const b64=await fileToB64(mf.file);
        const ext=mf.file.type?.includes("pdf")?"pdf":mf.file.type?.includes("png")?"png":"jpg";
        const tipoLabel=TIPOS_MANUAL.find(t=>t.k===manualTipo)?.l.replace(/[^\w]/g,"")||"DOC";
        const nombre=manualNombre.trim()||`${target.nombre.split(" ")[0]}_${tipoLabel}.${ext}`;
        const doc={ id:uid(),tipo:manualTipo.startsWith("vuelo")?"vuelo":manualTipo,nombre,data:b64,mimeType:mf.file.type,archivo:mf.file.name };
        if(target.type==="acomp"){ updClient(target.clientId,c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===target.id?{...a,personalDocs:[...(a.personalDocs||[]),doc]}:a)})); }
        else{ updClient(target.id,c=>({...c,personalDocs:[...(c.personalDocs||[]),doc]})); }
      }
      setManualSaving(false); setManualDone(true); setManualFiles([]); setManualNombre("");
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {/* Mode toggle */}
        <div style={{ display:"flex",background:A.card2,borderRadius:10,padding:4,marginBottom:14,gap:4 }}>
          {[{k:"ia",l:"🤖 IA (Auto)"},{k:"manual",l:"📁 Manual"}].map(item=>(
            <button key={item.k} onClick={()=>setMode(item.k)} style={{ flex:1,background:mode===item.k?A.cyan:A.card2,color:mode===item.k?A.bg:A.muted,border:"none",borderRadius:8,padding:"10px",fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer" }}>{item.l}</button>
          ))}
        </div>
        {mode==="ia"&&(<>
        ...
        {mode==="manual"&&(
          <div>
            <div style={{ background:A.orange+"15",border:`1px solid ${A.orange}33`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontFamily:BF,fontSize:12,color:A.orange }}>
              📁 Asignación manual — sube documentos directamente a un viajero sin IA
            </div>
            {/* Viajero selector */}
            <div style={{ marginBottom:12 }}>
              <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Viajero</div>
              <select value={manualTarget} onChange={e=>setManualTarget(e.target.value)} style={{ ...ais }}>
                {allTargets.map(t=>(
                  <option key={t.id} value={t.id}>{t.nombre}{t.type==="acomp"?" (acomp.)":""}</option>
                ))}
              </select>
            </div>
            {/* Tipo */}
            <div style={{ marginBottom:12 }}>
              <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:8 }}>Tipo de documento</div>
              <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6 }}>
                {TIPOS_MANUAL.map(t=>(
                  <button key={t.k} onClick={()=>setManualTipo(t.k)} style={{ padding:"8px 4px",borderRadius:8,cursor:"pointer",fontFamily:BF,fontSize:10,border:`2px solid ${manualTipo===t.k?A.orange:A.border}`,background:manualTipo===t.k?A.orange+"22":A.card,color:manualTipo===t.k?A.orange:A.muted,textAlign:"center",lineHeight:1.4 }}>{t.l}</button>
                ))}
              </div>
            </div>
            {/* Nombre personalizado (opcional) */}
            <div style={{ marginBottom:12 }}>
              <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Nombre del archivo (opcional)</div>
              <input value={manualNombre} onChange={e=>setManualNombre(e.target.value)} placeholder="Ej: Juan_Billete_IDA.pdf" style={{ ...ais }} />
            </div>
            {/* File picker */}
            <input ref={manualRef} type="file" accept="application/pdf,image/*" multiple style={{ display:"none" }} onChange={addManualFiles} />
            <button onClick={()=>manualRef.current.click()} style={{ width:"100%",background:A.purple+"22",border:`1.5px dashed ${A.purple}`,borderRadius:10,padding:"12px",color:A.purple,fontFamily:BF,fontSize:13,cursor:"pointer",fontWeight:700,marginBottom:10 }}>
              📎 Seleccionar archivos ({manualFiles.length} seleccionados)
            </button>
            {manualFiles.length>0&&(
              <div style={{ background:A.card2,borderRadius:10,padding:"10px 12px",marginBottom:12,border:`1px solid ${A.border}` }}>
                {manualFiles.map(mf=>(
                  <div key={mf.id} style={{ display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:`1px solid ${A.border}22` }}>
                    <span style={{ fontSize:14 }}>📄</span>
                    <span style={{ flex:1,fontFamily:BF,fontSize:12,color:A.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{mf.name}</span>
                    <button onClick={()=>setManualFiles(prev=>prev.filter(x=>x.id!==mf.id))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:16,cursor:"pointer" }}>✕</button>
                  </div>
                ))}
              </div>
            )}
            {manualDone&&(
              <div style={{ background:A.green+"22",border:`1px solid ${A.green}44`,borderRadius:10,padding:"12px",marginBottom:12,textAlign:"center",fontFamily:BF,fontSize:14,color:A.green,fontWeight:700 }}>
                ✅ Documentos asignados correctamente
              </div>
            )}
            <button onClick={assignManual} disabled={!manualFiles.length||!manualTarget||manualSaving}
              style={{ width:"100%",padding:"14px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:15,letterSpacing:2,cursor:manualFiles.length&&manualTarget?"pointer":"default",background:manualFiles.length&&manualTarget?`linear-gradient(90deg,${A.orange},#cc7a00)`:"#1a1a1a",color:"#fff",textTransform:"uppercase" }}>
              {manualSaving?"GUARDANDO...":"ASIGNAR AL VIAJERO"}
            </button>
          </div>
        )}
      </div>
...
function Client({ go, cid, clients, trips, logout, sC, sT }) {
  ...
  const updatePassportPhoto = e => {
    const f = e.target.files[0]; if(!f) return;
    const r = new FileReader();
    r.onload = ev => sC(clients.map(x=>x.id===cid?{...x,passportPhoto:ev.target.result}:x));
    r.readAsDataURL(f);
    e.target.value="";
  };
      ...
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", fontFamily: BF, color: A.text, paddingBottom: 32 }}>
        <div style={{ background: "linear-gradient(135deg,#070718 0%,#0f1f3d 60%,#07070f 100%)", padding: "28px 20px 24px", borderBottom: `1px solid ${A.border}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
            ...
            {/* Passport photo avatar - tappable to update */}
            <input ref={passportInputRef} type="file" accept="image/*" style={{display:"none"}} onChange={updatePassportPhoto} />
            <div onClick={()=>passportInputRef.current.click()} title="Actualizar foto pasaporte"
              style={{ width:40,height:50,borderRadius:8,overflow:"hidden",border:`2px solid ${cl.passportPhoto?A.cyan+"88":A.border}`,background:A.card2,cursor:"pointer",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",position:"relative" }}>
              {cl.passportPhoto
                ? <img src={cl.passportPhoto} style={{ width:"100%",height:"100%",objectFit:"cover" }} alt="" />
                : <span style={{ fontFamily:ANTON,fontSize:16,color:A.muted }}>{cl.nombre[0]?.toUpperCase()}</span>
              }
              <div style={{ position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.55)",height:14,display:"flex",alignItems:"center",justifyContent:"center" }}>
