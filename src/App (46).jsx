...
function CopyBtn({ text, style }) {
  ...
  return (
    <button onClick={e=>{ e.stopPropagation(); navigator.clipboard.writeText(text||"").then(()=>{ setCop(true); setTimeout(()=>setCop(false),1800); }); }}
      style={{ background:cop?"#22c55e22":"transparent",border:"1px solid "+(cop?"#22c55e":"#ffffff22"),borderRadius:6,padding:"3px 7px",cursor:"pointer",color:cop?"#22c55e":"#ffffff66",fontSize:11,fontFamily:"monospace",lineHeight:1,flexShrink:0,...(style||{}) }}>
      {cop?"✓":"⎘"}
    </button>
  );
}
  ...
  const PeopleTab=()=>{
    ...
    const buildFlightList=()=>{
      ...
      confirmed.forEach((c,i)=>{
        ...
        lines.push(`${i+1}. ${c.nombre} | ${c.docNumero||"—"} | ${bday} | ${exp}`);
      ...
      <div style={{ padding:"0 16px" }}>
        ...
        {listModal && (
          <div style={{ position:"fixed",inset:0,zIndex:2000,background:A.bg,display:"flex",flexDirection:"column",overflowY:"auto" }}>
            ...
            {listView==="vuelo" && (
              <div style={{ padding:"20px 20px 120px" }}>
                <div style={{ fontFamily:BF,fontSize:12,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:18 }}>
                  {confirmed.reduce((acc,c)=>acc+1+(c.acompanantes||[]).length,0)} pasajeros totales
                </div>
                {confirmed.map((c,i)=>{
                  const allPax=[{...c,isMain:true},...(c.acompanantes||[]).map(a=>({...a,isMain:false}))];
                  return allPax.map((p,pi)=>{
                    ...
                    const num=confirmed.slice(0,i).reduce((acc,x)=>acc+1+(x.acompanantes||[]).length,0)+pi+1;
                    return (
                      <div key={p.id||c.id+"-"+pi} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${p.isMain?A.border:A.purple+"55"}`,overflow:"hidden" }}>
                        <div style={{ padding:"12px 18px",borderBottom:`1px solid ${A.border}33`,display:"flex",alignItems:"center",gap:12,background:p.isMain?"transparent":A.purple+"10" }}>
                          <div style={{ fontFamily:ANTON,fontSize:28,color:A.cyan,lineHeight:1,width:36,textAlign:"center" }}>{num}</div>
                          <div style={{ flex:1 }}>
                            <div style={{ fontFamily:ANTON,fontSize:22,color:A.text,lineHeight:1.1 }}>{p.nombre}</div>
                            {!p.isMain&&<div style={{ fontFamily:BF,fontSize:11,color:A.purple,marginTop:3 }}>+ acompañante de {c.nombre.split(" ")[0]}</div>}
                          </div>
                          <CopyBtn text={p.nombre} />
                        </div>
