...
// ============================================================
// SECURE AUTH — PINs viven en Supabase, nunca en el cliente
// ============================================================
const verifyPin = async (pin, type = "admin") => {
  try {
    const { data, error } = await supabase.rpc("tl_verify_and_login", {
      input_pin: pin, pin_type: type
    });
...
const verifySessionToken = async (token, type = "admin") => {
  ...
  try {
    const { data } = await supabase.rpc("tl_verify_session", {
      input_token: token, token_type: type
    });
    return !!data;
  } catch { return false; }
};
...
const sendOneSignal = async (title, body, filters=[]) => {
  // Llamamos al proxy de Netlify — nunca directamente al API de OneSignal desde el browser
  // (el API de OneSignal bloquea peticiones de navegadores por CORS y expondría la REST Key)
  try {
    ...
    console.error("[OneSignal proxy] Fetch error:", e);
    return { success: false, error: e.message };
  }
};

// ============================================================
// STORAGE — Supabase (metadatos) + localStorage (datos completos incl. fotos)
// ============================================================
...
// Elimina campos base64 grandes antes de guardar en Supabase
const stripBinary = (k, v) => {
  if (k !== SK_C && k !== SK_T) return v;
  if (!Array.isArray(v)) return v;
  return v.map(item => {
    const stripped = { ...item };
    // Clientes: quitar fotos de pasaporte (base64 pesado)
    if (stripped.passportPhoto) stripped.passportPhoto = "__local__";
    if (stripped.passportPhotos) stripped.passportPhotos = [];
    // Trips: quitar data de facturas
    if (stripped.facturas) stripped.facturas = (stripped.facturas || []).map(f => ({ ...f, data: f.data ? "__local__" : null }));
    return stripped;
  });
};

// Fusiona datos de Supabase (sin fotos) con localStorage (con fotos)
const mergeBinary = (k, supaVal, localVal) => {
  ...
  return supaVal.map(item => {
    ...
    const merged = { ...item };
    if (k === SK_C) {
      if (local.passportPhoto && local.passportPhoto !== "__local__") merged.passportPhoto = local.passportPhoto;
...
const db = {
  async get(k) {
    // Leer siempre localStorage primero (tiene fotos completas)
    let localVal = null;
    try { const r = localStorage.getItem(k); if (r) localVal = JSON.parse(r); } catch {}
    // Luego Supabase para tener datos actualizados desde otros dispositivos
    try {
      const { data, error } = await supabase.from("travelike_store").select("value").eq("key", k).maybeSingle();
      if (!error && data?.value) {
        const supaVal = data.value;
        // Si hay datos en ambos, fusionar (Supabase manda, localStorage aporta fotos)
  ...
  async set(k, v) {
    // localStorage siempre: guarda TODO incluyendo fotos
    try { localStorage.setItem(k, JSON.stringify(v)); } catch {}
    mem[k] = v;
    // Supabase: guarda versión sin fotos (para sincronización entre dispositivos)
    try {
...
const generarFacturaPDF = async (fa) => {
  ...
  if (fa.tipo?.includes("image")) {
    ...
    doc.text("Factura: " + fa.nombre, 10, 20);
  }
  ...
  const press = async d => {
    ...
    if (next.length === len) {
      if (onVerifyAsync) {
        ...
        setLoading(false);
        if (result === true || result?.status === "ok") {
          onSuccess(result?.token);
        } else if (result?.status === "locked") {
          setLockMsg(`Bloqueado ${result.minutes || 30} min`);
          setVal("");
        } else {
          setErr(true);
          if (result?.remaining !== undefined) setLockMsg(`${result.remaining} intentos restantes`);
          setTimeout(() => { setVal(""); setErr(false); setLockMsg(""); }, 1800);
        }
      } else if (correctPin) {
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // Verificar token de sesión admin en Supabase (no basta con que exista en localStorage)
      let admValid = false;
      if (savedToken && savedToken !== "true" && savedToken !== true) {
        admValid = await verifySessionToken(savedToken, "admin");
        if (!admValid) await db.set(SK_ADMIN, null); // token expirado → limpiar
      }
      if (admValid) { setAdminOk(true); setAdminToken(savedToken); }

      // Hash en URL → acceso de cliente (tiene prioridad sobre sesión guardada)
      ...
      // Si hay token admin válido → siempre ir al admin (aunque haya sesión de cliente)
      if (admValid) return setScreen("ahome");

      // Sin token admin → comprobar sesión de cliente
...
function Home({ go, goClient, clients }) {
  ...
  useEffect(()=>{
    (async()=>{
      try {
        const ses = localStorage.getItem("tv9-session");
        if (ses) {
          const parsed = JSON.parse(ses);
          if (parsed?.code) {
            const cl = clients.find(x => x.code === parsed.code);
            if (cl) setSavedClient(cl);
          }
        }
      } catch {}
    })();
...
function PinScreen({ go, onOk, alreadyOk }) {
  ...
  const tryPin = async () => {
    ...
    if (result.status === "ok") {
      ...
      setStatus("locked");
      setMsg(`Demasiados intentos. Bloqueado ${result.minutes || 30} minutos.`);
    } else if (result.status === "invalid") {
      setStatus("error");
      const r = result.remaining;
      setMsg(r > 0 ? `PIN incorrecto — ${r} intento${r > 1 ? "s" : ""} restante${r > 1 ? "s" : ""}` : "PIN incorrecto — último intento agotado");
      setTimeout(() => setStatus("idle"), 3000);
    } else {
      setStatus("error");
      setMsg("Error de conexión. Comprueba tu red.");
      setTimeout(() => setStatus("idle"), 3000);
    }
  };
      ...
      {status === "locked" ? (
        ...
        <>
          ...
          <button
            ...
            style={{ width: "100%", padding: "18px", border: "none", borderRadius: 14, fontFamily: ANTON, fontSize: 16, letterSpacing: 2, cursor: status === "loading" || !pin.trim() ? "default" : "pointer", background: status === "loading" ? A.card2 : A.cyan, color: status === "loading" ? A.muted : A.bg, textTransform: "uppercase", marginBottom: 16, opacity: !pin.trim() ? 0.5 : 1 }}>
...
function NotifAdmin({ trips, clients }) {
  ...
  const send=async(title,body,target,tripId)=>{
    ...
    let errorMsg = "";
    try {
      const filters = target === "trip" && tripId
        ? [{ field: "tag", key: "tripId", relation: "=", value: tripId }]
        : [];
      const result = await sendOneSignal(title, body, filters);
      console.log("[OneSignal] Respuesta:", JSON.stringify(result));
      const count = target === "all"
        ? clients.filter(c => c.notifEnabled).length
        : clients.filter(c => c.tripId === tripId && c.notifEnabled).length;
      ...
      console.error("[OneSignal] Error catch:", e);
      setSent({ title, count: 0, ok: false, errorMsg: e.message });
    }
    setSending(false);
          ...
          {facturas.filter(f=>!f.gastoId).length===0?<AEmpty text="Sin facturas generales" />:facturas.filter(f=>!f.gastoId).map(fa=>(
            <div key={fa.id} style={{ background:A.card,borderRadius:10,marginBottom:8,border:`1px solid ${A.gold}33`,overflow:"hidden" }}>
              <div style={{ padding:"10px 14px",display:"flex",alignItems:"center",gap:10 }}>
                ...
                <button onClick={()=>generarFacturaPDF(fa)} title="Generar PDF" style={{ ...ab(A.red+"22",A.red),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.red}44`,fontFamily:BF,flexShrink:0 }}>PDF</button>
...
function Passport({ go, cid, clients, setClients, trips, sC, logout }) {
  ...
  const [c0, setC0] = useState(false); // consentimiento RGPD base — NUEVO
  const [c1, setC1] = useState(false); // pasaporte
  const [c2, setC2] = useState(false); // fotos redes
  ...
  const canSubmit = c0 && c1; // fotos opcionales para no bloquear

  const submit = async () => {
    if (!canSubmit) return;
    // Primero actualizamos estado local y navegamos — NO esperamos a Supabase
    const updated = clients.map(x => x.id === cid
      ? { ...x, passportPhoto: photos[0] || null, passportPhotos: photos, rgpdConsent: true, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false }
      : x
    );
    setClients(updated); // actualización inmediata sin esperar db
    go("notifprompt", { cid }); // navegamos ya
    db.set(SK_C, updated); // guardamos en background (no bloqueante)
  };
    ...
    <div style={{ fontFamily: BF, background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text }}>
      ...
      <div style={{ padding: "24px 20px" }}>
        <div style={{ background: A.card, borderRadius: 14, padding: 20, border: `1px solid ${A.border}`, marginBottom: 16 }}>
          ...
          {/* CHECKBOX 0: RGPD base — obligatorio */}
          <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12, cursor: "pointer", padding: "14px", background: c0 ? A.cyan + "12" : A.card2, borderRadius: 14, border: `2px solid ${c0 ? A.cyan : A.border}` }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: BF, fontSize: 12, color: A.muted, lineHeight: 1.6 }}>He leído y acepto el tratamiento de mis datos personales por Travelike SL para la gestión de mi viaje, conforme al RGPD. {" "}
                <span onClick={e => { e.preventDefault(); setShowPriv(true); }} style={{ color: A.cyan, textDecoration: "underline", cursor: "pointer" }}>Leer política de privacidad →</span>
              </div>
            </div>
          </label>

          {/* CHECKBOX 1: pasaporte — obligatorio */}
          ...
          {/* CHECKBOX 2: fotos redes — opcional */}
          ...
          <div style={{ fontFamily: BF, fontSize: 11, color: A.muted, marginBottom: 16, textAlign: "center" }}>
            <span style={{ color: A.red }}>*</span> Campos obligatorios. Puedes ejercer tus derechos RGPD escribiendo a{" "}
            <span style={{ color: A.cyan }}>lauralietor@hotmail.com</span>
          </div>
...
function NotifPrompt({ go, cid, clients, sC, logout }) {
  ...
  // Detección: ¿es iPhone/iPad?
  const isIOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
  // Detección: ¿está instalada como PWA en iOS? (window.navigator.standalone solo existe en Safari iOS)
  ...
  const enable = async () => {
    ...
    if (cl) {
      try {
        // ✅ Usar window.OneSignal directamente — OneSignalDeferred es solo la cola de init,
        //    no es confiable dentro de handlers async una vez que OneSignal ya está inicializado.
        const OneSignal = window.OneSignal;
        if (OneSignal) {
          const granted = await OneSignal.Notifications.requestPermission();
          console.log("[OneSignal] Permiso:", granted);
          if (granted) {
            // Vincular este dispositivo al código de cliente
            await OneSignal.login(cl.code);
            // Añadir tag de viaje para poder filtrar por tripId al enviar
            if (cl.tripId) await OneSignal.User.addTag("tripId", cl.tripId);
            // ⏳ Esperar a que OneSignal sincronice la suscripción y los tags con sus servidores
            //    Sin este delay, los tags pueden no estar disponibles aún al enviar notificaciones
            await new Promise(r => setTimeout(r, 3000));
            // Log para verificar que la suscripción se creó correctamente
            const subId = OneSignal.User?.PushSubscription?.id;
            console.log("[OneSignal] Subscription ID:", subId || "⚠️ sin ID — la suscripción no se creó");
          } else {
            console.warn("[OneSignal] Permiso denegado por el usuario");
          }
        } else {
          console.warn("[OneSignal] window.OneSignal no disponible todavía");
        }
      } catch (e) {
        console.warn("[OneSignal] Error al registrar:", e);
      }
    }
  ...
  // ── iOS + NO instalada como PWA → instrucciones de instalación ──
  if (isIOS && !isPWA) {
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", fontFamily: BF, color: A.text }}>
        <div style={{ background: "linear-gradient(135deg,#07070f 0%,#0f1f3d 60%,#07070f 100%)", padding: "40px 24px 32px", textAlign: "center", borderBottom: `1px solid ${A.border}` }}>
          ...
          <div style={{ fontFamily: ANTON, fontSize: 30, color: "#fff", letterSpacing: 1, marginBottom: 10, textTransform: "uppercase", lineHeight: 1.1 }}>
            AÑADE LA APP<br /><span style={{ color: A.cyan }}>A TU INICIO</span>
          </div>
          <div style={{ fontSize: 15, color: A.muted, lineHeight: 1.7 }}>
            Para recibir notificaciones en iPhone, primero instala la app en tu pantalla de inicio.
          </div>
        </div>
        <div style={{ flex: 1, padding: "24px 20px" }}>
          ...
          <div style={{ background: A.gold + "15", border: `1px solid ${A.gold}44`, borderRadius: 14, padding: "14px 16px", marginBottom: 24, display: "flex", gap: 12, alignItems: "flex-start" }}>
            <span style={{ fontSize: 20, flexShrink: 0 }}>💡</span>
            <div style={{ fontFamily: BF, fontSize: 13, color: A.gold, lineHeight: 1.6 }}>
              Una vez instalada, la app se abrirá automáticamente como aplicación nativa y podrás activar las notificaciones desde allí.
            </div>
          </div>
  ...
  // ── Android / Desktop / iOS ya como PWA → flujo normal OneSignal ──
  return (
    ...
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", fontFamily: BF, color: A.text, paddingBottom: 32 }}>
        ...
        <div style={{ textAlign: "center", paddingTop: 4 }}>
          ...
          <button onClick={() => {
            if (window.confirm("¿Quieres solicitar el borrado de tus datos? Se enviará una solicitud a Travelike.\n\nTambién puedes escribir a: lauralietor@hotmail.com")) {
              window.open(`mailto:lauralietor@hotmail.com?subject=Solicitud%20de%20borrado%20de%20datos%20RGPD&body=Hola,%0A%0ASoy%20${encodeURIComponent(cl.nombre)}%20y%20solicito%20el%20borrado%20de%20mis%20datos%20personales%20conforme%20al%20art%C3%ADculo%2017%20del%20RGPD.%0A%0AGracias.`, "_blank");
            }
          }} style={{ background: "none", border: "none", color: A.muted, fontSize: 11, cursor: "pointer", fontFamily: BF, textDecoration: "underline" }}>🗑️ Borrar mis datos</button>
        </div>
      </div>
    );
  }
