  ...
  const PeopleTab=()=>{
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {listModal && (
          <AModal title="Listas del viaje" onClose={()=>setListModal(false)}>
            <div style={{ display:"flex",background:A.bg,borderRadius:10,padding:4,marginBottom:12,gap:4 }}>
              <button onClick={()=>{ setListView("confirmados"); setCopied2(false); }} style={{ flex:1,background:listView==="confirmados"?A.green:A.card2,color:listView==="confirmados"?"#fff":A.muted,border:"none",borderRadius:8,padding:"9px",fontFamily:BF,fontSize:11,fontWeight:700,cursor:"pointer" }}>🛏 Rooming ({confirmed.length})</button>
              <button onClick={()=>{ setListView("vuelo"); setCopied2(false); }} style={{ flex:1,background:listView==="vuelo"?A.cyan:A.card2,color:listView==="vuelo"?"#fff":A.muted,border:"none",borderRadius:8,padding:"9px",fontFamily:BF,fontSize:11,fontWeight:700,cursor:"pointer" }}>✈️ Vuelo</button>
              <button onClick={()=>{ setListView("interesados"); setCopied2(false); }} style={{ flex:1,background:listView==="interesados"?A.orange:A.card2,color:listView==="interesados"?"#fff":A.muted,border:"none",borderRadius:8,padding:"9px",fontFamily:BF,fontSize:11,fontWeight:700,cursor:"pointer" }}>👀 Interes. ({interesados.length})</button>
            </div>
            {listView==="vuelo" && (
              <div style={{ marginBottom:12 }}>
                {confirmed.map((c,i)=>{
                  const bday=c.birthDate?new Date(c.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):"—";
                  const exp=c.passportExpiry?new Date(c.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):"—";
                  const expWarn=passportWarn(c.passportExpiry);
                  const allPax=[{nombre:c.nombre,docNumero:c.docNumero,birthDate:c.birthDate,passportExpiry:c.passportExpiry,isMain:true},...(c.acompanantes||[]).map(a=>({...a,isMain:false}))];
                  return allPax.map((p,pi)=>{
                    const pbday=p.birthDate?new Date(p.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):"—";
                    const pexp=p.passportExpiry?new Date(p.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):"—";
                    const pexpWarn=passportWarn(p.passportExpiry);
                    return (
                      <div key={p.id||c.id+pi} style={{ background:A.card,borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1px solid ${p.isMain?A.border:A.purple+"33"}` }}>
                        <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6 }}>
                          {!p.isMain&&<span style={{ fontSize:9,color:A.purple,fontFamily:BF,background:A.purple+"20",padding:"2px 6px",borderRadius:4 }}>+acomp</span>}
                          <div style={{ fontFamily:ANTON,fontSize:14,color:"#fff",flex:1 }}>{p.nombre}</div>
                          <div style={{ fontFamily:ANTON,fontSize:13,color:p.docNumero?A.cyan:A.muted,letterSpacing:2 }}>{p.docNumero||"SIN DOC"}</div>
                        </div>
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                          <div style={{ background:A.bg,borderRadius:8,padding:"6px 8px" }}>
                            <div style={{ fontFamily:BF,fontSize:8,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:2 }}>Nacimiento</div>
                            <div style={{ fontFamily:ANTON,fontSize:13,color:A.gold }}>{pbday}</div>
                          </div>
                          <div style={{ background:A.bg,borderRadius:8,padding:"6px 8px" }}>
                            <div style={{ fontFamily:BF,fontSize:8,color:pexpWarn?A.orange:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:2 }}>Caducidad</div>
                            <div style={{ fontFamily:ANTON,fontSize:13,color:pexpWarn?A.orange:A.text }}>{pexp} {pexpWarn?"⚠️":""}</div>
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div style={{ marginBottom:12 }}>
                {(()=>{
                  ...
                  ["doble_jun","doble_sep"].forEach(rk=>{
                    if(!byRoom[rk]?.length) return;
                    const roomInfo=ROOMS[rk];
                    rows.push(<div key={rk+"_hdr"} style={{ fontFamily:BF,fontSize:9,color:roomInfo.color,letterSpacing:2,textTransform:"uppercase",marginBottom:6,marginTop:4,fontWeight:700 }}>{roomInfo.label} ({Math.ceil(byRoom[rk].length/2)} hab.)</div>);
                    const seen=new Set();
                    byRoom[rk].forEach(c=>{
                      ...
                      rows.push(
                        <div key={c.id} style={{ background:A.card,borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1px solid ${roomInfo.color}33` }}>
                          <div style={{ fontFamily:BF,fontSize:9,color:roomInfo.color,letterSpacing:2,marginBottom:4 }}>HAB {hab++}</div>
                          <div style={{ display:"flex",flexDirection:"column",gap:4 }}>
                            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                              <div style={{ width:28,height:28,borderRadius:6,overflow:"hidden",flexShrink:0,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:12,color:A.cyan }}>
                                {c.passportPhoto?<img src={c.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:c.nombre[0]}
                              </div>
                              <div style={{ fontFamily:BF,fontSize:13,fontWeight:700,color:"#fff",flex:1 }}>{c.nombre}</div>
                            </div>
                            {partner&&(
                              <div style={{ display:"flex",alignItems:"center",gap:8,paddingLeft:4 }}>
                                <div style={{ width:3,height:24,background:roomInfo.color+"66",borderRadius:2,flexShrink:0 }} />
                                <div style={{ width:26,height:26,borderRadius:6,overflow:"hidden",flexShrink:0,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:11,color:A.cyan }}>
                                  {partner.passportPhoto?<img src={partner.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:partner.nombre[0]}
                                </div>
                                <div style={{ fontFamily:BF,fontSize:13,color:A.text }}>{partner.nombre}</div>
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  ["individual","triple","cuadruple","busca"].forEach(rk=>{
                    if(!byRoom[rk]?.length) return;
                    const roomInfo=ROOMS[rk];
                    rows.push(<div key={rk+"_hdr"} style={{ fontFamily:BF,fontSize:9,color:roomInfo.color,letterSpacing:2,textTransform:"uppercase",marginBottom:6,marginTop:4,fontWeight:700 }}>{roomInfo.label} ({byRoom[rk].length})</div>);
                    const seen2=new Set();
                    if(rk==="triple"||rk==="cuadruple"){
                      ...
                      groups.forEach((grp,gi)=>{
                        rows.push(
                          <div key={rk+gi} style={{ background:A.card,borderRadius:10,padding:"10px 12px",marginBottom:6,border:`1px solid ${roomInfo.color}33` }}>
                            <div style={{ fontFamily:BF,fontSize:9,color:roomInfo.color,letterSpacing:2,marginBottom:4 }}>HAB {hab++}</div>
                            {grp.map(c=><div key={c.id} style={{ fontFamily:BF,fontSize:13,color:"#fff",marginBottom:2 }}>· {c.nombre}</div>)}
                          </div>
                        );
                      });
                    } else {
                      byRoom[rk].forEach(c=>{
                        rows.push(
                          <div key={c.id} style={{ background:A.card,borderRadius:10,padding:"9px 12px",marginBottom:6,border:`1px solid ${roomInfo.color}33`,display:"flex",alignItems:"center",gap:8 }}>
                            <div style={{ fontFamily:BF,fontSize:13,color:"#fff",flex:1 }}>{c.nombre}</div>
                            <span style={{ fontSize:9,color:roomInfo.color,fontFamily:BF }}>{roomInfo.short}</span>
                          </div>
                        );
                      });
                    }
            ...
            <pre style={{ background:A.bg,borderRadius:10,padding:12,fontSize:11,lineHeight:1.9,color:A.text,overflowX:"auto",whiteSpace:"pre-wrap",marginBottom:12,border:`1px solid ${A.border}`,fontFamily:"monospace" }}>{currentList}</pre>
            <button onClick={()=>{ navigator.clipboard.writeText(currentList).then(()=>{ setCopied2(true); setTimeout(()=>setCopied2(false),2500); }); }} style={{ ...ab(copied2?A.green:A.cyan,A.bg),width:"100%",borderRadius:10,fontFamily:BF }}>{copied2?"¡Copiado!":"Copiar texto"}</button>
          </AModal>
        )}
