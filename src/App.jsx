...
function NotifPrompt({ go, cid, clients, sC, logout }) {
  ...
  const enable = async () => {
    ...
    if (window.OneSignalDeferred && cl) {
      // Timeout de seguridad: si OneSignal no responde en 8s, continuamos igualmente
      await Promise.race([
        new Promise(resolve => {
          window.OneSignalDeferred.push(async (OneSignal) => {
            ...
            } catch (e) { console.warn("[OneSignal] Error al registrar:", e); }
            resolve();
          });
        }),
