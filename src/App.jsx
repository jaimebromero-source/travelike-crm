...
const db = {
  async get(k) {
    try {
      ...
      if (!error && data) return data.value ?? null;
    } catch {}
    try { const r = localStorage.getItem(k); return r ? JSON.parse(r) : null; } catch { return mem[k] ?? null; }
  },
  async set(k, v) {
    try {
      await supabase.from("travelike_store").upsert({ key: k, value: v, updated_at: new Date().toISOString() }, { onConflict: "key" });
    } catch {}
    ...
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:10001,display:"flex",alignItems:"flex-end" }}>
      <div style={{ background:A.card2,borderRadius:"20px 20px 0 0",padding:"24px",width:"100%",maxHeight:"85vh",overflowY:"auto",borderTop:`1px solid ${A.border}`,boxSizing:"border-box" }}>
        ...
        {[
          ...
          ["¿Cuánto tiempo los guardamos?","Los datos del viaje se conservan durante la relación contractual y hasta 4 años después. Los datos fiscales se guardan 5 años por obligación legal. La foto del pasaporte se elimina al finalizar el viaje."],
...
function Passport({ go, cid, clients, trips, sC, setScreen, setNav }) {
  ...
  const canSubmit = photos.length > 0 && c0 && c1;

  const submit = async () => {
    if (!canSubmit) return;
    await sC(clients.map(x => x.id === cid ? { ...x, passportPhoto: photos[0], passportPhotos: photos, rgpdConsent: true, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false } : x));
