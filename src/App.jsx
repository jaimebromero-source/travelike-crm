...
function NotifPrompt({ go, cid, clients, sC, logout }) {
  ...
  const enable = async () => {
    ...
    try {
      ...
      const OneSignal = await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error("OneSignal SDK no cargado")), 8000);
      ...
      if (granted && cl) {
        setStatusMsg("Registrando dispositivo...");
        // Vincular dispositivo con el código del viajero
        await OneSignal.login(cl.code);
        // Tag para filtrar por viaje
        if (cl.tripId) await OneSignal.User.addTag("tripId", cl.tripId);
        console.log("[OneSignal] Registrado como:", cl.code, "tripId:", cl.tripId);
      }
    } catch (e) {
      console.warn("[OneSignal] Error:", e.message);
      // Continuamos igualmente — el usuario puede recibir notifs aunque falle el tag
    }
  ...
  // iOS: detectar si están en Safari sin PWA instalada
  ...
  return (
    <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", fontFamily: BF, color: A.text, textAlign: "center" }}>
      ...
      <div style={{ fontSize: 17, color: A.muted, lineHeight: 1.7, marginBottom: 24 }}>Activa las notificaciones para recibir avisos sobre tus pagos, vuelos y documentación.</div>

      {showIOSWarning && (
        <div style={{ background: A.orange+"22", border:`1px solid ${A.orange}44`, borderRadius:14, padding:"14px 16px", marginBottom:24, textAlign:"left" }}>
          <div style={{ fontFamily:ANTON, fontSize:15, color:A.orange, marginBottom:6 }}>📱 INSTALA LA APP PRIMERO</div>
          <div style={{ fontSize:13, color:A.muted, lineHeight:1.7 }}>
            En iPhone, las notificaciones solo funcionan con la app instalada.<br/>
            Pulsa <strong style={{color:"#fff"}}>Compartir → "Añadir a pantalla de inicio"</strong> y luego vuelve a activarlas.
          </div>
        </div>
      )}

      {loading && statusMsg && (
        <div style={{ fontSize:13, color:A.cyan, marginBottom:16, fontFamily:BF }}>{statusMsg}</div>
      )}
