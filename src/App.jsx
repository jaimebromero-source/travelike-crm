...
function AHome({ go, trips, clients, sT, sC, cfg, sCfg, logoutAdmin }) {
  ...
  const passWarnList=clients.filter(c=>passportWarn(c.passportExpiry)&&!c.passportExpiryDismissed).map(c=>{ const d=daysUntilExpiry(c.passportExpiry); return {...c,_sub:`Caduca ${d!==null&&d>=0?`en ${d} días`:"VENCIDO"}`}; });
...
function ATrip({ go, tid, initTab, trips, clients, crm, sT, sC, sCrm }) {
                ...
                <React.Fragment key={c.id}>
                  <div style={{ display:"grid",gridTemplateColumns:`1fr ${pc.map(()=>"1fr").join(" ")}`,borderBottom:`1px solid ${A.border}44`,background:allPaid?A.green+"08":urgent?A.orange+"06":"transparent" }}>
                    <div style={{ padding:"10px 14px",display:"flex",alignItems:"center",gap:6 }}>
                      ...
                      <div><div style={{ fontFamily:ANTON,fontSize:12,color:A.text,lineHeight:1 }}>{c.nombre.split(" ")[0]}</div><div style={{ fontSize:8,color:fp.k==="vip"?A.gold:A.muted,fontFamily:BF }}>{fp.icon}</div></div>
                    </div>
                    {pc.map((p,i)=>{
  ...
  const FinanzasTab=()=>{
    ...
    const scanFactura=async(fa)=>{ 
      if(!fa.data) return; 
      setScanningFA(fa.id); setScanResult(null);
      try {
        const parts=fa.tipo?.includes("image")?
          [{ inline_data:{ mime_type:fa.tipo,data:fa.data } },{ text:"Analiza esta factura o ticket. Extrae: proveedor, fecha, importe total, moneda, concepto/descripción. Responde en español, de forma concisa y estructurada con etiquetas como Proveedor:, Fecha:, Importe:, Moneda:, Concepto:." }]:
          [{ text:`Analiza el siguiente documento (PDF en base64): ${fa.data.slice(0,800)}...\nExtrae: proveedor, fecha, importe total, moneda, concepto. Responde en español con Proveedor:, Fecha:, Importe:, Moneda:, Concepto:.` }];
        const text=await callGemini(parts);
        setScanResult({ id:fa.id, text });
          ...
          {facturas.filter(f=>!f.gastoId).length===0?<AEmpty text="Sin facturas generales" />:facturas.filter(f=>!f.gastoId).map(fa=>(
            <div key={fa.id} style={{ background:A.card,borderRadius:10,marginBottom:8,border:`1px solid ${A.gold}33`,overflow:"hidden" }}>
              <div style={{ padding:"10px 14px",display:"flex",alignItems:"center",gap:10 }}>
                ...
                <button onClick={()=>scanFactura(fa)} disabled={scanningFA===fa.id} style={{ ...ab(A.purple+"22",A.purple),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.purple}44`,fontFamily:BF,flexShrink:0 }}>{scanningFA===fa.id?"⏳":"🤖"}</button>
...
function Passport({ go, cid, clients, trips, sC, setScreen, setNav }) {
  ...
  const onFile = async e => {
    ...
    r.onload = async ev => {
      setPhoto(ev.target.result); setExtracting(true);
      try {
        const b64 = ev.target.result.split(",")[1]; const mt = f.type || "image/jpeg";
  ...
  const submit = async () => {
    if (!photo || !c1) return;
    await sC(clients.map(x => x.id === cid ? { ...x, passportPhoto: photo, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false } : x));
  ...
  return (
    <div style={{ fontFamily: BF, background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text }}>
      <div style={{ background: "linear-gradient(135deg,#07070f 0%,#0f1f3d 60%,#07070f 100%)", padding: "40px 24px 32px", borderBottom: `1px solid ${A.border}` }}>
        <div style={{ fontFamily: ANTON, fontSize: 36, color: "#fff", lineHeight: 0.9, marginBottom: 12 }}>{cl.nombre.split(" ")[0].toUpperCase()}</div>
        {trip && <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 12 }}><span style={{ fontSize: 40 }}>{trip.flag}</span><div><div style={{ fontFamily: ANTON, fontSize: 20, color: "#fff", letterSpacing: 1 }}>{trip.name.toUpperCase()}</div><div style={{ fontSize: 14, color: A.muted, fontFamily: BF, marginTop: 2 }}>{trip.fechas}</div></div></div>}
      </div>
      <div style={{ padding: "24px 20px" }}>
        <div style={{ background: A.card, borderRadius: 14, padding: 20, border: `1px solid ${A.border}`, marginBottom: 16 }}>
          ...
          <div onClick={() => ref.current.click()} style={{ background: photo ? "transparent" : A.card2, border: `2px dashed ${photo ? A.cyan : A.border}`, borderRadius: 12, height: 180, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", marginBottom: 16, position: "relative" }}>
            {photo ? <img src={photo} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" /> : <div style={{ textAlign: "center", color: A.muted }}><div style={{ fontSize: 40, marginBottom: 8 }}>📷</div><div style={{ fontFamily: ANTON, fontSize: 16, color: A.cyan, letterSpacing: 1 }}>TOCA PARA FOTOGRAFIAR</div></div>}
          </div>
          <input ref={ref} type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={onFile} />
          <div style={{ background: A.card2, borderRadius: 12, padding: "14px", marginBottom: 14, border: `1px solid ${extracting ? A.purple + "44" : expiry ? A.green + "44" : A.border}` }}>
            <div style={{ fontFamily: BF, fontSize: 11, color: extracting ? A.purple : expiry ? A.green : A.muted, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>{extracting ? "Leyendo con IA..." : expiry ? "Detectada" : "Caducidad del pasaporte"}</div>
            <input type="date" value={expiry} onChange={e => setExpiry(e.target.value)} style={{ ...ais, colorScheme: "dark" }} />
            {expiryWarn && <div style={{ background: A.orange + "22", border: `1px solid ${A.orange}44`, borderRadius: 8, padding: "8px 10px", marginTop: 8, fontSize: 12, color: A.orange, fontFamily: BF }}>{expiryDays !== null && expiryDays >= 0 ? `Caduca en ${expiryDays} días` : "Pasaporte vencido"}</div>}
          </div>
          <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 14, cursor: "pointer", padding: "12px", background: c1 ? A.green + "15" : "rgba(26,26,0,0.5)", borderRadius: 12, border: `2px solid ${c1 ? A.green : A.border}` }}>
            ...
            <span style={{ fontSize: 13, color: A.text, lineHeight: 1.55, fontFamily: BF }}>Obligatorio. Acepto que Travelike use mi foto de pasaporte para los trámites del viaje.</span>
          </label>
          <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 20, cursor: "pointer", padding: "12px", background: c2 ? A.purple + "15" : A.card2, borderRadius: 12, border: `2px solid ${c2 ? A.purple : A.border}` }}>
            <input type="checkbox" checked={c2} onChange={e => setC2(e.target.checked)} style={{ marginTop: 4, width: 20, height: 20, flexShrink: 0, accentColor: A.purple }} />
            <span style={{ fontSize: 13, color: A.text, lineHeight: 1.55, fontFamily: BF }}>Acepto que Travelike use mis fotos del viaje en redes sociales.</span>
          </label>
          <button onClick={submit} disabled={!photo || !c1} style={{ width: "100%", padding: "18px", border: "none", borderRadius: 12, fontFamily: ANTON, fontSize: 16, letterSpacing: 3, cursor: photo && c1 ? "pointer" : "default", background: photo && c1 ? `linear-gradient(90deg,${A.cyan},#0099bb)` : A.card2, color: photo && c1 ? A.bg : A.muted, textTransform: "uppercase" }}>CONTINUAR</button>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{ padding: "16px" }}>
          ...
          {[...vuelos, ...myDocs].map((v, i) => (
            <div key={i} style={{ background: A.card, border: `1px solid ${A.border}`, borderRadius: 18, padding: "20px", marginBottom: 14 }}>
              ...
              <button style={{ width: "100%", padding: "14px", border: "none", borderRadius: 10, fontFamily: ANTON, fontSize: 14, letterSpacing: 3, cursor: "pointer", background: `linear-gradient(90deg,${A.red},#c00020)`, color: "#fff", textTransform: "uppercase" }}>DESCARGAR</button>
            </div>
          ))}
        </div>
      </div>
