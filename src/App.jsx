...
const BANK_ACCOUNTS=[
  {id:"principal",label:"TraveLike SL — Principal",iban:"ES08 3190 0096 4064 3188 5125",titular:"TraveLike SL"},
  {id:"alternativa",label:"TraveLike SL — Alternativa",iban:"ES48 0128 0690 9001 0008 6284",titular:"TraveLike SL"},
];
const BANK_IBAN=BANK_ACCOUNTS[0].iban,BANK_TITULAR=BANK_ACCOUNTS[0].titular,WA_NUM="34606575646";
...
const ROOMS={doble_jun:{label:"🛌 King (cama doble)",short:"King",cap:2,color:A.cyan},doble_sep:{label:"🛏️ Twins (camas sep.)",short:"Twins",cap:2,color:A.lightblue},individual:{label:"👤 Individual",short:"Individual",cap:1,color:A.orange},triple:{label:"👨‍👩‍👦 Triple",short:"Triple",cap:3,color:A.gold},cuadruple:{label:"👨‍👩‍👧‍👦 Cuádruple",short:"Cuádruple",cap:4,color:A.purple},busca:{label:"🔍 Busca compañero/a",short:"Busca",cap:2,color:"#FF6B6B"}};
...
const emptyT=()=>({vuelos:[],docs:[],pagosConfig:[{label:"Reserva",fecha:"",fechaISO:"",importe:""},{label:"Pago Final",fecha:"",fechaISO:"",importe:""}],info:[],webUrl:"",hotels:[],emergencias:emptyEmergencias(),surveyEnabled:false,surveyConfig:{categories:[...DEFAULT_SURVEY_CATS],surveyResponses:[]},maletaImprescindibles:[...DEFAULT_IMPRESCINDIBLES],maletaCats:DEFAULT_MALETA_CATS.map(c=>({...c,items:[...c.items]})),gastos:[],facturas:[],facturasVenta:[],currency:"EUR",tripBankId:"principal"});
...
const stripBinary=(k,v)=>{
  if(k===SK_GG){if(!v||typeof v!=="object"||Array.isArray(v))return v;return{...v,facturas:(v.facturas||[]).map(f=>({...f,data:f.data?"__local__":null}))};}
  if(k!==SK_C&&k!==SK_T)return v;if(!Array.isArray(v))return v;return v.map(item=>{const stripped={...item};if(stripped.passportPhotos)stripped.passportPhotos=[];if(stripped.facturas)stripped.facturas=(stripped.facturas||[]).map(f=>({...f,data:f.data?"__local__":null}));return stripped;});};
const mergeBinary=(k,supaVal,localVal)=>{
  if(k===SK_GG){if(!supaVal||!localVal||typeof supaVal!=="object"||typeof localVal!=="object")return supaVal;const merged={...supaVal};if(supaVal.facturas&&localVal.facturas){merged.facturas=supaVal.facturas.map(f=>{const lf=localVal.facturas?.find(lf2=>lf2.id===f.id);return lf?.data&&lf.data!=="__local__"?{...f,data:lf.data}:f;});}return merged;}
  if((k!==SK_C&&k!==SK_T)||!Array.isArray(supaVal)||!Array.isArray(localVal))return supaVal;return supaVal.map(item=>{const local=localVal.find(l=>l.id===item.id);if(!local)return item;const merged={...item};if(k===SK_C){if(local.passportPhotos?.length)merged.passportPhotos=local.passportPhotos;}if(k===SK_T){if(local.facturas?.length){merged.facturas=item.facturas?.map(f=>{const lf=local.facturas?.find(lf2=>lf2.id===f.id);return lf?.data&&lf.data!=="__local__"?{...f,data:lf.data}:f;})||local.facturas;}}return merged;});};
...
function CartaDePagoGenerator({onBack}){
  ...
  const[selectedBankId,setSelectedBankId]=useState("principal");
  ...
  const PreviewCard=()=>(
    <div style={{border:`1px solid ${A.border}`,borderRadius:12,overflow:"hidden",background:"#fff"}}>
      ...
      <div style={{padding:"12px 18px"}}>
        ...
        <div style={{background:"#0D2137",borderRadius:6,padding:"8px 12px",marginBottom:10}}><div style={{fontSize:7,color:"#1AB5B0",letterSpacing:2,marginBottom:3}}>DATOS BANCARIOS</div><div style={{fontSize:10,color:"#fff",fontWeight:600}}>{selectedBank.iban}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.4)",marginTop:1}}>Titular: {selectedBank.titular}</div></div>
    ...
    <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF,paddingBottom:80}}>
      ...
      <div style={{padding:"14px 16px"}}>
        {tab==="general"&&(<div>
          ...
          <div style={{fontFamily:BF,fontSize:9,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${A.border}`}}>🏦 CUENTA BANCARIA</div>
          {BANK_ACCOUNTS.map(ba=>(<button key={ba.id} onClick={()=>setSelectedBankId(ba.id)} style={{width:"100%",background:selectedBankId===ba.id?A.cyan+"18":A.card2,border:`2px solid ${selectedBankId===ba.id?A.cyan:A.border}`,borderRadius:10,padding:"10px 14px",marginBottom:8,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10}}><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:12,fontWeight:700,color:selectedBankId===ba.id?A.cyan:"#fff"}}>{ba.label}</div><div style={{fontFamily:"monospace",fontSize:11,color:A.muted,marginTop:2}}>{ba.iban}</div></div>{selectedBankId===ba.id&&<span style={{color:A.cyan,fontSize:16}}>✓</span>}</button>))}
          <div style={{fontFamily:BF,fontSize:9,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${A.border}`,marginTop:6}}>LOGOTIPO</div>
      ...
      <div style={{position:"fixed",left:-9999,top:0,width:794,pointerEvents:"none",zIndex:-1}}>
        <div ref={vRef} style={{width:794,minHeight:1000,background:"#fff",fontFamily:"'Barlow Condensed',sans-serif"}}>
          ...
          <div style={{padding:"32px 40px"}}>
            ...
            <div style={{background:"#0D2137",borderRadius:8,padding:"16px 20px",marginBottom:24}}><div style={{fontSize:9,color:"#1AB5B0",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Datos bancarios para transferencia</div><div style={{display:"flex",gap:32}}><div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>IBAN</div><div style={{fontSize:13,color:"#fff",fontWeight:600}}>{selectedBank.iban}</div></div><div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Titular</div><div style={{fontSize:13,color:"#fff",fontWeight:600}}>{selectedBank.titular}</div></div></div></div>
...
function GastosGenerales({onBack,gastosGen,setGastosGen}){
  ...
  const upd=fn=>{const next={...gastosGen,...fn(gastosGen)};setGastosGen(next);};
  ...
  const normImporte=v=>(v||"").replace(",",".");
  const addGasto=()=>{const imp=normImporte(gImporte);if(!imp||+imp<=0)return;const importeEUR=gCurrency==="EUR"?+imp:+(normImporte(gImporteEUR)||imp);const next={...gastosGen,gastos:[...(gastosGen.gastos||[]),{id:uid(),tipo:gTipo,descripcion:gDesc,importe:+imp,currency:gCurrency,importeEUR,fecha:gFecha,nota:gNota,facturas:[]}]};setGastosGen(next);setGTipo("otros");setGDesc("");setGImporte("");setGImporteEUR("");setGFecha(new Date().toISOString().split("T")[0]);setGNota("");setShowForm(false);};
      ...
      <div style={{padding:"12px 16px"}}>
        ...
        {showForm&&(
          <div style={{background:A.card2,borderRadius:14,padding:"14px",marginBottom:12,border:`1px solid ${A.orange}44`}}>
            ...
            <button onClick={addGasto} disabled={!gImporte||+normImporte(gImporte)<=0} style={{width:"100%",padding:"13px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:15,letterSpacing:2,cursor:"pointer",background:gImporte&&+normImporte(gImporte)>0?A.orange:"#1a1a1a",color:"#fff",textTransform:"uppercase"}}>REGISTRAR GASTO</button>
          </div>
        )}
...
// ══════════════════════════════════════════════════════════
// PROGRAMA BUILDER CONSTANTS (top-level)
const PGOLD="#d4a843";const PDARK="#06060f";const PGREEN="#22c55e";const PRED="#e74c3c";
const pinp={width:"100%",background:"#0a0a18",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,padding:"9px 12px",color:"#f0ede8",fontFamily:"Outfit,sans-serif",fontSize:13,outline:"none",boxSizing:"border-box"};
const paddBtn={background:"rgba(212,168,67,0.14)",border:"1.5px dashed rgba(212,168,67,0.4)",borderRadius:10,padding:"9px 14px",color:PGOLD,fontFamily:"Outfit,sans-serif",fontSize:12,cursor:"pointer"};
const puid=()=>Math.random().toString(36).slice(2,9);
const pLoadScript=src=>new Promise((res,rej)=>{if(document.querySelector('script[src="'+src+'"]')){return res();}const s=document.createElement("script");s.src=src;s.onload=res;s.onerror=rej;document.head.appendChild(s);});
const pGeocode=async name=>{try{const r=await fetch("https://nominatim.openstreetmap.org/search?format=json&q="+encodeURIComponent(name)+"&limit=1");const d=await r.json();if(d[0])return{lat:+d[0].lat,lng:+d[0].lon};return null;}catch{return null;}};
const pmakeGlobalBg=()=>({enabled:false,url:"",opacity:0.08,baseColor:"#07070f"});

// PROGRAMA BUILDER HELPERS + PTLPage
// ══════════════════════════════════════════════════════════
function PLabel({children}){return <div style={{fontFamily:"Outfit",fontSize:10,color:"#666",letterSpacing:1.2,textTransform:"uppercase",marginBottom:5}}>{children}</div>;}
function PField({label,value,onChange,placeholder}){return(<div style={{marginBottom:11}}><PLabel>{label}</PLabel><input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder||""} style={pinp}/></div>);}
function PTextarea({label,value,onChange,rows=3}){return(<div style={{marginBottom:11}}><PLabel>{label}</PLabel><textarea value={value} onChange={e=>onChange(e.target.value)} rows={rows} style={{...pinp,resize:"vertical",lineHeight:1.65}}/></div>);}
function PColorPicker({label,value,onChange}){return(<div style={{marginBottom:11}}><PLabel>{label}</PLabel><div style={{display:"flex",alignItems:"center",gap:9}}><div style={{position:"relative",width:34,height:34,borderRadius:8,overflow:"hidden",border:"1px solid rgba(255,255,255,0.15)",background:value,flexShrink:0}}><input type="color" value={value} onChange={e=>onChange(e.target.value)} style={{position:"absolute",inset:-4,width:"calc(100% + 8px)",height:"calc(100% + 8px)",opacity:0,cursor:"pointer"}}/></div><input value={value} onChange={e=>onChange(e.target.value)} style={{...pinp,flex:1,fontFamily:"monospace",fontSize:12}}/></div></div>);}
function PSlider({label,value,onChange,min=0,max=1,step=0.05}){return(<div style={{marginBottom:11}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><PLabel>{label}</PLabel><span style={{fontFamily:"Outfit",fontSize:11,color:PGOLD}}>{Math.round(value*100)}%</span></div><input type="range" min={min} max={max} step={step} value={value} onChange={e=>onChange(Number(e.target.value))} style={{width:"100%",accentColor:PGOLD}}/></div>);}
function PToggle({options,value,onChange}){return(<div style={{display:"flex",gap:4,marginBottom:11,background:"rgba(255,255,255,0.04)",borderRadius:9,padding:3}}>{options.map(([v,label])=>(<button key={v} onClick={()=>onChange(v)} style={{flex:1,padding:"6px 0",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"Outfit",fontSize:11,fontWeight:700,background:value===v?PGOLD:"transparent",color:value===v?PDARK:"#666",transition:"all .18s"}}>{label}</button>))}</div>);}
function PActBtn({onClick,danger,children}){return(<button onClick={onClick} style={{width:26,height:26,borderRadius:6,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,background:danger?"rgba(231,76,60,0.14)":"rgba(255,255,255,0.07)",color:danger?PRED:"#888"}}>{children}</button>);}
function PPhotoSourcePicker({url,onChange}){const fileRef=useRef();const isBase64=url&&url.startsWith("data:");const handleFile=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>onChange(ev.target.result);r.readAsDataURL(file);};return(<div style={{marginBottom:11}}><PLabel>Fuente de la foto</PLabel><div style={{display:"flex",gap:4,marginBottom:9,background:"rgba(255,255,255,0.04)",borderRadius:9,padding:3}}>{[["url","🔗 URL"],["local","📁 Subir foto"]].map(([v,lbl])=>{const active=v==="local"?isBase64:!isBase64;return(<button key={v} onClick={()=>{if(v==="local")fileRef.current?.click();else onChange("");}} style={{flex:1,padding:"6px 0",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"Outfit",fontSize:11,fontWeight:700,background:active?PGOLD:"transparent",color:active?PDARK:"#666"}}>{lbl}</button>);})}</div><input ref={fileRef} type="file" accept="image/*" onChange={handleFile} style={{display:"none"}}/>{!isBase64&&<input value={url||""} onChange={e=>onChange(e.target.value)} placeholder="https://..." style={pinp}/>}{isBase64&&(<div style={{position:"relative",borderRadius:9,overflow:"hidden",border:"1px solid rgba(212,168,67,0.25)"}}><img src={url} alt="preview" style={{width:"100%",height:90,objectFit:"cover",display:"block"}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to right,transparent 55%,rgba(6,6,15,0.9))",display:"flex",alignItems:"center",justifyContent:"flex-end",padding:"0 10px",gap:6}}><button onClick={()=>fileRef.current?.click()} style={{padding:"5px 10px",borderRadius:7,border:"none",background:PGOLD,color:PDARK,fontFamily:"Outfit",fontSize:10,fontWeight:700,cursor:"pointer"}}>Cambiar</button><button onClick={()=>onChange("")} style={{padding:"5px 10px",borderRadius:7,border:"none",background:"rgba(231,76,60,0.8)",color:"#fff",fontFamily:"Outfit",fontSize:10,fontWeight:700,cursor:"pointer"}}>Quitar</button></div></div>)}</div>);}
function PBgEditor({bg,onChange,label="Fondo"}){return(<div style={{padding:13,background:"rgba(255,255,255,0.025)",borderRadius:11,marginBottom:11,border:"1px solid rgba(255,255,255,0.07)"}}><div style={{fontFamily:"Outfit",fontSize:10,color:PGOLD,letterSpacing:1.5,textTransform:"uppercase",marginBottom:11}}>{label}</div><PToggle options={[["color","🎨 Color liso"],["photo","📷 Foto"]]} value={bg.type} onChange={t=>onChange({...bg,type:t})}/>{bg.type==="color"&&<PColorPicker label="Color de fondo" value={bg.color} onChange={c=>onChange({...bg,color:c})}/>}{bg.type==="photo"&&(<><PPhotoSourcePicker url={bg.url} onChange={u=>onChange({...bg,url:u})}/><PSlider label="Opacidad de la foto" value={bg.opacity} onChange={o=>onChange({...bg,opacity:o})}/><PColorPicker label="Color base (detrás de la foto)" value={bg.baseColor} onChange={c=>onChange({...bg,baseColor:c})}/></>)}</div>);}
function PSectionCard({title,open,onToggle,accent=PGOLD,actions,children}){return(<div style={{marginBottom:9,borderRadius:13,overflow:"hidden",border:`1px solid ${open?accent+"44":"rgba(255,255,255,0.07)"}`,transition:"border-color .2s"}}><div onClick={onToggle} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 13px",background:open?accent+"18":"rgba(255,255,255,0.03)",cursor:"pointer",userSelect:"none"}}><div style={{display:"flex",alignItems:"center",gap:9}}><div style={{width:3,height:16,borderRadius:2,background:open?accent:"rgba(255,255,255,0.18)"}}/><span style={{fontFamily:"Anton",fontSize:11,letterSpacing:2,color:open?"#fff":"#777"}}>{title}</span></div><div style={{display:"flex",alignItems:"center",gap:6}} onClick={e=>e.stopPropagation()}>{actions}<span style={{color:"#555",fontSize:13,transition:"transform .2s",transform:open?"rotate(180deg)":"none",pointerEvents:"none"}}>▾</span></div></div>{open&&<div style={{padding:"13px",background:"rgba(0,0,0,0.22)"}}>{children}</div>}</div>);}
function PBlockCard({block,onChange,onRemove}){return(<div style={{padding:11,marginBottom:7,background:"rgba(255,255,255,0.03)",borderRadius:9,border:"1px solid rgba(255,255,255,0.07)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}><span style={{fontFamily:"Outfit",fontSize:9,color:PGOLD,letterSpacing:1.2,textTransform:"uppercase"}}>{block.type==="text-badge"?"✦ Texto + Emoji":"✦ Foto + Texto"}</span><button onClick={onRemove} style={{width:22,height:22,borderRadius:5,border:"none",background:"rgba(231,76,60,0.14)",color:PRED,cursor:"pointer",fontSize:11}}>✕</button></div>{block.type==="text-badge"&&(<><PTextarea label="Texto del bloque" value={block.text} onChange={v=>onChange({text:v})}/><div style={{display:"flex",gap:8}}><div style={{flex:1}}><PField label="Emoji badge" value={block.badge} onChange={v=>onChange({badge:v})} placeholder="🌍"/></div><div style={{flexShrink:0,minWidth:130}}><PLabel>Badge posición</PLabel><PToggle options={[["right","Derecha →"],["left","← Izquierda"]]} value={block.badgeRight?"right":"left"} onChange={v=>onChange({badgeRight:v==="right"})}/></div></div></>)}{block.type==="photo-text"&&(<><PPhotoSourcePicker url={block.photoUrl||""} onChange={v=>onChange({photoUrl:v})}/><PTextarea label="Texto descriptivo" value={block.text} onChange={v=>onChange({text:v})}/><PLabel>Posición de la foto</PLabel><PToggle options={[["left","📷 Izquierda"],["right","Derecha 📷"]]} value={block.photoLeft?"left":"right"} onChange={v=>onChange({photoLeft:v==="left"})}/></>)}</div>);}
function PListItemRow({value,onChange,onRemove}){return(<div style={{display:"flex",gap:6,marginBottom:6}}><input value={value} onChange={e=>onChange(e.target.value)} style={{...pinp,flex:1}}/><button onClick={onRemove} style={{width:32,flexShrink:0,borderRadius:8,border:"none",background:"rgba(231,76,60,0.12)",color:PRED,cursor:"pointer",fontSize:12}}>✕</button></div>);}
function PMapStopEditor({stops,onChange}){const[loading,setLoading]=useState(null);const geocode=async(stopId,name)=>{if(!name.trim())return;setLoading(stopId);const coords=await pGeocode(name);setLoading(null);if(coords)onChange(stops.map(s=>s.id===stopId?{...s,...coords}:s));else alert(`No se encontró "${name}".`);};const add=()=>onChange([...stops,{id:puid(),name:"",lat:null,lng:null}]);const remove=id=>onChange(stops.filter(s=>s.id!==id));const update=(id,key,val)=>onChange(stops.map(s=>s.id===id?{...s,[key]:val}:s));const move=(i,dir)=>{const arr=[...stops],to=i+dir;if(to<0||to>=arr.length)return;[arr[i],arr[to]]=[arr[to],arr[i]];onChange(arr);};return(<div><div style={{fontFamily:"Outfit",fontSize:11,color:"#666",marginBottom:11,lineHeight:1.6}}>Escribe cada destino y pulsa 📍 para localizarlo.</div>{stops.map((s,i)=>(<div key={s.id} style={{display:"flex",gap:5,alignItems:"center",marginBottom:7}}><div style={{width:20,height:20,borderRadius:"50%",background:PGOLD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontFamily:"Anton",fontSize:9,color:PDARK}}>{i+1}</span></div><input value={s.name} onChange={e=>update(s.id,"name",e.target.value)} onKeyDown={e=>e.key==="Enter"&&geocode(s.id,s.name)} placeholder="Ciudad o destino..." style={{...pinp,flex:1}}/>{s.lat&&<span style={{fontFamily:"Outfit",fontSize:10,color:PGREEN,flexShrink:0}}>✓</span>}<button onClick={()=>geocode(s.id,s.name)} disabled={loading===s.id||!s.name} style={{width:28,height:28,borderRadius:7,border:"none",flexShrink:0,background:"rgba(212,168,67,0.14)",color:PGOLD,cursor:s.name?"pointer":"not-allowed",fontSize:13}}>{loading===s.id?"⏳":"📍"}</button>{i>0&&<PActBtn onClick={()=>move(i,-1)}>↑</PActBtn>}{i<stops.length-1&&<PActBtn onClick={()=>move(i,1)}>↓</PActBtn>}<PActBtn onClick={()=>remove(s.id)} danger>✕</PActBtn></div>))}<button onClick={add} style={{...paddBtn,width:"100%",marginTop:4}}>+ Añadir destino</button></div>);}
function PMapSVG({stops,accentColor,width,height}){const svgRef=useRef(null);const[ready,setReady]=useState(false);useEffect(()=>{Promise.all([pLoadScript("https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.5/d3.min.js"),pLoadScript("https://cdnjs.cloudflare.com/ajax/libs/topojson/3.0.2/topojson.min.js")]).then(()=>fetch("https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json")).then(r=>r.json()).then(world=>{const d3=window.d3,topojson=window.topojson;const el=svgRef.current;if(!el)return;const W=width||el.clientWidth||390,H=height||el.clientHeight||700;d3.select(el).selectAll("*").remove();const svg=d3.select(el).attr("viewBox",`0 0 ${W} ${H}`);const validStops=stops.filter(s=>s.lat&&s.lng);const allCountries=topojson.feature(world,world.objects.countries);const visitedFeatures=allCountries.features.filter(f=>validStops.some(s=>{try{return d3.geoContains(f,[s.lng,s.lat]);}catch(e){return false;}}));const proj=d3.geoNaturalEarth1();if(validStops.length===0){proj.fitExtent([[20,60],[W-20,H-40]],{type:"Sphere"});}else if(visitedFeatures.length>0){proj.fitExtent([[32,72],[W-32,H-48]],{type:"FeatureCollection",features:visitedFeatures});}else{const lngs=validStops.map(s=>s.lng),lats=validStops.map(s=>s.lat);const padLng=Math.max((Math.max(...lngs)-Math.min(...lngs))*1.2,30),padLat=Math.max((Math.max(...lats)-Math.min(...lats))*1.2,25);proj.fitExtent([[24,60],[W-24,H-36]],{type:"Feature",geometry:{type:"Polygon",coordinates:[[[Math.min(...lngs)-padLng,Math.min(...lats)-padLat],[Math.max(...lngs)+padLng,Math.min(...lats)-padLat],[Math.max(...lngs)+padLng,Math.max(...lats)+padLat],[Math.min(...lngs)-padLng,Math.max(...lats)+padLat],[Math.min(...lngs)-padLng,Math.min(...lats)-padLat]]]}})}const path=d3.geoPath().projection(proj);const visitedIds=new Set(visitedFeatures.map(f=>f.id));svg.insert("path","path").datum({type:"Sphere"}).attr("d",path).attr("fill","#0a0a20");svg.append("path").datum(d3.geoGraticule()()).attr("d",path).attr("fill","none").attr("stroke","rgba(255,255,255,0.03)").attr("stroke-width",0.5);svg.append("g").selectAll("path").data(allCountries.features.filter(f=>!visitedIds.has(f.id))).join("path").attr("d",path).attr("fill","#141430").attr("stroke","#252560").attr("stroke-width",0.4);if(visitedFeatures.length>0){svg.append("g").selectAll("path").data(visitedFeatures).join("path").attr("d",path).attr("fill",accentColor).attr("fill-opacity",0.22).attr("stroke",accentColor).attr("stroke-width",1.2).attr("stroke-opacity",0.7);}svg.append("path").datum(topojson.mesh(world,world.objects.countries,(a,b)=>a!==b)).attr("d",path).attr("fill","none").attr("stroke","rgba(80,80,160,0.15)").attr("stroke-width",0.3);if(validStops.length>1){const coords=validStops.map(s=>proj([s.lng,s.lat]));for(let i=0;i<coords.length-1;i++){const[x1,y1]=coords[i],[x2,y2]=coords[i+1];const mx=(x1+x2)/2,my=(y1+y2)/2-Math.hypot(x2-x1,y2-y1)*0.18;const cp="M"+x1+","+y1+" Q"+mx+","+my+" "+x2+","+y2;svg.append("path").attr("d",cp).attr("fill","none").attr("stroke",PGOLD).attr("stroke-width",1.5).attr("stroke-dasharray","6,4").attr("opacity",0.85);}}validStops.forEach((stop,i)=>{const[x,y]=proj([stop.lng,stop.lat]);const g=svg.append("g").attr("transform","translate("+x+","+y+")");g.append("circle").attr("r",9).attr("fill",PGOLD).attr("opacity",0.92);g.append("text").attr("text-anchor","middle").attr("dominant-baseline","central").attr("font-size",8).attr("font-family","Anton,sans-serif").attr("fill","#06060f").attr("font-weight","bold").text(i+1);const labelAbove=y>H*0.72;const ly=labelAbove?-22:22;const lw=stop.name.length*6.8+18;g.append("rect").attr("x",-lw/2).attr("y",ly-11).attr("width",lw).attr("height",17).attr("rx",5).attr("fill","rgba(6,6,15,0.9)").attr("stroke",PGOLD).attr("stroke-width",0.5).attr("stroke-opacity",0.3);g.append("text").attr("text-anchor","middle").attr("y",ly).attr("dominant-baseline","central").attr("font-size",10.5).attr("font-family","Outfit,sans-serif").attr("font-weight","600").attr("fill","#f0ede8").text(stop.name);});setReady(true);}).catch(()=>{});},[stops,accentColor,width,height]);return(<><svg ref={svgRef} style={{position:"absolute",inset:0,width:"100%",height:"100%"}}/>{!ready&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{textAlign:"center"}}><div style={{width:32,height:32,border:"2px solid rgba(212,168,67,0.1)",borderTop:"2px solid "+PGOLD,borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 10px"}}/><div style={{fontFamily:"Outfit",fontSize:10,color:"#555",letterSpacing:2}}>CARGANDO MAPA...</div></div></div>}</>);}

function PTLPage({prog,pageIndex,totalPages}){
  const mapOffset=prog.mapEnabled?1:0;const tc=prog.textColor||"#f0ede8";const gb=prog.globalBg||pmakeGlobalBg();const ac=prog.cover.accentColor;
  const BgLayers=({bg})=>{if(!bg)return<div style={{position:"absolute",inset:0,background:PDARK}}/>;if(bg.type==="color")return<div style={{position:"absolute",inset:0,background:bg.color||PDARK}}/>;return<><div style={{position:"absolute",inset:0,background:bg.baseColor||PDARK}}/>{bg.url&&<img src={bg.url} crossOrigin="anonymous" alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:bg.opacity,display:"block"}}/>}<div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,rgba(0,0,0,0.05) 0%,rgba(5,5,12,0.5) 100%)"}}/></>;};
  const GlobalBg=()=>{if(!gb.enabled||!gb.url)return null;return<><div style={{position:"absolute",inset:0,background:gb.baseColor||PDARK,zIndex:3}}/><img src={gb.url} crossOrigin="anonymous" alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:gb.opacity,zIndex:4,display:"block"}}/></>;};
  const blockStyle={background:"rgba(8,5,5,0.9)",borderRadius:16,padding:"14px 17px",color:tc,fontSize:13,lineHeight:1.72,textAlign:"center",border:"1px solid rgba(212,168,67,0.18)"};
  const badgeStyle={width:66,height:66,minWidth:66,borderRadius:"50%",background:"#0f0f0f",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,flexShrink:0,border:"2px solid rgba(212,168,67,0.35)",overflow:"hidden"};
  const renderBlocks=day=>day.blocks.map(b=>{if(b.type==="text-badge"){const right=b.badgeRight!==false;return(<div key={b.id} style={{display:"flex",gap:12,alignItems:"flex-start"}}>{!right&&<div style={badgeStyle}>{b.badge}</div>}<div style={{flex:1}}><div style={blockStyle}>{b.text}</div></div>{right&&<div style={badgeStyle}>{b.badge}</div>}</div>);}return(<div key={b.id} style={{display:"flex",gap:12,alignItems:"flex-start"}}>{b.photoLeft&&b.photoUrl&&<div style={{width:"44%",flexShrink:0,borderRadius:12,overflow:"hidden",height:140}}><img src={b.photoUrl} crossOrigin="anonymous" alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/></div>}<div style={{flex:1}}><div style={blockStyle}>{b.text}</div></div>{!b.photoLeft&&b.photoUrl&&<div style={{width:"44%",flexShrink:0,borderRadius:12,overflow:"hidden",height:140}}><img src={b.photoUrl} crossOrigin="anonymous" alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/></div>}</div>);});
  const PageNum=({num})=>(<div style={{position:"absolute",bottom:13,right:16,zIndex:50,fontFamily:"Outfit",fontSize:9,letterSpacing:2.5,color:"rgba(212,168,67,0.5)",textTransform:"uppercase",pointerEvents:"none"}}>{String(num).padStart(2,"0")} · {String(totalPages).padStart(2,"0")}</div>);
  const WebUrl=()=>(<div style={{position:"absolute",bottom:13,left:"50%",transform:"translateX(-50%)",fontSize:10,letterSpacing:2.5,color:PGOLD,opacity:0.72,whiteSpace:"nowrap",fontFamily:"Outfit",zIndex:50}}>WWW.TRAVELIKE.COM</div>);
  if(pageIndex===0){return(<div style={{position:"absolute",inset:0,overflow:"hidden"}}><BgLayers bg={prog.cover.bg}/><GlobalBg/><div style={{position:"absolute",top:0,left:0,right:0,height:5,background:"linear-gradient(90deg,"+ac+","+PGOLD+","+ac+")",zIndex:10}}/><div style={{position:"absolute",inset:0,zIndex:9,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",paddingBottom:62,paddingLeft:28,paddingRight:28,textAlign:"center"}}><div style={{fontFamily:"Outfit",fontSize:10,letterSpacing:5,color:PGOLD,textTransform:"uppercase",marginBottom:10}}>{prog.cover.overtitle}</div><h1 style={{fontFamily:"Anton",fontSize:44,letterSpacing:2,color:tc,textTransform:"uppercase",lineHeight:1.05,marginBottom:16}}>PROGRAMA<br/>{prog.cover.title}</h1><div style={{width:50,height:2,background:"linear-gradient(90deg,transparent,"+PGOLD+",transparent)",marginBottom:14}}/>{prog.cover.clientName&&<div style={{fontFamily:"Outfit",fontSize:13,letterSpacing:4,color:tc,textTransform:"uppercase",fontWeight:300,marginBottom:22}}>{prog.cover.clientName}</div>}{prog.cover.price&&(<div style={{marginBottom:22,padding:"14px 28px",background:"rgba(0,0,0,0.85)",borderRadius:18,border:"1.5px solid "+PGOLD+"44",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>{prog.cover.priceFrom&&<div style={{fontFamily:"Outfit",fontSize:9,letterSpacing:4,color:"#aaa",textTransform:"uppercase"}}>DESDE</div>}<div style={{display:"flex",alignItems:"flex-start",gap:3}}><span style={{fontFamily:"Outfit",fontSize:18,color:PGOLD,fontWeight:300,marginTop:7}}>€</span><span style={{fontFamily:"Anton",fontSize:50,color:PGOLD,lineHeight:1,letterSpacing:-1}}>{prog.cover.price}</span></div>{prog.cover.priceNote&&<div style={{fontFamily:"Outfit",fontSize:10,color:"rgba(212,168,67,0.7)",letterSpacing:1}}>{prog.cover.priceNote}</div>}</div>)}<div style={{display:"flex",gap:24,alignItems:"center"}}>{prog.cover.stats.map((s,i)=>(<div key={s.id||i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}><span style={{fontFamily:"Anton",fontSize:26,color:tc,lineHeight:1}}>{s.value}</span><span style={{fontFamily:"Outfit",fontSize:9,color:"#888",letterSpacing:2}}>{s.label}</span></div>))}</div></div>{prog.days.length>0&&<div style={{position:"absolute",bottom:0,left:0,right:0,height:44,background:ac,display:"flex",alignItems:"center",justifyContent:"center",borderTop:"2px solid "+PGOLD,zIndex:10}}><span style={{fontFamily:"Anton",fontSize:12,letterSpacing:2.5,color:tc}}>{prog.mapEnabled?"🗺 VER RUTA DEL VIAJE →":"DÍA 1: "+prog.days[0].title.toUpperCase()+" →"}</span></div>}<PageNum num={1}/></div>);}
  if(prog.mapEnabled&&pageIndex===1){return(<div style={{position:"absolute",inset:0,overflow:"hidden",background:"#06060f"}}><GlobalBg/><div style={{position:"absolute",top:0,left:0,right:0,zIndex:10,background:ac,padding:"11px 18px 12px",borderBottom:"2px solid "+PGOLD}}><div style={{fontFamily:"Outfit",fontSize:9,letterSpacing:3,color:PGOLD,textTransform:"uppercase",marginBottom:2}}>{prog.cover.overtitle}</div><div style={{fontFamily:"Anton",fontSize:16,letterSpacing:1,color:tc,textTransform:"uppercase"}}>RUTA DEL VIAJE · {prog.cover.title.toUpperCase()}</div></div><div style={{position:"absolute",top:52,left:0,right:0,bottom:32,zIndex:6}}><PMapSVG stops={prog.mapStops} accentColor={ac}/></div>{prog.mapStops.filter(s=>s.name).length>0&&<div style={{position:"absolute",bottom:32,left:14,zIndex:15,background:"rgba(6,6,15,0.92)",borderRadius:12,border:"1px solid rgba(212,168,67,0.2)",padding:"10px 14px"}}>{prog.mapStops.filter(s=>s.name).map((s,i)=>(<div key={s.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:i<prog.mapStops.filter(x=>x.name).length-1?6:0}}><div style={{width:17,height:17,borderRadius:"50%",background:PGOLD,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontFamily:"Anton",fontSize:8,color:PDARK}}>{i+1}</span></div><span style={{fontFamily:"Outfit",fontSize:10,color:"#e8e0d0"}}>{s.name}</span></div>))}</div>}<WebUrl/><PageNum num={2}/></div>);}
  const dayIndex=pageIndex-1-mapOffset;
  if(dayIndex>=0&&dayIndex<prog.days.length){const day=prog.days[dayIndex];return(<div style={{position:"absolute",inset:0,overflow:"hidden"}}><BgLayers bg={day.bg}/><GlobalBg/><div style={{position:"absolute",top:0,left:0,right:0,zIndex:10,background:day.headerColor,padding:"11px 18px 12px",borderBottom:"2px solid "+PGOLD}}><div style={{fontFamily:"Outfit",fontSize:9,letterSpacing:3,color:PGOLD,textTransform:"uppercase",marginBottom:2}}>{prog.cover.overtitle}</div><div style={{fontFamily:"Anton",fontSize:16,letterSpacing:1,color:tc,textTransform:"uppercase"}}>DÍA {day.num}: {day.title.toUpperCase()}</div></div><div style={{position:"absolute",inset:0,zIndex:9,paddingTop:74,paddingBottom:42,paddingLeft:14,paddingRight:14,display:"flex",flexDirection:"column",gap:18,justifyContent:"center"}}>{renderBlocks(day)}</div><WebUrl/><PageNum num={2+mapOffset+dayIndex}/></div>);}
  if(pageIndex===totalPages-2){return(<div style={{position:"absolute",inset:0,background:PDARK,overflow:"hidden"}}><GlobalBg/><div style={{position:"absolute",top:0,left:0,right:0,height:4,background:"linear-gradient(90deg,"+PGREEN+","+PGOLD+","+PGREEN+")",zIndex:10}}/><div style={{position:"absolute",inset:0,zIndex:9,overflowY:"auto",padding:"26px 18px 40px",display:"flex",flexDirection:"column",gap:16}}><div style={{textAlign:"center"}}><div style={{fontFamily:"Outfit",fontSize:10,letterSpacing:4,color:PGOLD,marginBottom:8}}>{prog.cover.title}</div><h2 style={{fontFamily:"Anton",fontSize:26,color:tc,letterSpacing:1,lineHeight:1.1}}>QUÉ INCLUYE<br/>EL PROGRAMA</h2><div style={{width:44,height:2,background:"linear-gradient(90deg,transparent,"+PGOLD+",transparent)",margin:"10px auto 0"}}/></div><div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><div style={{width:24,height:24,borderRadius:"50%",background:"rgba(34,197,94,0.15)",border:"1.5px solid "+PGREEN,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:PGREEN,fontSize:12}}>✓</span></div><span style={{fontFamily:"Anton",fontSize:12,letterSpacing:3,color:PGREEN}}>INCLUYE</span></div><div style={{display:"flex",flexDirection:"column",gap:5}}>{prog.includes.map(it=>(<div key={it.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"rgba(34,197,94,0.06)",borderRadius:10,border:"1px solid rgba(34,197,94,0.14)"}}><span style={{color:PGREEN,fontSize:12,flexShrink:0}}>✓</span><span style={{fontFamily:"Outfit",fontSize:12,color:tc,lineHeight:1.4}}>{it.text}</span></div>))}</div></div><div><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><div style={{width:24,height:24,borderRadius:"50%",background:"rgba(155,28,28,0.2)",border:"1.5px solid #9B1C1C",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{color:PRED,fontSize:12}}>✗</span></div><span style={{fontFamily:"Anton",fontSize:12,letterSpacing:3,color:PRED}}>NO INCLUYE</span></div><div style={{display:"flex",flexDirection:"column",gap:5}}>{prog.excludes.map(it=>(<div key={it.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:"rgba(155,28,28,0.07)",borderRadius:10,border:"1px solid rgba(155,28,28,0.16)"}}><span style={{color:PRED,fontSize:12,flexShrink:0}}>✗</span><span style={{fontFamily:"Outfit",fontSize:12,color:tc,lineHeight:1.4}}>{it.text}</span></div>))}</div></div></div><PageNum num={totalPages-1}/></div>);}
  return(<div style={{position:"absolute",inset:0,background:PDARK,overflow:"hidden"}}><GlobalBg/><div style={{position:"absolute",inset:0,background:"linear-gradient(160deg,"+ac+"22 0%,"+PDARK+" 45%)",zIndex:5}}/><div style={{position:"absolute",top:0,left:0,right:0,height:4,background:"linear-gradient(90deg,"+ac+","+PGOLD+","+ac+")",zIndex:10}}/><div style={{position:"absolute",inset:0,zIndex:9,display:"flex",flexDirection:"column",padding:"28px 22px 36px",justifyContent:"space-between"}}><div style={{textAlign:"center"}}><div style={{display:"inline-block",padding:"4px 14px",borderRadius:20,background:ac+"33",border:"1px solid "+ac+"66",marginBottom:14}}><span style={{fontFamily:"Outfit",fontSize:9,letterSpacing:3,color:ac,textTransform:"uppercase",fontWeight:700}}>✦ {prog.cta.tagline}</span></div><h2 style={{fontFamily:"Anton",fontSize:30,color:tc,letterSpacing:1,lineHeight:1.1,marginBottom:8,textTransform:"uppercase"}}>{prog.cta.headline}</h2><div style={{fontFamily:"Outfit",fontSize:10,color:"#666",letterSpacing:2}}>{prog.cover.title.toUpperCase()}</div></div><div style={{textAlign:"center",padding:"18px 0",borderTop:"1px solid rgba(255,255,255,0.07)",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>{prog.cover.priceFrom&&<div style={{fontFamily:"Outfit",fontSize:10,letterSpacing:4,color:"#666",textTransform:"uppercase",marginBottom:4}}>DESDE</div>}<div style={{display:"flex",alignItems:"flex-start",justifyContent:"center",gap:4}}><span style={{fontFamily:"Outfit",fontSize:22,color:PGOLD,fontWeight:300,marginTop:10}}>€</span><span style={{fontFamily:"Anton",fontSize:60,color:PGOLD,lineHeight:1,letterSpacing:-1}}>{prog.cover.price}</span></div><div style={{fontFamily:"Outfit",fontSize:11,color:"#888",letterSpacing:1,marginTop:4}}>{prog.cover.priceNote}</div><div style={{display:"inline-flex",alignItems:"center",gap:8,marginTop:12,padding:"8px 18px",borderRadius:22,background:"rgba(12,9,9,0.95)",border:"1px solid rgba(255,255,255,0.1)"}}><span style={{fontSize:14}}>🔒</span><span style={{fontFamily:"Outfit",fontSize:12,color:tc}}>Reserva con solo <strong style={{color:PGOLD}}>{prog.cta.deposit}</strong> {prog.cta.depositLabel}</span></div></div><div style={{display:"flex",flexDirection:"column",gap:7}}>{prog.cta.perks.map(p=>(<div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 13px",background:"rgba(255,255,255,0.04)",borderRadius:10,border:"1px solid rgba(255,255,255,0.07)"}}><span style={{color:PGREEN,fontSize:13,flexShrink:0}}>✓</span><span style={{fontFamily:"Outfit",fontSize:12,color:tc,lineHeight:1.4}}>{p.text}</span></div>))}</div><div style={{textAlign:"center",padding:"9px 16px",background:ac+"18",borderRadius:12,border:"1px solid "+ac+"40"}}><span style={{fontFamily:"Outfit",fontSize:11,color:ac}}>⏳ {prog.cta.deadline}</span></div><div style={{textAlign:"center"}}><div style={{display:"inline-block",padding:"16px 36px",borderRadius:30,background:"linear-gradient(135deg,"+PGOLD+",#b8891e)"}}><div style={{fontFamily:"Anton",fontSize:16,letterSpacing:3,color:PDARK}}>{prog.cta.ctaText}</div></div><div style={{marginTop:14,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}><div style={{fontFamily:"Outfit",fontSize:10,color:"#666",letterSpacing:1}}>{prog.cta.contactLabel.toUpperCase()}</div><div style={{fontFamily:"Anton",fontSize:15,color:tc,letterSpacing:1}}>{prog.cta.contact}</div></div></div><div style={{textAlign:"center"}}><div style={{fontFamily:"Outfit",fontSize:9,letterSpacing:3,color:PGOLD,opacity:0.45}}>WWW.TRAVELIKE.COM</div></div></div><PageNum num={totalPages}/></div>);
}
...
function ConstructorProgramas({onBack}){
  const[prog,setProg]=useState(()=>({
    cover:{title:"JAPÓN 2026",overtitle:"TRAVELIKE VIAJES",clientName:"",price:"2.990",priceFrom:true,priceNote:"por persona en hab. doble",accentColor:"#00f0ff",stats:[{id:"s1",value:"14",label:"DÍAS"},{id:"s2",value:"12",label:"CIUDADES"},{id:"s3",value:"30",label:"PAX"}],bg:{type:"photo",url:"",opacity:0.55,baseColor:"#07070f"}},
    mapEnabled:true,mapStops:[],
    days:[],includes:[{id:uid(),text:"Vuelos internacionales incluidos"},{id:uid(),text:"Alojamiento en hoteles seleccionados"},{id:uid(),text:"Guía español durante todo el viaje"},{id:uid(),text:"Seguro de viaje completo"}],excludes:[{id:uid(),text:"Comidas y cenas salvo indicación"},{id:uid(),text:"Gastos personales y propinas"}],
    cta:{headline:"RESERVA TU PLAZA",tagline:"Plazas limitadas",deposit:"500€",depositLabel:"de reserva",ctaText:"RESERVAR AHORA",contactLabel:"Contacta con nosotros",contact:"travelike.com",deadline:"Oferta válida hasta fin de mes",perks:[{id:uid(),text:"Grupos reducidos — máximo 20 personas"},{id:uid(),text:"Guía nativo español"},{id:uid(),text:"Cancelación gratuita hasta 30 días antes"}]},
    textColor:"#f0ede8",globalBg:{enabled:false,url:"",opacity:0.08,baseColor:"#07070f"}
  }));
  const[openSec,setOpenSec]=useState("cover");const[preview,setPreview]=useState(false);const[currentPage,setCurrentPage]=useState(0);const[gen,setGen]=useState(false);
  const updProg=fn=>setProg(p=>typeof fn==="function"?fn(p):{...p,...fn});
  const updCover=fn=>updProg(p=>({...p,cover:{...p.cover,...(typeof fn==="function"?fn(p.cover):fn)}}));
  const totalPages=1+(prog.mapEnabled?1:0)+prog.days.length+2;
  const addDay=()=>{const num=prog.days.length+1;updProg(p=>({...p,days:[...p.days,{id:puid(),num,title:`DÍA ${num}`,bg:{type:"color",color:"#07070f",url:"",opacity:0.6,baseColor:"#07070f"},headerColor:"#0f1824",blocks:[{id:puid(),type:"text-badge",text:"Descripción del día...",badge:"🌏",badgeRight:true}]}]}));};
  const delDay=id=>updProg(p=>({...p,days:p.days.filter(d=>d.id!==id)}));
  const updDay=(id,fn)=>updProg(p=>({...p,days:p.days.map(d=>d.id===id?{...d,...fn(d)}:d)}));
  const addBlock=(dayId,type)=>updDay(dayId,d=>({...d,blocks:[...d.blocks,{id:puid(),type,text:"",badge:"✨",badgeRight:true,photoUrl:"",photoLeft:true}]}));
  const updBlock=(dayId,blockId,fn)=>updDay(dayId,d=>({...d,blocks:d.blocks.map(b=>b.id===blockId?{...b,...fn(b)}:b)}));
  const delBlock=(dayId,blockId)=>updDay(dayId,d=>({...d,blocks:d.blocks.filter(b=>b.id!==blockId)}));
  const downloadPDF=async()=>{
    setGen(true);
    try{
      await pLoadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js");
      await pLoadScript("https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js");
      ...
      for(let i=0;i<totalPages;i++){
        ...
        const inner=document.createElement("div");inner.style.cssText=`width:${W}px;height:${H}px;position:relative;`;
        container.appendChild(inner);document.body.appendChild(container);
        const root=window.ReactDOM?.createRoot?window.ReactDOM.createRoot(inner):null;
        if(root)root.render(window.React.createElement(PTLPage,{prog,pageIndex:i,totalPages}));
        await new Promise(r=>setTimeout(r,800));
        const canvas=await window.html2canvas(container,{scale:2,useCORS:true,allowTaint:true,backgroundColor:null,logging:false});
        const img=canvas.toDataURL("image/jpeg",0.92);
        const pw=pdf.internal.pageSize.getWidth(),ph=pdf.internal.pageSize.getHeight();
        const ratio=Math.min(pw/canvas.width,ph/canvas.height);
        if(i>0)pdf.addPage();
        pdf.addImage(img,"JPEG",(pw-canvas.width*ratio)/2,0,canvas.width*ratio,canvas.height*ratio);
        document.body.removeChild(container);
      }
      pdf.save(`programa-${prog.cover.title.toLowerCase().replace(/\s+/g,"-")}.pdf`);
    }catch(e){alert("Error generando PDF: "+e.message);}
    setGen(false);
  };
  return(<div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF,paddingBottom:80}}>
    <div style={{background:A.card,padding:"14px 16px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${A.border}`,position:"sticky",top:0,zIndex:10}}>
      <button onClick={onBack} style={{background:A.card2,border:`1px solid ${A.border}`,color:A.text,borderRadius:8,width:36,height:36,cursor:"pointer",fontSize:16,flexShrink:0}}>←</button>
      <div style={{fontFamily:ANTON,fontSize:17,color:"#fff",letterSpacing:1,textTransform:"uppercase",flex:1}}>Constructor de Programas</div>
      <button onClick={()=>{setPreview(!preview);setCurrentPage(0);}} style={{background:preview?PGOLD+"33":A.card2,border:`1px solid ${preview?PGOLD+"66":A.border}`,borderRadius:8,padding:"6px 12px",color:preview?PGOLD:A.muted,fontSize:11,cursor:"pointer",fontFamily:BF,fontWeight:700}}>{preview?"✎ Editar":"👁 Preview"}</button>
    </div>
    {preview?(<div style={{padding:"14px 16px"}}>
      <div style={{position:"relative",width:"100%",aspectRatio:"390/700",background:"#06060f",borderRadius:16,overflow:"hidden",border:`1px solid ${A.border}`,marginBottom:12}}>
        <PTLPage prog={prog} pageIndex={currentPage} totalPages={totalPages}/>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <button onClick={()=>setCurrentPage(p=>Math.max(0,p-1))} disabled={currentPage===0} style={{flex:1,padding:"10px",background:A.card,border:`1px solid ${A.border}`,borderRadius:10,color:currentPage===0?A.muted:A.text,cursor:currentPage===0?"default":"pointer",fontFamily:BF,fontSize:14}}>← Anterior</button>
        <div style={{fontFamily:ANTON,fontSize:14,color:PGOLD,minWidth:60,textAlign:"center"}}>{currentPage+1} / {totalPages}</div>
        <button onClick={()=>setCurrentPage(p=>Math.min(totalPages-1,p+1))} disabled={currentPage===totalPages-1} style={{flex:1,padding:"10px",background:A.card,border:`1px solid ${A.border}`,borderRadius:10,color:currentPage===totalPages-1?A.muted:A.text,cursor:currentPage===totalPages-1?"default":"pointer",fontFamily:BF,fontSize:14}}>Siguiente →</button>
      </div>
      <button onClick={downloadPDF} disabled={gen} style={{width:"100%",padding:"16px",background:`linear-gradient(90deg,${PGOLD},#b8891e)`,border:"none",borderRadius:12,color:PDARK,fontFamily:ANTON,fontSize:16,letterSpacing:2,cursor:gen?"wait":"pointer"}}>
        {gen?"⏳ Generando PDF...":"⬇️ DESCARGAR PDF"}
      </button>
    </div>):(<div style={{padding:"14px 16px"}}>
      <PSectionCard title="🖼 PORTADA" open={openSec==="cover"} onToggle={()=>setOpenSec(s=>s==="cover"?null:"cover")} accent={prog.cover.accentColor}>
        <PField label="Título del programa" value={prog.cover.title} onChange={v=>updCover({title:v})} placeholder="JAPÓN 2026"/>
        <PField label="Subtítulo / Agencia" value={prog.cover.overtitle} onChange={v=>updCover({overtitle:v})} placeholder="TRAVELIKE VIAJES"/>
        <PField label="Nombre del cliente (opcional)" value={prog.cover.clientName} onChange={v=>updCover({clientName:v})} placeholder="Para: María García"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <PField label="Precio destacado" value={prog.cover.price} onChange={v=>updCover({price:v})} placeholder="2.990"/>
          <PField label="Nota del precio" value={prog.cover.priceNote} onChange={v=>updCover({priceNote:v})} placeholder="por persona"/>
        </div>
        <PColorPicker label="Color de acento" value={prog.cover.accentColor} onChange={v=>updCover({accentColor:v})}/>
        <PBgEditor bg={prog.cover.bg} onChange={bg=>updCover({bg})} label="Fondo de portada"/>
        <div style={{marginTop:8}}><PLabel>Estadísticas de portada</PLabel>{prog.cover.stats.map(s=>(<div key={s.id} style={{display:"flex",gap:8,marginBottom:8}}><input value={s.value} onChange={e=>updCover({stats:prog.cover.stats.map(x=>x.id===s.id?{...x,value:e.target.value}:x)})} style={{...pinp,width:80,flex:"none",textAlign:"center",fontFamily:"Anton,sans-serif",fontSize:18}} placeholder="14"/><input value={s.label} onChange={e=>updCover({stats:prog.cover.stats.map(x=>x.id===s.id?{...x,label:e.target.value}:x)})} style={{...pinp,flex:1}} placeholder="DÍAS"/><button onClick={()=>updCover({stats:prog.cover.stats.filter(x=>x.id!==s.id)})} style={{background:"none",border:"none",color:PRED,fontSize:18,cursor:"pointer"}}>✕</button></div>))}<button onClick={()=>updCover({stats:[...prog.cover.stats,{id:puid(),value:"",label:""}]})} style={{...paddBtn,width:"100%"}}>+ Estadística</button></div>
      </PSectionCard>
      <PSectionCard title="🗺 MAPA DE RUTA" open={openSec==="mapa"} onToggle={()=>setOpenSec(s=>s==="mapa"?null:"mapa")} accent={PGOLD}>
        <PToggle options={[["true","✅ Incluir mapa"],["false","❌ Sin mapa"]]} value={String(prog.mapEnabled)} onChange={v=>updProg({mapEnabled:v==="true"})}/>
        {prog.mapEnabled&&<PMapStopEditor stops={prog.mapStops} onChange={stops=>updProg({mapStops:stops})}/>}
      </PSectionCard>
      {prog.days.map((day,di)=>(<PSectionCard key={day.id} title={`📅 DÍA ${day.num}: ${day.title}`} open={openSec===day.id} onToggle={()=>setOpenSec(s=>s===day.id?null:day.id)} accent="#4FC3F7" actions={<PActBtn onClick={()=>{if(window.confirm("¿Eliminar este día?"))delDay(day.id);}} danger>🗑</PActBtn>}>
        <PField label="Título del día" value={day.title} onChange={v=>updDay(day.id,()=>({title:v}))} placeholder="Llegada y visita al templo"/>
        <PColorPicker label="Color de cabecera" value={day.headerColor} onChange={v=>updDay(day.id,()=>({headerColor:v}))}/>
        <PBgEditor bg={day.bg} onChange={bg=>updDay(day.id,()=>({bg}))} label="Fondo del día"/>
        <div style={{marginTop:8}}><PLabel>Bloques de contenido</PLabel>{day.blocks.map(block=>(<PBlockCard key={block.id} block={block} onChange={changes=>updBlock(day.id,block.id,()=>changes)} onRemove={()=>delBlock(day.id,block.id)}/>))}<div style={{display:"flex",gap:8,marginTop:8}}><button onClick={()=>addBlock(day.id,"text-badge")} style={{...paddBtn,flex:1,fontSize:11}}>+ Texto + Emoji</button><button onClick={()=>addBlock(day.id,"photo-text")} style={{...paddBtn,flex:1,fontSize:11}}>+ Foto + Texto</button></div></div>
      </PSectionCard>))}
      <button onClick={addDay} style={{...paddBtn,width:"100%",marginBottom:12}}>+ Añadir día</button>
      <PSectionCard title="✅ INCLUYE / NO INCLUYE" open={openSec==="includes"} onToggle={()=>setOpenSec(s=>s==="includes"?null:"includes")} accent={PGREEN}>
        <PLabel>Incluye</PLabel>{prog.includes.map(it=>(<PListItemRow key={it.id} value={it.text} onChange={v=>updProg(p=>({...p,includes:p.includes.map(x=>x.id===it.id?{...x,text:v}:x)}))} onRemove={()=>updProg(p=>({...p,includes:p.includes.filter(x=>x.id!==it.id)}))}/>))}<button onClick={()=>updProg(p=>({...p,includes:[...p.includes,{id:puid(),text:""}]}))} style={{...paddBtn,width:"100%",marginBottom:12}}>+ Añadir</button>
        <PLabel>No incluye</PLabel>{prog.excludes.map(it=>(<PListItemRow key={it.id} value={it.text} onChange={v=>updProg(p=>({...p,excludes:p.excludes.map(x=>x.id===it.id?{...x,text:v}:x)}))} onRemove={()=>updProg(p=>({...p,excludes:p.excludes.filter(x=>x.id!==it.id)}))}/>))}<button onClick={()=>updProg(p=>({...p,excludes:[...p.excludes,{id:puid(),text:""}]}))} style={{...paddBtn,width:"100%"}}>+ Añadir</button>
      </PSectionCard>
      <PSectionCard title="📣 PÁGINA CTA / RESERVA" open={openSec==="cta"} onToggle={()=>setOpenSec(s=>s==="cta"?null:"cta")} accent={PGOLD}>
        <PField label="Título principal" value={prog.cta.headline} onChange={v=>updProg(p=>({...p,cta:{...p.cta,headline:v}}))} placeholder="RESERVA TU PLAZA"/>
        <PField label="Tagline" value={prog.cta.tagline} onChange={v=>updProg(p=>({...p,cta:{...p.cta,tagline:v}}))} placeholder="Plazas limitadas"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          <PField label="Importe reserva" value={prog.cta.deposit} onChange={v=>updProg(p=>({...p,cta:{...p.cta,deposit:v}}))} placeholder="500€"/>
          <PField label="Label importe" value={prog.cta.depositLabel} onChange={v=>updProg(p=>({...p,cta:{...p.cta,depositLabel:v}}))} placeholder="de reserva"/>
        </div>
        <PField label="Texto del botón CTA" value={prog.cta.ctaText} onChange={v=>updProg(p=>({...p,cta:{...p.cta,ctaText:v}}))} placeholder="RESERVAR AHORA"/>
        <PField label="Contacto visible" value={prog.cta.contact} onChange={v=>updProg(p=>({...p,cta:{...p.cta,contact:v}}))} placeholder="travelike.com"/>
        <PField label="Plazo / urgencia" value={prog.cta.deadline} onChange={v=>updProg(p=>({...p,cta:{...p.cta,deadline:v}}))} placeholder="Oferta válida hasta fin de mes"/>
        <PLabel>Perks (puntos de venta)</PLabel>{prog.cta.perks.map(p=>(<PListItemRow key={p.id} value={p.text} onChange={v=>updProg(pr=>({...pr,cta:{...pr.cta,perks:pr.cta.perks.map(x=>x.id===p.id?{...x,text:v}:x)}}))} onRemove={()=>updProg(pr=>({...pr,cta:{...pr.cta,perks:pr.cta.perks.filter(x=>x.id!==p.id)}}))}/>))}<button onClick={()=>updProg(p=>({...p,cta:{...p.cta,perks:[...p.cta.perks,{id:puid(),text:""}]}}))} style={{...paddBtn,width:"100%"}}>+ Añadir perk</button>
      </PSectionCard>
      <PSectionCard title="🌐 FONDO GLOBAL" open={openSec==="globalBg"} onToggle={()=>setOpenSec(s=>s==="globalBg"?null:"globalBg")} accent={A.purple}>
        <PLabel>Aplica una textura/foto de fondo a TODAS las páginas</PLabel>
        <PToggle options={[["true","✅ Activado"],["false","❌ Desactivado"]]} value={String(!!(prog.globalBg||{}).enabled)} onChange={v=>updProg(p=>({...p,globalBg:{...(p.globalBg||{enabled:false,url:"",opacity:0.08,baseColor:"#07070f"}),enabled:v==="true"}}))}/>
        {(prog.globalBg||{}).enabled&&<PBgEditor bg={prog.globalBg} onChange={bg=>updProg({globalBg:bg})} label="Fondo global"/>}
      </PSectionCard>
    </div>)}
  </div>);
}
...
export default function App(){
  ...
  const[trips,setTrips]=useState([]);const[clients,setClients]=useState([]);const[crm,setCrm]=useState({});
  const[cfg,setCfg]=useState({quickTripId:null,quickBtnLabel:"💰 Ver gastos",quickBtnColor:A.orange,quickTripId2:null,quickBtnLabel2:"📊 Ver pagos",quickBtnColor2:A.cyan});
  ...
  useEffect(()=>{(async()=>{
    ...
    const rp=await db.get(SK_RP);const gg=await db.get(SK_GG);
    if(!t){t=DT;await db.set(SK_T,t);}if(!c){c=DC;await db.set(SK_C,c);}
    if(!cr){cr={};await db.set(SK_CRM,cr);}if(!cf){cf={quickTripId:null,quickBtnLabel:"💰 Ver gastos",quickBtnColor:A.orange,quickTripId2:null,quickBtnLabel2:"📊 Ver pagos",quickBtnColor2:A.cyan};await db.set(SK_CFG,cf);}
    ...
    setTrips(tt);setClients(cc);setCrm(cr);setCfg(cf);if(rp)setReservas(rp);if(gg){const ggNorm=(Array.isArray(gg)||!gg||typeof gg!=="object")?{gastos:[],facturas:[]}:{gastos:gg.gastos||[],facturas:gg.facturas||[]};setGastosGen(ggNorm);}
    let admValid=false;
  ...
  const sT=async v=>{setTrips(v);await db.set(SK_T,v);};const sC=v=>{setClients(v);db.set(SK_C,v).catch(()=>{});};
  const sCrm=async v=>{setCrm(v);await db.set(SK_CRM,v);};const sCfg=async v=>{setCfg(v);await db.set(SK_CFG,v);};
  const sReservas=async v=>{setReservas(v);await db.set(SK_RP,v);};const sGastosGen=async v=>{setGastosGen(v);await db.set(SK_GG,v);};
...
function AHome({go,trips,clients,sT,sC,cfg,sCfg,logoutAdmin,reservas,sReservas,gastosGen,sGastosGen}){
  const[subScreen,setSubScreen]=useState(null);const[cfgModal1,setCfgModal1]=useState(false);const[cfgModal2,setCfgModal2]=useState(false);const[addTripModal,setAddTripModal]=useState(false);const[passModal,setPassModal]=useState(false);
  ...
  const[tripPrice,setTripPrice]=useState("");const[tripFechas,setTripFechas]=useState("");const[tripWebUrl,setTripWebUrl]=useState("");const[tripCurrency,setTripCurrency]=useState(trips.length>0?(trips[trips.length-1].currency||"EUR"):"EUR");const[currMenu,setCurrMenu]=useState(false);
  ...
  const quickTrip2=cfg.quickTripId2?trips.find(t=>t.id===cfg.quickTripId2):null;
  ...
  return(<div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF}}>
    ...
    <div style={{padding:"10px 16px 0"}}>
      {quickTrip?(<button onClick={()=>go("atrip",{tid:quickTrip.id,initTab:"finanzas"})} style={{width:"100%",background:`${cfg.quickBtnColor}22`,border:`2px solid ${cfg.quickBtnColor}55`,borderRadius:14,padding:"11px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,boxSizing:"border-box"}}><span style={{fontSize:24}}>{quickTrip.flag}</span><div style={{flex:1,textAlign:"left"}}><div style={{fontFamily:ANTON,fontSize:14,color:cfg.quickBtnColor,letterSpacing:1,textTransform:"uppercase"}}>{cfg.quickBtnLabel}</div><div style={{fontFamily:BF,fontSize:10,color:A.muted,marginTop:1}}>{quickTrip.name}</div></div><div onClick={e=>{e.stopPropagation();setCfgModal1(true);}} style={{color:A.muted,fontSize:16,cursor:"pointer",padding:"4px 6px"}}>⚙️</div></button>):(<button onClick={()=>setCfgModal1(true)} style={{width:"100%",background:A.card2,border:`1.5px dashed ${A.border}`,borderRadius:14,padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,color:A.muted,fontFamily:BF,fontSize:12,boxSizing:"border-box"}}><span style={{fontSize:18}}>⚡</span>Añadir botón de acceso rápido</button>)}
      <div style={{marginTop:8}}>
      {quickTrip2?(<button onClick={()=>go("atrip",{tid:quickTrip2.id,initTab:"finanzas"})} style={{width:"100%",background:`${cfg.quickBtnColor2||A.cyan}22`,border:`2px solid ${cfg.quickBtnColor2||A.cyan}55`,borderRadius:14,padding:"11px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,boxSizing:"border-box"}}>
        <span style={{fontSize:24}}>{quickTrip2.flag}</span>
        <div style={{flex:1,textAlign:"left"}}><div style={{fontFamily:ANTON,fontSize:14,color:cfg.quickBtnColor2||A.cyan,letterSpacing:1,textTransform:"uppercase"}}>{cfg.quickBtnLabel2||"Ver pagos"}</div><div style={{fontFamily:BF,fontSize:10,color:A.muted,marginTop:1}}>{quickTrip2.name}</div></div>
        <div onClick={e=>{e.stopPropagation();setCfgModal2(true);}} style={{color:A.muted,fontSize:16,cursor:"pointer",padding:"4px 6px"}}>⚙️</div>
      </button>):(<button onClick={()=>setCfgModal2(true)} style={{width:"100%",background:A.card2,border:`1.5px dashed ${A.border}`,borderRadius:14,padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,color:A.muted,fontFamily:BF,fontSize:12,boxSizing:"border-box"}}><span style={{fontSize:18}}>⚡</span>Segundo botón rápido</button>)}
      </div>
    </div>
    <div style={{padding:"12px 16px"}}>
    ...
    {cfgModal1&&<QuickCfgModal which={1} cfg={cfg} trips={trips} sCfg={sCfg} onClose={()=>setCfgModal1(false)}/>}
    {cfgModal2&&<QuickCfgModal which={2} cfg={cfg} trips={trips} sCfg={sCfg} onClose={()=>setCfgModal2(false)}/>}
    {passModal&&<FilteredListModal title="Alertas de pasaporte" clients={passWarnList} onClose={()=>setPassModal(false)}/>}
  </div>);}

function QuickCfgModal({which,cfg,trips,sCfg,onClose}){
  const isFirst=which===1;
  const[label,setLabel]=useState(isFirst?(cfg.quickBtnLabel||"💰 Ver gastos"):(cfg.quickBtnLabel2||"📊 Ver pagos"));
  const[color,setColor]=useState(isFirst?(cfg.quickBtnColor||A.orange):(cfg.quickBtnColor2||A.cyan));
  const[tripId,setTripId]=useState(isFirst?(cfg.quickTripId||""):(cfg.quickTripId2||""));
  const save=()=>{
    if(isFirst) sCfg({...cfg,quickTripId:tripId||null,quickBtnLabel:label,quickBtnColor:color});
    else sCfg({...cfg,quickTripId2:tripId||null,quickBtnLabel2:label,quickBtnColor2:color});
    onClose();
  };
  const COLORS=[A.orange,A.cyan,A.gold,A.green,A.red,A.purple,"#FF6B6B","#00CED1"];
  return(<AModal title={`Botón rápido ${which}`} onClose={onClose}>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>{COLORS.map(c=><button key={c} onClick={()=>setColor(c)} style={{width:36,height:36,borderRadius:10,background:c,border:`3px solid ${color===c?"#fff":c}`,cursor:"pointer",flexShrink:0}}/>)}</div>
    <ARow><button onClick={onClose} style={{...ab(A.card2,A.muted),flex:1,border:`1px solid ${A.border}`}}>Cancelar</button><button onClick={save} style={{...ab(A.cyan,A.bg),flex:2}}>Guardar</button></ARow>
  </AModal>);
}
// ══════════════════════════════════════════════════════════
// CLIENTES TAB, NOTIF ADMIN (unchanged from original)
// ══════════════════════════════════════════════════════════
// ── ClientLinkBtns: extracted to comply with Rules of Hooks ──
// -- LinkWABtns --
function LinkWABtns({c,trips}){
  const[cop,setCop]=React.useState(false);
  const link=`${window.location.href.split("#")[0]}#${c.code}`;
  const trip=trips.find(t=>t.id===c.tripId);
  const wa=()=>{ const msg=trip?`Hola ${c.nombre.split(" ")[0]}!\n\nPortal ${trip.name}:\n${link}`:`Hola ${c.nombre.split(" ")[0]}!\n\nPortal Travelike:\n${link}`; window.open(`https://wa.me/${WA_NUM}?text=${encodeURIComponent(msg)}`,"_blank"); };
  return(<><button onClick={()=>{navigator.clipboard.writeText(link).then(()=>{setCop(true);setTimeout(()=>setCop(false),2000);});}} style={{...ab(cop?A.green+"22":A.cyan+"15",cop?A.green:A.cyan),flex:1,border:`1px solid ${cop?A.green:A.cyan}44`,padding:"6px 8px",fontSize:11,borderRadius:8,fontFamily:BF}}>{cop?"Copiado!":"Link"}</button><button onClick={wa} style={{...ab(A.green+"15",A.green),flex:1,border:`1px solid ${A.green}44`,padding:"6px 8px",fontSize:11,borderRadius:8,fontFamily:BF}}>WhatsApp</button></>);
}

function ClientLinkBtns({c,trips,expandedDoc,setExpandedDoc}){
  const[cop,setCop]=useState(false);
  const link=`${window.location.href.split("#")[0]}#${c.code}`;
  const wa=()=>{const t2=trips.find(t=>t.id===c.tripId);const msg=t2?`Hola ${c.nombre.split(" ")[0]}!\n\nPortal ${t2.name}:\n${link}`:`Hola ${c.nombre.split(" ")[0]}!\n\nPortal Travelike:\n${link}`;window.open(`https://wa.me/${WA_NUM}?text=${encodeURIComponent(msg)}`,"_blank");};
  return(<>
    <button onClick={()=>{navigator.clipboard.writeText(link).then(()=>{setCop(true);setTimeout(()=>setCop(false),2000);});}} style={{...ab(cop?A.green+"22":A.cyan+"15",cop?A.green:A.cyan),flex:1,fontSize:11,padding:"7px 8px",borderRadius:8,border:`1px solid ${cop?A.green:A.cyan}44`,fontFamily:BF}}>{cop?"¡Copiado!":"🔗 Link"}</button>
    <button onClick={wa} style={{...ab(A.green+"15",A.green),flex:1,fontSize:11,padding:"7px 8px",borderRadius:8,border:`1px solid ${A.green}44`,fontFamily:BF}}>💬 WhatsApp</button>
    <button onClick={()=>setExpandedDoc(expandedDoc===c.id?null:c.id)} style={{...ab(expandedDoc===c.id?A.cyan+"22":A.card2,expandedDoc===c.id?A.cyan:A.muted),flex:1,fontSize:11,padding:"7px 8px",borderRadius:8,border:`1px solid ${expandedDoc===c.id?A.cyan+"44":A.border}`,fontFamily:BF}}>🛂 Doc</button>
  </>);
}
    ...
    {displayed.map(c=>{
      ...
      return(<div key={c.id} style={{background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`1px solid ${A.border}44`}}>
        ...
        <div style={{display:"flex",gap:6,marginBottom:6}}>
          <ClientLinkBtns c={c} trips={trips} expandedDoc={expandedDoc} setExpandedDoc={setExpandedDoc}/>
        </div>
...
// ══════════════════════════════════════════════════════════
function PeopleTab({trip,tc,clients,trips,sC,updTrip,updClient,passRef,upIdx,setUpIdx,passModal,setPassModal,chPassRef,addPass,hasUrgent,stFilterModal,setStFilterModal,rmModal,setRmModal,roomMenu,setRoomMenu,acmpModal,setAcmpModal,fpMenu,setFpMenu,setRoommate,setTab,viewPhoto,setViewPhoto,go}){
  ...
  return(
    <div style={{ padding:"0 16px" }}>
      <button onClick={()=>go("ahome")} style={{ background:"transparent",border:"none",color:A.cyan,fontSize:14,fontFamily:BF,cursor:"pointer",marginBottom:12 }}>← Volver al menú</button>
      ...
      <div style={{ display:"flex",gap:8,marginBottom:12 }}>
        <button onClick={()=>setAddModal("existing")} style={{ ...ab(A.purple+"22",A.purple),flex:3,border:`1.5px solid ${A.purple}44`,borderRadius:10,padding:"12px",fontSize:14,fontFamily:BF,fontWeight:700 }}>Añadir viajero existente ({allOtherClients.length})</button>
        {(confirmed.length>0||interesados.length>0)&&<button onClick={()=>setListModal(true)} style={{ ...ab(A.green+"22",A.green),flex:1,border:`1.5px solid ${A.green}44`,borderRadius:10,padding:"10px",fontSize:11,fontFamily:BF,fontWeight:700 }}>LISTAS</button>}
      </div>
        ...
        return(
          <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":c.groupId?A.purple+"44":A.border+"44"}` }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
              ...
              <div style={{ flex:1,minWidth:0 }}><div style={{ fontFamily:ANTON,fontSize:16,color:A.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{c.nombre}{c.groupId&&<span style={{ flexShrink:0,fontSize:10,background:A.purple+"22",color:A.purple,border:`1px solid ${A.purple}44`,borderRadius:6,padding:"1px 6px",fontFamily:BF,fontWeight:700 }}> Grupo {tc.filter(x=>x.groupId===c.groupId).length}</span>}</div><div style={{ fontSize:10,color:st.color,fontFamily:BF }}>{st.emoji} {st.label}</div></div>
      ...
      <input ref={passRef} type="file" accept="image/jpeg,image/png,image/webp" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]&&upIdx) addPass(e.target.files[0],upIdx); setUpIdx(null); e.target.value=""; }}/>
      ...
      {addModal==="existing"&&(()=>{
        const toggleGroup=(c)=>{ const gm=c.groupId?allOtherClients.filter(x=>x.groupId===c.groupId).map(x=>x.id):[c.id]; const allS=gm.every(id=>exSelected.includes(id)); setExSelected(prev=>allS?prev.filter(id=>!gm.includes(id)):[...new Set([...prev,...gm])]); };
        const sorted=[...filteredEx].sort((a,b)=>{ if(a.groupId&&b.groupId)return a.groupId.localeCompare(b.groupId); if(a.groupId)return -1; if(b.groupId)return 1; return 0; });
        const seenGroups=new Set();
        return(<AModal title="Añadir viajeros existentes" onClose={()=>{setAddModal(null);setExSearch("");setExSelected([]);}}>
          <div style={{fontFamily:BF,fontSize:12,color:A.muted,marginBottom:10}}>Los grupos se seleccionan juntos.</div>
          <SearchBar value={exSearch} onChange={setExSearch} placeholder="Buscar..."/>
          {filteredEx.length===0?<AEmpty text="Sin clientes disponibles"/>:sorted.map(c=>{
            const sel=exSelected.includes(c.id); const cTrip=c.tripId?trips.find(t=>t.id===c.tripId):null;
            const isGroupLeader=c.groupId&&!seenGroups.has(c.groupId); if(c.groupId) seenGroups.add(c.groupId);
            const groupSize=c.groupId?allOtherClients.filter(x=>x.groupId===c.groupId).length:1;
            return(<div key={c.id}>
              {isGroupLeader&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 2px 6px",marginTop:6}}><div style={{flex:1,height:1,background:A.purple+"44"}}/><span style={{fontFamily:BF,fontSize:10,color:A.purple,fontWeight:700}}>GRUPO · {groupSize} personas</span><div style={{flex:1,height:1,background:A.purple+"44"}}/></div>}
              <button onClick={()=>toggleGroup(c)} style={{...ab(sel?A.cyan+"22":c.groupId?A.purple+"11":A.card2,A.text),width:"100%",marginBottom:6,textAlign:"left",border:`2px solid ${sel?A.cyan:c.groupId?A.purple+"55":A.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:36,height:36,borderRadius:8,background:sel?A.cyan+"33":A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:15,color:A.cyan,flexShrink:0,overflow:"hidden"}}>{c.passportPhoto?<img src={c.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:6}} alt=""/>:c.nombre[0]}</div>
                <div style={{flex:1}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:sel?A.cyan:A.text}}>{c.nombre}</div><div style={{fontSize:10,color:cTrip?A.orange:A.muted,fontFamily:BF}}>{cTrip?`${cTrip.flag} ${cTrip.name}`:"Sin viaje"} · {c.code}</div></div>
                {c.groupId&&isGroupLeader&&!sel&&<span style={{fontSize:9,background:A.purple+"22",color:A.purple,border:`1px solid ${A.purple}44`,borderRadius:6,padding:"2px 6px",fontFamily:BF,fontWeight:700,flexShrink:0}}>+{groupSize}</span>}
          ...
          {exSelected.length>0&&<div style={{position:"sticky",bottom:0,background:A.card2,borderRadius:12,padding:"12px",marginTop:8,border:`1px solid ${A.cyan}44`}}>
            <div style={{fontFamily:BF,fontSize:11,color:A.muted,marginBottom:6,textAlign:"center"}}>{exSelected.length} viajero{exSelected.length!==1?"s":""} seleccionado{exSelected.length!==1?"s":""}</div>
            <button onClick={()=>{sC(clients.map(x=>exSelected.includes(x.id)?{...x,tripId:trip.id,pagosEstado:(trip.pagosConfig||[]).map(()=>"pendiente"),pagosImporteCustom:(trip.pagosConfig||[]).map(()=>null)}:x));setAddModal(null);setExSearch("");setExSelected([]);}} style={{...ab(`linear-gradient(90deg,${A.cyan},#0099bb)`,A.bg),width:"100%",borderRadius:10,fontFamily:BF,fontSize:14}}>✓ Añadir {exSelected.length} al viaje</button>
          </div>}
        </AModal>);
      })()}
    </div>
...
function PagosTab({trip,tc,clients,updTrip,updClient,setRegistroPagoModal,setTab,go}){
  ...
  const savePc=(impOverride)=>{if(editPcModal===null)return;const imp=impOverride!==undefined?impOverride:pcImporte;updTrip(t=>({...t,pagosConfig:t.pagosConfig.map((x,j)=>j===editPcModal?{...x,label:pcLabel,importe:imp}:x)}));setEditPcModal(null);};
  return(
    <div style={{ padding:"0 16px" }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
        <button onClick={()=>go("ahome")} style={{ background:"transparent",border:"none",color:A.cyan,fontSize:14,fontFamily:BF,cursor:"pointer" }}>← Volver al menú</button>
        <div style={{ display:"flex",gap:6 }}>
      ...
      {editPcModal!==null&&(<AModal title="Editar hito de pago" onClose={()=>setEditPcModal(null)}>
        ...
        <div style={{ background:A.card,border:`1px solid ${A.border}`,borderRadius:10,padding:"14px",fontSize:26,fontFamily:ANTON,color:A.cyan,textAlign:"right",marginBottom:10 }}>{pcImporte.replace(/[^\d.]/g,"")||"0"} €</div>
        <AmountPad value={pcImporte.replace(/[^\d.]/g,"")} onChange={v=>setPcImporte(v)}/>
        <ARow><button onClick={()=>{if((trip.pagosConfig||[]).length>1)updTrip(t=>({...t,pagosConfig:t.pagosConfig.filter((_,j)=>j!==editPcModal)}));setEditPcModal(null);}} style={{ ...ab(A.red+"15",A.red),flex:1,border:`1px solid ${A.red}33` }}>Borrar</button><button onClick={()=>setEditPcModal(null)} style={{ ...ab(A.card2,A.muted),flex:1,border:`1px solid ${A.border}` }}>Cancelar</button><button onClick={()=>{const v=pcImporte.replace(/[^\d.]/g,"");savePc(v?v+"€":"");}} style={{ ...ab(A.cyan,A.bg),flex:2 }}>Guardar</button></ARow>
      </AModal>)}
    </div>
  );
}
...
function FinanzasTab({trip,tc,clients,trips,updTrip,setTab,go}){
  ...
  const[gCurrency,setGCurrency]=useState(()=>{try{return localStorage.getItem("tv9-last-currency")||trip.currency||"EUR";}catch{return trip.currency||"EUR";}});const[gFecha,setGFecha]=useState(new Date().toISOString().split("T")[0]);
  const[gNota,setGNota]=useState("");const[gImporteEUR,setGImporteEUR]=useState("");const[converting,setConverting]=useState(false);
  const[currMenu,setCurrMenu]=useState(false);const[showStats,setShowStats]=useState(false);
  ...
  const setGCurrencyAndSave=(c)=>{setGCurrency(c);try{localStorage.setItem("tv9-last-currency",c);}catch{}};
  ...
  return(
    <div style={{ padding:"0 16px" }}>
      ...
      {tipoMenu&&<GastoTipoMenu current={gTipo} onSelect={t=>setGTipo(t)} onClose={()=>setTipoMenu(false)}/>}
      {currMenu&&<CurrencyMenu current={gCurrency} onSelect={c=>setGCurrencyAndSave(c)} onClose={()=>setCurrMenu(false)}/>}
...
function AIDocsTab({trip,tc,clients,updTrip,updClient,setTab,go}){
...
function EditTab({trip,trips,clients,updTrip,updClient,sC,setTab,editSec,setEditSec,go}){
  ...
  return(<div style={{ padding:"0 16px" }}>
    ...
    {sec==="pagos"&&<div>
      <div style={{ background:A.card2,borderRadius:12,padding:"14px",marginBottom:14,border:`2px solid ${A.cyan}33` }}>
        <div style={{ fontFamily:BF,fontSize:10,color:A.cyan,letterSpacing:2,textTransform:"uppercase",marginBottom:10 }}>🏦 Cuenta bancaria para pagos</div>
        <div style={{ fontFamily:BF,fontSize:12,color:A.muted,marginBottom:10 }}>Los clientes verán esta cuenta en su portal para realizar las transferencias.</div>
        {BANK_ACCOUNTS.map(ba=>(<button key={ba.id} onClick={()=>updTrip(t=>({...t,tripBankId:ba.id}))} style={{ width:"100%",background:(trip.tripBankId||"principal")===ba.id?A.cyan+"22":A.card,border:`2px solid ${(trip.tripBankId||"principal")===ba.id?A.cyan:A.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:12 }}><div style={{ flex:1 }}><div style={{ fontFamily:BF,fontSize:13,fontWeight:700,color:(trip.tripBankId||"principal")===ba.id?A.cyan:"#fff" }}>{ba.label}</div><div style={{ fontFamily:"monospace",fontSize:12,color:A.muted,marginTop:2 }}>{ba.iban}</div></div>{(trip.tripBankId||"principal")===ba.id&&<span style={{ color:A.cyan,fontSize:18 }}>✓</span>}</button>))}
      </div>
  ...
  return(
    <div style={{ background:A.bg,minHeight:"100vh",color:A.text,paddingBottom:24,fontFamily:BF }}>
      <div style={{ background:A.card,padding:"14px 16px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${A.border}`,position:"sticky",top:0,zIndex:10 }}>
        ...
        <button onClick={()=>tab==="menu"?go("ahome"):setTab("menu")} style={{ background:A.card2,border:`1px solid ${A.border}`,color:A.text,borderRadius:8,width:36,height:36,cursor:"pointer",fontSize:16,flexShrink:0 }}>←</button>
      </div>
      <div style={{ minHeight:"calc(100vh - 160px)" }}>
        ...
        {tab==="pagos"&&<PagosTab trip={trip} tc={tc} clients={clients} updTrip={updTrip} updClient={updClient} setRegistroPagoModal={setRegistroPagoModal} setTab={setTab} go={go}/>}
        {tab==="finanzas"&&<FinanzasTab trip={trip} tc={tc} clients={clients} trips={trips} updTrip={updTrip} setTab={setTab} go={go}/>}
        {tab==="ai"&&<AIDocsTab trip={trip} tc={tc} clients={clients} updTrip={updTrip} updClient={updClient} setTab={setTab} go={go}/>}
        {tab==="notes"&&<div style={{ padding:"0 16px" }}><button onClick={()=>go("ahome")} style={{ background:"transparent",border:"none",color:A.cyan,fontSize:14,fontFamily:BF,cursor:"pointer",marginBottom:12,marginTop:12 }}>← Volver al menú</button><textarea value={notes} onChange={e=>setNotes(e.target.value)} onBlur={()=>sCrm({...crm,[tid]:{...(crm[tid]||{}),notes}})} placeholder="Escribe tus anotaciones..." style={{ ...ais,minHeight:280,resize:"vertical",lineHeight:1.7 }}/><div style={{ fontSize:10,color:A.muted,marginTop:6,textAlign:"right",fontFamily:BF }}>Se guarda al salir del campo</div></div>}
        {tab==="edit"&&<EditTab trip={trip} trips={trips} clients={clients} updTrip={updTrip} updClient={updClient} sC={sC} setTab={setTab} editSec={editSec} setEditSec={setEditSec} go={go}/>}
      </div>
  ...
  return(<div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF }}>
    ...
    <div style={{ padding:"28px 22px" }}>
      ...
      {step===1&&(<div>
        ...
        <div style={{ fontSize:14,color:A.muted,lineHeight:1.6,marginBottom:8 }}>Sube una foto clara de la página de datos de tu pasaporte o DNI. <strong style={{color:A.red}}>Es obligatorio para continuar.</strong></div>
        {!photo&&<div style={{ background:A.red+"18",border:`1px solid ${A.red}44`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontFamily:BF,fontSize:13,color:A.red }}>⚠️ Debes subir la foto del pasaporte para poder continuar</div>}
        ...
        <ARow><button onClick={()=>setStep(0)} style={{ ...ab(A.card2,A.muted),flex:1,border:`1px solid ${A.border}`,borderRadius:10,fontFamily:BF }}>Atrás</button><button onClick={()=>photo&&setStep(2)} disabled={!photo} style={{ ...ab(photo?`linear-gradient(90deg,${A.cyan},#0099bb)`:A.card2,photo?A.bg:A.muted),flex:2,borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:1,border:"none",opacity:photo?1:0.5,cursor:photo?"pointer":"not-allowed" }}>Continuar →</button></ARow>
      </div>)}
      {step===2&&(<div>
...
function Client({go,cid,clients,trips,logout,sC,sT}){
  ...
  if(!cl)return<NoTrips cl={cl} logout={logout}/>;
  ...
  const groupMembers=cl.groupId?clients.filter(c=>c.groupId===cl.groupId):[cl];
    ...
    return(
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",fontFamily:BF,color:A.text,paddingBottom:32}}>
        ...
        <div style={{padding:"20px 16px"}}>
          <div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:4,textTransform:"uppercase",marginBottom:14,textAlign:"center"}}>¿QUÉ QUIERES CONSULTAR?</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            {[
              ...
              {emoji:"💳",label:"Pagos",sublabel:pagosPendientes>0?`${pagosPendientes} pendiente${pagosPendientes>1?"s":""}`:pagosPendientes===0?"Estado de tus pagos":"Todo pagado",color:pagosPendientes>0?A.orange:A.green,badge:pagosPendientes>0?pagosPendientes:null,v:"pagos"},
              {emoji:"ℹ️",label:"Información",sublabel:"Consejos del destino",color:A.purple,badge:infoCount,v:"info"},
            ].map(item=>(
              <div key={item.v} onClick={()=>setView(item.v)} style={{background:A.card,borderRadius:20,padding:"20px 16px",border:`1.5px solid ${item.color}33`,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:10,position:"relative",overflow:"hidden"}}>
                ...
                <div>
                  <div style={{fontFamily:ANTON,fontSize:20,color:item.color,letterSpacing:1,textTransform:"uppercase",lineHeight:1,marginBottom:3}}>{item.label}</div>
                  <div style={{fontFamily:BF,fontSize:12,color:A.muted}}>{item.sublabel}</div>
                </div>
              </div>
            ))}
          </div>
          <div onClick={()=>setView("maleta")} style={{background:A.card,borderRadius:20,padding:"16px 20px",border:`1.5px solid ${A.cyan}33`,cursor:"pointer",display:"flex",alignItems:"center",gap:14,marginBottom:12,position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:-24,right:-24,width:90,height:90,borderRadius:"50%",background:A.cyan+"10",pointerEvents:"none"}}/>
            <div style={{fontSize:40,lineHeight:1}}>🎒</div>
            <div style={{flex:1}}>
              <div style={{fontFamily:ANTON,fontSize:20,color:A.cyan,letterSpacing:1,textTransform:"uppercase",lineHeight:1,marginBottom:3}}>Mi maleta</div>
              <div style={{fontFamily:BF,fontSize:12,color:A.muted}}>Lista de equipaje</div>
            </div>
            <div style={{color:A.cyan,fontSize:20}}>›</div>
          </div>
          {groupMembers.length>1&&(
            <div onClick={()=>setView("pasaportes_grupo")} style={{background:A.card,borderRadius:20,padding:"16px",border:`1.5px solid ${A.purple}44`,cursor:"pointer",marginBottom:12,position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,borderRadius:"50%",background:A.purple+"10",pointerEvents:"none"}}/>
              <div style={{fontFamily:BF,fontSize:10,color:A.purple,letterSpacing:3,textTransform:"uppercase",marginBottom:10,fontWeight:700}}>👥 Mi grupo</div>
              <div style={{display:"flex",gap:10}}>
                {groupMembers.map(m=>(
                  <div key={m.id} style={{flex:1,background:A.bg,borderRadius:14,padding:"12px 10px",border:`2px solid ${m.passportPhoto?A.cyan+"66":A.border}`,textAlign:"center"}}>
                    <div style={{width:56,height:70,borderRadius:10,overflow:"hidden",margin:"0 auto 8px",border:`2px solid ${m.passportPhoto?A.cyan:A.border}`,background:A.card2,display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {m.passportPhoto?<img src={m.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<span style={{fontSize:28}}>🛂</span>}
                    </div>
                    <div style={{fontFamily:ANTON,fontSize:12,color:m.id===cid?A.cyan:A.text,lineHeight:1.2,marginBottom:3}}>{m.nombre.split(" ")[0].toUpperCase()}</div>
                    <div style={{fontFamily:BF,fontSize:9,color:m.passportPhoto?A.green:A.orange}}>{m.passportPhoto?"✓ Subido":"⚠️ Pendiente"}</div>
                  </div>
                ))}
              </div>
              <div style={{marginTop:10,fontFamily:BF,fontSize:11,color:groupMembers.every(m=>m.passportPhoto)?A.green:A.orange,textAlign:"center"}}>
                {groupMembers.filter(m=>m.passportPhoto).length}/{groupMembers.length} con datos completos
                {!groupMembers.every(m=>m.passportPhoto)&&<span style={{color:A.orange}}> — ⚠️ Faltan datos</span>}
              </div>
            </div>
          )}
          {trip.webUrl?<a href={trip.webUrl} target="_blank" rel="noreferrer" style={{display:"block",background:A.card,borderRadius:18,padding:"14px 18px",border:`1px solid ${A.border}`,display:"flex",alignItems:"center",gap:12,textDecoration:"none",marginBottom:12}}><span style={{fontSize:28}}>🌐</span><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:13,color:A.muted}}>Web del viaje</div></div><div style={{color:A.cyan,fontSize:18}}>›</div></a>:<div style={{background:A.card,borderRadius:18,padding:"14px 18px",border:`1px solid ${A.border}44`,display:"flex",alignItems:"center",gap:12,opacity:0.5,marginBottom:12}}><span style={{fontSize:28}}>🌐</span><div style={{fontFamily:BF,fontSize:13,color:A.muted}}>Web del viaje — próximamente</div></div>}
        </div>
  ...
  if(view==="pasaportes_grupo"){
    const groupPhotoRefs=useRef({});
    const uploadForMember=(memberId,file)=>{
      const r=new FileReader(); r.onload=ev=>{
        const img=new Image(); img.onload=()=>{
          ...
          sC(clients.map(x=>x.id===memberId?{...x,passportPhoto:canvas.toDataURL("image/jpeg",0.78)}:x));
        };
        img.onerror=()=>sC(clients.map(x=>x.id===memberId?{...x,passportPhoto:ev.target.result}:x));
        img.src=ev.target.result;
      }; r.readAsDataURL(file);
    };
    return(
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,paddingBottom:40,fontFamily:BF}}>
        <SubHeader title="Mi Grupo" color={A.purple}/>
        <div style={{padding:"16px"}}>
          <div style={{fontFamily:BF,fontSize:12,color:A.muted,marginBottom:16,lineHeight:1.6}}>Aquí podéis subir y actualizar los pasaportes de todo el grupo. Cualquiera puede subir para otro.</div>
          {groupMembers.map(m=>{
            const isMe=m.id===cid;
            const exp=m.passportExpiry?new Date(m.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"}):null;
            const warn=passportWarn(m.passportExpiry);
            return(
              <div key={m.id} style={{background:A.card,borderRadius:20,padding:"16px",marginBottom:14,border:`2px solid ${isMe?A.cyan+"66":A.purple+"44"}`}}>
                <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                  <div onClick={()=>{groupPhotoRefs.current[m.id].value="";groupPhotoRefs.current[m.id].click();}}
                    style={{width:90,height:120,borderRadius:14,overflow:"hidden",flexShrink:0,border:`3px solid ${m.passportPhoto?A.cyan+"88":A.border}`,background:A.card2,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",position:"relative"}}>
                    {m.passportPhoto?<img src={m.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<><div style={{fontSize:32,marginBottom:6}}>📷</div><div style={{fontFamily:BF,fontSize:9,color:A.muted,textAlign:"center"}}>Subir foto</div></>}
                    <div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(0,0,0,0.55)",padding:"4px 0",textAlign:"center",fontFamily:BF,fontSize:9,color:"#fff"}}>{m.passportPhoto?"✏️ Cambiar":"📷 Añadir"}</div>
                  </div>
                  <input ref={el=>{if(el)groupPhotoRefs.current[m.id]=el;}} type="file" accept="image/jpeg,image/png,image/webp" style={{display:"none"}} onChange={e=>{if(e.target.files[0])uploadForMember(m.id,e.target.files[0]);e.target.value="";}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      <div style={{fontFamily:ANTON,fontSize:20,color:isMe?A.cyan:"#fff",letterSpacing:0.5}}>{m.nombre}</div>
                      {isMe&&<span style={{background:A.cyan+"22",border:`1px solid ${A.cyan}44`,borderRadius:20,padding:"2px 8px",fontFamily:BF,fontSize:9,color:A.cyan,fontWeight:700}}>TÚ</span>}
                    </div>
                    {m.passportPhoto?<div style={{background:A.green+"18",border:`1px solid ${A.green}33`,borderRadius:8,padding:"5px 10px",marginBottom:8,display:"inline-flex",alignItems:"center",gap:6}}><span style={{color:A.green}}>✓</span><span style={{fontFamily:BF,fontSize:12,color:A.green,fontWeight:700}}>Pasaporte subido</span></div>:<div style={{background:A.orange+"18",border:`1px solid ${A.orange}33`,borderRadius:8,padding:"5px 10px",marginBottom:8,display:"inline-flex",alignItems:"center",gap:6}}><span>⚠️</span><span style={{fontFamily:BF,fontSize:12,color:A.orange,fontWeight:700}}>Pendiente</span></div>}
                    {m.docNumero&&<div style={{fontFamily:ANTON,fontSize:16,color:A.cyan,letterSpacing:2,marginBottom:4}}>{m.docNumero}</div>}
                    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      {m.birthDate&&<div style={{background:A.card2,borderRadius:8,padding:"4px 10px",fontFamily:BF,fontSize:11,color:A.gold}}>🎂 {new Date(m.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"})}</div>}
                      {exp&&<div style={{background:warn?A.orange+"22":A.card2,borderRadius:8,padding:"4px 10px",fontFamily:BF,fontSize:11,color:warn?A.orange:A.muted}}>📅 {exp}{warn?" ⚠️":""}</div>}
                    </div>
                    <button onClick={()=>{groupPhotoRefs.current[m.id].value="";groupPhotoRefs.current[m.id].click();}} style={{marginTop:10,width:"100%",padding:"10px",border:`1.5px dashed ${isMe?A.cyan:A.purple}`,borderRadius:10,background:"transparent",color:isMe?A.cyan:A.purple,fontFamily:BF,fontSize:12,cursor:"pointer",fontWeight:700}}>📷 {m.passportPhoto?"Actualizar foto":"Subir pasaporte"}</button>
                  </div>
                </div>
              </div>
            );
  ...
  if(view==="maleta"){
    ...
    const toggle=key=>{const nm=marcados.includes(key)?marcados.filter(k=>k!==key):[...marcados,key];sC(clients.map(x=>x.id===cid?{...x,maletaMarcados:nm}:x));};
    const addPersonal=()=>{if(!maletaNewItem.trim())return;const texto=maletaNewItem.trim();setMaletaNewItem("");sC(clients.map(x=>x.id===cid?{...x,maletaPersonal:[...(x.maletaPersonal||[]),{id:uid(),texto}]}:x));};
      ...
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,paddingBottom:40,fontFamily:BF}}>
        ...
        <div style={{padding:"14px 16px"}}>
          <div style={{background:A.card2,borderRadius:14,padding:"14px 16px",marginBottom:14,border:`1px solid ${A.border}`}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontFamily:ANTON,fontSize:16,color:A.text}}>EMPACADO</div><div style={{fontFamily:ANTON,fontSize:16,color:checkedCount===totalItems&&totalItems>0?A.green:A.cyan}}>{checkedCount}/{totalItems}</div></div>
          ...
          {[...vuelos,...myDocs].map((v,i)=>(
            <div key={i} style={{background:A.card,border:`1px solid ${A.border}`,borderRadius:18,padding:"20px",marginBottom:14}}>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
                ...
                <div><div style={{fontFamily:ANTON,fontSize:16,color:"#fff",letterSpacing:1,lineHeight:1.2,marginBottom:4}}>{v.nombre}</div><div style={{fontFamily:BF,fontSize:13,color:A.muted}}>{v.descripcion||v.archivo}</div></div>
              </div>
              {v.data?(<a href={`data:${v.mimeType||"application/pdf"};base64,${v.data}`} download={v.nombre} style={{display:"block",width:"100%",padding:"14px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"pointer",background:`linear-gradient(90deg,${A.red},#c00020)`,color:"#fff",textTransform:"uppercase",textDecoration:"none",textAlign:"center",boxSizing:"border-box"}}>DESCARGAR</a>):(<div style={{width:"100%",padding:"14px",borderRadius:10,fontFamily:ANTON,fontSize:13,background:A.card2,color:A.muted,textAlign:"center"}}>Pendiente de asignación</div>)}
            </div>
          ))}
          <BigBack/>
        </div>
          ...
          {[...docs,...myDocs].map((d,i)=>(
            <div key={i} style={{background:A.card,border:`1px solid ${A.border}`,borderRadius:18,padding:"20px",marginBottom:14}}>
              <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
                ...
                <div><div style={{fontFamily:ANTON,fontSize:16,color:"#fff",letterSpacing:1,lineHeight:1.2,marginBottom:4}}>{d.nombre}</div><div style={{fontFamily:BF,fontSize:13,color:A.muted}}>{d.descripcion||d.archivo}</div></div>
              </div>
              {d.data?(<a href={`data:${d.mimeType||"application/pdf"};base64,${d.data}`} download={d.nombre} style={{display:"block",width:"100%",padding:"14px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"pointer",background:`linear-gradient(90deg,${A.red},#c00020)`,color:"#fff",textTransform:"uppercase",textDecoration:"none",textAlign:"center",boxSizing:"border-box"}}>DESCARGAR</a>):(<div style={{width:"100%",padding:"14px",borderRadius:10,fontFamily:ANTON,fontSize:13,background:A.card2,color:A.muted,textAlign:"center"}}>Pendiente de asignación</div>)}
            </div>
          ))}
          <BigBack/>
        </div>
  ...
  if(view==="pagos"){
    ...
    const tripBank=BANK_ACCOUNTS.find(b=>b.id===(trip.tripBankId||"principal"))||BANK_ACCOUNTS[0];
    const isTransfer=fp.k==="transferencia"||fp.k==="vip";
    return(
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,paddingBottom:32,fontFamily:BF}}>
        ...
        <div style={{padding:"16px"}}>
          <div style={{background:`linear-gradient(135deg,${A.cyan}18 0%,#172030 100%)`,borderRadius:20,padding:"20px",marginBottom:14,border:`1px solid ${A.cyan}33`}}>
            <div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:4,textTransform:"uppercase",marginBottom:10}}>Forma de pago: {fp.k!=="vip"?`${fp.icon} ${fp.label}`:"🏦 Transferencia"}</div>
            {isTransfer&&(<div><div style={{fontFamily:ANTON,fontSize:20,color:"#fff",letterSpacing:2,marginBottom:4}}>{tripBank.titular}</div><div style={{fontFamily:"monospace",fontSize:14,color:A.muted,letterSpacing:1,marginBottom:12}}>{tripBank.iban}</div><CopyBtn text={tripBank.iban} label="Copiar IBAN"/></div>)}
            {pendiente>=0&&pc[pendiente]&&(<div style={{marginTop:14,background:A.bg,borderRadius:12,padding:"12px 14px",border:`1px solid ${A.gold}44`}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                <div>
                  <div style={{fontFamily:ANTON,fontSize:20,color:A.gold}}>{(cl.pagosImporteCustom||[])[pendiente]||pc[pendiente].importe||"—"}</div>
                  <div style={{fontSize:12,color:A.muted,fontFamily:BF}}>{pc[pendiente].label} · {pc[pendiente].fecha||"Fecha por confirmar"}</div>
                </div>
                <CopyBtn text={`${(cl.pagosImporteCustom||[])[pendiente]||pc[pendiente].importe||""} - ${fp.k!=="vip"?fp.label:"Transferencia"}: ${pc[pendiente].label} - ${trip.name}`} label="Copiar concepto"/>
              </div>
              {isTransfer&&(<div style={{background:A.card2,borderRadius:10,padding:"10px 12px",border:`1px solid ${A.border}`}}>
                <div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Datos para transferencia</div>
                <div style={{fontFamily:ANTON,fontSize:13,color:"#fff",marginBottom:2}}>{tripBank.titular}</div>
                <div style={{fontFamily:"monospace",fontSize:13,color:A.cyan,letterSpacing:0.5,marginBottom:8,wordBreak:"break-all"}}>{tripBank.iban}</div>
                <CopyBtn text={tripBank.iban} label="📋 Copiar IBAN"/>
              </div>)}
            </div>)}
          </div>
          {pc.map((p,i)=>{const done=pe[i]==="pagado";const urg=!done&&isUrgent(p.fechaISO);const ovd=!done&&isOverdue(p.fechaISO);const imp=(cl.pagosImporteCustom||[])[i]||p.importe||"—";return(<div key={i} style={{background:A.card,borderRadius:16,padding:"16px 18px",marginBottom:10,border:`2px solid ${done?A.green+"44":ovd?A.red+"44":urg?A.orange+"44":A.border}`,display:"flex",alignItems:"center",gap:14}}><div style={{width:44,height:44,borderRadius:"50%",background:done?A.green+"22":ovd?A.red+"22":urg?A.orange+"22":A.card2,border:`2px solid ${done?A.green:ovd?A.red:urg?A.orange:A.border}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontFamily:ANTON,fontSize:18,color:done?A.green:ovd?A.red:urg?A.orange:A.muted}}>{done?"✓":i+1}</span></div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:15,fontWeight:700,color:done?A.green:A.text}}>{p.label}</div><div style={{fontSize:12,color:A.muted,fontFamily:BF}}>{p.fecha||"Fecha por confirmar"}</div></div><div style={{fontFamily:ANTON,fontSize:18,color:done?A.green:A.gold}}>{imp}</div></div>);})}
          <BigBack/>
        </div>
      </div>
    );
  ...
  if(view==="info"){
    const info=trip.info||[];const hotels=trip.hotels||[];
    const emerg=trip.emergencias||emptyEmergencias();
    const emergItems=[{k:"policia",l:"Policía",n:emerg.policia},{k:"ambulancia",l:"Ambulancia",n:emerg.ambulancia},{k:"bomberos",l:"Bomberos",n:emerg.bomberos},{k:"tourleader",l:"Tour Leader",n:emerg.tourleader}].filter(e=>e.n);
    return(
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,paddingBottom:32,fontFamily:BF}}>
        <SubHeader title="Información" color={A.purple}/>
        <div style={{padding:"16px"}}>
          {hotels.map(h=>(<div key={h.id} style={{background:A.card,borderRadius:20,padding:"20px",marginBottom:14,border:`2px solid ${A.gold}55`}}>{h.fechasEstancia&&<div style={{display:"inline-flex",alignItems:"center",gap:6,background:A.gold+"22",border:`1px solid ${A.gold}44`,borderRadius:30,padding:"6px 14px",marginBottom:14}}><span style={{fontSize:14}}>📅</span><span style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.gold}}>{h.fechasEstancia}</span></div>}<div style={{fontFamily:ANTON,fontSize:24,color:"#fff",marginBottom:14,textTransform:"uppercase"}}>{h.nombre}</div>{h.direccion&&<div style={{background:A.bg,borderRadius:12,padding:"12px 14px",border:`1px solid ${A.border}`,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20,flexShrink:0}}>📍</span><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:15,color:A.text,fontWeight:700}}>{h.direccion}</div></div><CopyBtn text={h.direccion}/></div>}</div>))}
          {info.map((it,i)=>(<div key={i} style={{background:A.card,borderRadius:16,padding:"14px 16px",marginBottom:10,border:`1px solid ${A.border}`}}><div style={{display:"flex",gap:12,alignItems:"flex-start"}}><span style={{fontSize:26,flexShrink:0}}>{it.icono}</span><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:15,fontWeight:700,color:A.text,marginBottom:3}}>{it.titulo}</div><div style={{fontSize:13,color:A.muted,lineHeight:1.6,fontFamily:BF}}>{it.texto}</div>{it.url&&<a href={it.url} target="_blank" rel="noreferrer" style={{display:"inline-block",marginTop:8,fontSize:12,color:A.cyan,fontFamily:BF}}>Más info →</a>}</div></div></div>))}
          {emergItems.length>0&&(<div style={{background:A.red+"11",borderRadius:16,padding:"14px 16px",marginBottom:10,border:`1px solid ${A.red}33`}}><div style={{fontFamily:BF,fontSize:10,color:A.red,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>SOS Emergencias</div>{emergItems.map(e=>(<a key={e.k} href={`tel:${e.n}`} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:A.bg,borderRadius:10,marginBottom:8,textDecoration:"none",border:`1px solid ${A.border}`}}><span style={{fontFamily:BF,fontSize:13,color:A.muted,flex:1}}>{e.l}</span><span style={{fontFamily:ANTON,fontSize:17,color:A.red}}>{e.n}</span></a>))}</div>)}
