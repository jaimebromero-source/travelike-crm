...
function CartaDePagoGenerator({onBack}){
  ...
  const curBank=BANK_ACCOUNTS[selectedBank]||BANK_ACCOUNTS[0];
  ...
  const PreviewCard=()=>(
    <div style={{border:`1px solid ${A.border}`,borderRadius:12,overflow:"hidden",background:"#fff"}}>
      ...
      <div style={{padding:"12px 18px"}}>
        ...
        <div style={{background:"#0D2137",borderRadius:6,padding:"8px 12px",marginBottom:10}}><div style={{fontSize:7,color:"#1AB5B0",letterSpacing:2,marginBottom:3}}>DATOS BANCARIOS</div><div style={{fontSize:10,color:"#fff",fontWeight:600}}>{curBank.iban}</div><div style={{fontSize:9,color:"rgba(255,255,255,0.4)",marginTop:1}}>Titular: {curBank.titular}</div></div>
        ...
        {tab==="general"&&(<div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
            {BANK_ACCOUNTS.map((acc,i)=>(
              <button key={acc.id} onClick={()=>setSelectedBank(i)}
                style={{background:selectedBank===i?A.gold+"18":A.card2,border:`2px solid ${selectedBank===i?A.gold:A.border}`,borderRadius:12,padding:"12px 14px",cursor:"pointer",textAlign:"left",display:"flex",flexDirection:"column",gap:4}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontFamily:BF,fontSize:12,fontWeight:700,color:selectedBank===i?A.gold:A.text}}>{acc.label}</span>
                  {selectedBank===i&&<span style={{color:A.gold,fontSize:16}}>&#10003;</span>}
                </div>
                <span style={{fontFamily:"monospace",fontSize:12,color:selectedBank===i?A.gold:A.muted,letterSpacing:1}}>{acc.iban}</span>
              </button>
            ))}
          </div>
      ...
      <div style={{position:"fixed",left:-9999,top:0,width:794,pointerEvents:"none",zIndex:-1}}>
        <div ref={vRef} style={{width:794,minHeight:1000,background:"#fff",fontFamily:"'Barlow Condensed',sans-serif"}}>
          ...
          <div style={{padding:"32px 40px"}}>
            ...
            <div style={{background:"#0D2137",borderRadius:8,padding:"16px 20px",marginBottom:24}}><div style={{fontSize:9,color:"#1AB5B0",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Datos bancarios para transferencia</div><div style={{display:"flex",gap:32}}><div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>IBAN</div><div style={{fontSize:13,color:"#fff",fontWeight:600}}>{curBank.iban}</div></div><div><div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Titular</div><div style={{fontSize:13,color:"#fff",fontWeight:600}}>{curBank.titular}</div></div></div></div>
...
// -- LinkWABtns: Link+WA sin violar Reglas de Hooks --
function LinkWABtns({c,trips}){
  ...
  const wa=()=>{
    const msg=trip
      ?`Hola ${c.nombre.split(" ")[0]}! \u2708\ufe0f\n\nAqu\u00ed tienes tu portal para ${trip.name}:\n${link}\n\n\u00a1Estamos aqu\u00ed para lo que necesites! \ud83d\ude0a`
      :`Hola ${c.nombre.split(" ")[0]}! \u2708\ufe0f\n\nTu portal Travelike:\n${link}\n\n\u00a1Estamos aqu\u00ed para lo que necesites! \ud83d\ude0a`;
    window.open(`https://wa.me/${WA_NUM}?text=${encodeURIComponent(msg)}`,"_blank");
  };
  return(<>
    <button onClick={()=>{navigator.clipboard.writeText(link).then(()=>{setCop(true);setTimeout(()=>setCop(false),2000);});}}
      style={{...ab(cop?A.green+"22":A.cyan+"15",cop?A.green:A.cyan),flex:1,border:`1px solid ${cop?A.green:A.cyan}44`,padding:"6px 8px",fontSize:11,borderRadius:8,fontFamily:BF}}>
      {cop?"Copiado!":"Link"}
    </button>
    <button onClick={wa}
      style={{...ab(A.green+"15",A.green),flex:1,border:`1px solid ${A.green}44`,padding:"6px 8px",fontSize:11,borderRadius:8,fontFamily:BF}}>
      WhatsApp
    </button>
  </>);
}
...
function PeopleTab({trip,tc,clients,trips,sC,updTrip,updClient,passRef,upIdx,setUpIdx,passModal,setPassModal,chPassRef,addPass,hasUrgent,stFilterModal,setStFilterModal,rmModal,setRmModal,roomMenu,setRoomMenu,acmpModal,setAcmpModal,fpMenu,setFpMenu,setRoommate,setTab,viewPhoto,setViewPhoto}){
  ...
  return(
    <div style={{ padding:"0 16px" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
              ...
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                  ...
                  {c.groupId&&<span style={{ flexShrink:0,fontSize:10,background:A.purple+"22",color:A.purple,border:`1px solid ${A.purple}44`,borderRadius:6,padding:"1px 6px",fontFamily:BF,fontWeight:700 }}>Grupo {tc.filter(x=>x.groupId===c.groupId).length}</span>}
                </div>
      ...
      {addModal==="existing"&&(()=>{
        // Agrupar clientes por groupId para mostrarlos juntos
        const toggleGroup=(c)=>{
          const groupMembers=c.groupId?filteredEx.filter(x=>x.groupId===c.groupId).map(x=>x.id):[c.id];
          const allSel=groupMembers.every(id=>exSelected.includes(id));
          setExSelected(prev=>allSel?prev.filter(id=>!groupMembers.includes(id)):[...new Set([...prev,...groupMembers])]);
        };
        // Ordenar: grupos primero, luego por groupId para mantenerlos juntos
        const sorted=[...filteredEx].sort((a,b)=>{
        ...
        return(<AModal title="Añadir viajeros existentes" onClose={()=>{setAddModal(null);setExSearch("");setExSelected([]);}}>
          ...
          {filteredEx.length===0?<AEmpty text="Sin clientes disponibles"/>:sorted.map(c=>{
            ...
            const isGroupLeader=c.groupId&&!seenGroups.has(c.groupId);
            if(c.groupId) seenGroups.add(c.groupId);
            const groupSize=c.groupId?allOtherClients.filter(x=>x.groupId===c.groupId).length:1;
            const groupAllSel=c.groupId?allOtherClients.filter(x=>x.groupId===c.groupId).every(x=>exSelected.includes(x.id)):sel;
            return(<div key={c.id}>
              {isGroupLeader&&<div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 2px 6px",marginTop:4}}>
                <div style={{flex:1,height:1,background:A.purple+"44"}}/>
                <span style={{fontFamily:BF,fontSize:10,color:A.purple,fontWeight:700,letterSpacing:1}}>GRUPO · {groupSize} personas</span>
                <div style={{flex:1,height:1,background:A.purple+"44"}}/>
              </div>}
              <button onClick={()=>toggleGroup(c)} style={{ ...ab(sel?A.cyan+"22":c.groupId?A.purple+"11":A.card2,A.text),width:"100%",marginBottom:6,textAlign:"left",border:`2px solid ${sel?A.cyan:c.groupId?A.purple+"44":A.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:10 }}>
                ...
                {c.groupId&&isGroupLeader&&!sel&&<span style={{fontSize:9,background:A.purple+"22",color:A.purple,border:`1px solid ${A.purple}44`,borderRadius:6,padding:"2px 6px",fontFamily:BF,fontWeight:700,flexShrink:0}}>+{groupSize} juntos</span>}
  ...
  return(<div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF }}>
    ...
    <div style={{ padding:"28px 22px" }}>
      {step===0&&(<div>
        ...
        <div style={{ fontSize:15,color:A.muted,lineHeight:1.7,marginBottom:24 }}>¡Hola! Para preparar tu viaje solo necesitamos un par de cosas rápidas. En menos de 2 minutos lo tienes todo listo.</div>
      ...
      {step===1&&(<div>
        ...
        <div style={{ fontSize:14,color:A.muted,lineHeight:1.6,marginBottom:20 }}>Necesitamos una foto de la página principal de tu pasaporte o DNI. Puedes hacer una foto ahora o subir una que ya tengas en el móvil.</div>
        <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/webp" style={{ display:"none" }} onChange={handleFile}/>
        {photo?(<div style={{ borderRadius:14,overflow:"hidden",border:`2px solid ${A.cyan}`,marginBottom:20,position:"relative" }}><img src={photo} alt="Pasaporte" style={{ width:"100%",maxHeight:220,objectFit:"cover",display:"block" }}/><button onClick={()=>{setPhoto(null);fileRef.current.click();}} style={{ position:"absolute",top:8,right:8,background:"rgba(0,0,0,0.7)",border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontFamily:BF,fontSize:12,cursor:"pointer" }}>Cambiar</button></div>):(<button onClick={()=>fileRef.current.click()} style={{ width:"100%",height:160,background:A.card2,border:`2px dashed ${A.border}`,borderRadius:14,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,marginBottom:20 }}><span style={{ fontSize:40 }}>📷</span><span style={{ fontFamily:BF,fontSize:13,color:A.muted }}>Hacer foto o elegir de galería</span></button>)}
...
function Client({go,cid,clients,trips,logout,sC,sT}){
  ...
  return(<div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",paddingBottom:70,fontFamily:BF }}>
    ...
    <div style={{ padding:"12px 16px" }}>
      ...
      {section==="pagos"&&(<div>
        <div style={{fontFamily:ANTON,fontSize:20,color:"#fff",letterSpacing:1,marginBottom:16,textTransform:"uppercase"}}>💳 Mis Pagos</div>
        <div style={{background:allPaid?A.green+"15":anyUrgent?A.orange+"15":A.card,borderRadius:16,padding:"16px",marginBottom:14,border:`2px solid ${allPaid?A.green+"44":anyUrgent?A.orange+"44":A.border}`}}>
          <div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>{allPaid?"✅ Todo pagado":"Estado de pagos"}</div>
          {pc.map((p,i)=>{const done=pe[i]==="pagado";const urg=!done&&(isUrgent(p.fechaISO)||isOverdue(p.fechaISO));const ovd=!done&&isOverdue(p.fechaISO);const imp=(client.pagosImporteCustom||[])[i]||p.importe;const fp=FORMAS_PAGO.find(f=>f.k===(client.formaPago||"transferencia"))||FORMAS_PAGO[0];return(<div key={i} style={{background:done?A.green+"10":urg?A.orange+"10":A.bg,borderRadius:12,padding:"14px 16px",marginBottom:10,border:`2px solid ${done?A.green+"44":urg?A.orange+"44":A.border}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:done||urg?6:0}}><div><div style={{fontFamily:ANTON,fontSize:16,color:done?A.green:urg?A.orange:A.text,letterSpacing:0.5}}>{done?"✓ ":urg?"⚠️ ":""}{p.label}</div>{done?<div style={{fontSize:10,color:A.green,fontFamily:BF,marginTop:2}}>Pago confirmado · {fp.icon} {fp.label}</div>:p.fechaISO?<div style={{fontSize:11,color:ovd?A.red:urg?A.orange:A.muted,marginTop:2}}>Fecha límite: {isoToDisplay(p.fechaISO)}</div>:null}</div><div style={{fontFamily:ANTON,fontSize:22,color:done?A.green:urg?A.orange:A.cyan}}>{imp||"—"}</div></div></div>);})}
        </div>
        {!allPaid&&<div style={{background:A.card,borderRadius:16,padding:"16px",border:`1px solid ${A.border}`}}><div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>🏦 Datos para transferencia</div><div style={{background:A.bg,borderRadius:10,padding:"14px",border:`1px solid ${A.border}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase"}}>IBAN</div><CopyBtn text={BANK_IBAN} label="Copiar"/></div><div style={{fontFamily:ANTON,fontSize:16,color:A.cyan,letterSpacing:1.5,marginBottom:10,wordBreak:"break-all"}}>{BANK_IBAN}</div><div style={{fontFamily:BF,fontSize:13,color:A.muted}}>Titular: <strong style={{color:A.text}}>{BANK_TITULAR}</strong></div></div><div style={{marginTop:12,fontFamily:BF,fontSize:11,color:A.muted,lineHeight:1.7,textAlign:"center"}}>Indica tu nombre y destino en el concepto de la transferencia.</div></div>}
      </div>)}
      {section==="vuelos"&&(<div>
        <div style={{fontFamily:ANTON,fontSize:20,color:"#fff",letterSpacing:1,marginBottom:16,textTransform:"uppercase"}}>✈️ Mis Vuelos</div>
        {(trip.vuelos||[]).length===0&&clientDocs.filter(d=>d.tipo==="vuelo").length===0&&<div style={{background:A.card,borderRadius:16,padding:"28px",textAlign:"center",border:`1px solid ${A.border}`}}><div style={{fontSize:40,marginBottom:10}}>✈️</div><div style={{fontFamily:BF,fontSize:14,color:A.muted}}>Tus vuelos aparecerán aquí cuando el equipo los publique</div></div>}
        {(trip.vuelos||[]).length>0&&<div style={{background:A.card,borderRadius:16,padding:"16px",marginBottom:14,border:`1px solid ${A.border}`}}><div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>Vuelos del programa</div>{(trip.vuelos||[]).map((v,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<(trip.vuelos||[]).length-1?`1px solid ${A.border}22`:"none"}}><div style={{width:36,height:36,borderRadius:9,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:18}}>✈️</span></div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.text}}>{v.nombre}</div>{v.archivo&&<div style={{fontSize:11,color:A.muted,marginTop:2}}>Ref: {v.archivo}</div>}</div></div>))}</div>}
        {clientDocs.filter(d=>d.tipo==="vuelo").length>0&&<div style={{background:A.card,borderRadius:16,padding:"16px",marginBottom:14,border:`1px solid ${A.cyan}33`}}><div style={{fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>📎 Mis billetes descargables</div>{clientDocs.filter(d=>d.tipo==="vuelo").map(doc=>(<div key={doc.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${A.border}22`}}><span style={{fontSize:20}}>✈️</span><div style={{flex:1,minWidth:0}}><div style={{fontFamily:BF,fontSize:13,fontWeight:700,color:A.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{doc.nombre||doc.archivo}</div>{doc.descripcion&&<div style={{fontSize:11,color:A.muted}}>{doc.descripcion}</div>}</div>{doc.data&&<a href={`data:${doc.mimeType||"application/pdf"};base64,${doc.data}`} download={doc.nombre||"billete"} style={{background:A.cyan,border:"none",borderRadius:8,padding:"8px 14px",color:A.bg,fontSize:12,fontFamily:ANTON,letterSpacing:1,textDecoration:"none",flexShrink:0}}>↓ DESCARGAR</a>}</div>))}</div>}
        {acompanantes.filter(a=>(a.personalDocs||[]).some(d=>d.tipo==="vuelo")).map(a=>(<div key={a.id} style={{background:A.card,borderRadius:16,padding:"16px",marginBottom:10,border:`1px solid ${A.purple}33`}}><div style={{fontFamily:BF,fontSize:9,color:A.purple,letterSpacing:3,textTransform:"uppercase",marginBottom:10}}>✈️ Billetes de {a.nombre}</div>{(a.personalDocs||[]).filter(d=>d.tipo==="vuelo").map(doc=>(<div key={doc.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0"}}><div style={{flex:1,minWidth:0}}><div style={{fontFamily:BF,fontSize:13,fontWeight:700,color:A.text}}>{doc.nombre||doc.archivo}</div></div>{doc.data&&<a href={`data:${doc.mimeType||"application/pdf"};base64,${doc.data}`} download={doc.nombre||"billete"} style={{background:A.purple+"22",border:`1px solid ${A.purple}44`,borderRadius:8,padding:"6px 12px",color:A.purple,fontSize:11,fontFamily:BF,fontWeight:700,textDecoration:"none"}}>↓</a>}</div>))}</div>))}
      </div>)}
      {section==="docs"&&(()=>{
        const[docsMode,setDocsMode]=useState("view");const[queue,setQueue]=useState([]);const[processing,setProcessing]=useState(false);const[manFile,setManFile]=useState(null);const[manTipo,setManTipo]=useState("vuelo_ida");const[manTarget,setManTarget]=useState(null);const[manDesc,setManDesc]=useState("");const[manAssigning,setManAssigning]=useState(false);
        const fileRef2=useRef();const manFileRef2=useRef();
        const DOC_TIPOS=[{k:"vuelo_ida",icon:"✈️",label:"Billete IDA"},{k:"vuelo_vuelta",icon:"🔄",label:"Billete VUELTA"},{k:"seguro",icon:"🛡️",label:"Seguro de viaje"},{k:"voucher",icon:"🏨",label:"Voucher hotel"},{k:"visado",icon:"🛂",label:"Visado"},{k:"hotel",icon:"🏩",label:"Reserva hotel"},{k:"otro",icon:"📄",label:"Otro documento"}];
        const allTargets2=[{id:client.id,nombre:client.nombre,type:"client",clientId:client.id},...acompanantes.map(a=>({id:a.id,nombre:a.nombre,type:"acomp",clientId:client.id}))];
        const allMyDocs=[...clientDocs,...acompanantes.flatMap(a=>(a.personalDocs||[]).map(d=>({...d,_acompNombre:a.nombre,_acompId:a.id})))];
        const addFiles=e=>{const files=Array.from(e.target.files||[]);setQueue(q=>[...q,...files.map(f=>({id:uid(),file:f,name:f.name,status:"pending",result:null,b64:null,mime:f.type}))]);e.target.value="";};
        const processAll=async()=>{const pending=queue.filter(q=>q.status==="pending");if(!pending.length)return;setProcessing(true);const encoded=await Promise.all(pending.map(async item=>{const b64=await fileToB64(item.file);return{...item,b64};}));for(const item of encoded){setQueue(q=>q.map(x=>x.id===item.id?{...x,b64:item.b64,status:"processing"}:x));try{const prompt=`Analiza este documento de viaje.\nViajeros: ${allTargets2.map(t=>t.nombre).join(", ")}.\nExtrae:\n1.Nombre del pasajero\n2.Tipo: "vuelo_ida","vuelo_vuelta","seguro","voucher","visado","hotel","otro"\n3.Descripción breve\n4.Referencia\n5.Etiqueta corta\nResponde SOLO JSON:\n{"pasajero":"","tipo":"vuelo_ida","descripcion":"","referencia":"","etiqueta":"IDA"}`;const raw=await callClaude(item.b64,item.mime||"application/pdf",prompt);const parsed=JSON.parse(raw.replace(/```json|```/g,"").trim());const nom=(parsed.pasajero||"").toLowerCase().trim();const matchedTarget=allTargets2.find(t=>t.nombre.toLowerCase()===nom)||allTargets2.find(t=>nom.split(/\s+/).some(p=>p.length>2&&t.nombre.toLowerCase().includes(p)))||null;setQueue(q=>q.map(x=>x.id===item.id?{...x,status:"done",result:{...parsed,matchedTarget}}:x));}catch(e){setQueue(q=>q.map(x=>x.id===item.id?{...x,status:"error"}:x));}}setProcessing(false);};
        const assignDoc=(item,target,tipo)=>{const ext=item.mime?.includes("pdf")?"pdf":item.mime?.includes("png")?"png":"jpg";const etiqueta=item.result?.etiqueta||tipo.replace("vuelo_","").toUpperCase();const doc={id:uid(),tipo:tipo.startsWith("vuelo")?"vuelo":tipo,nombre:`${target.nombre.split(" ")[0]}_${etiqueta}.${ext}`,archivo:item.result?.referencia||item.name,descripcion:item.result?.descripcion,data:item.b64,mimeType:item.mime};if(target.type==="acomp"){updClient(c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===target.id?{...a,personalDocs:[...(a.personalDocs||[]),doc]}:a)}));}else{updClient(c=>({...c,personalDocs:[...(c.personalDocs||[]),doc]}));}setQueue(q=>q.map(x=>x.id===item.id?{...x,status:"assigned"}:x));};
        const assignManual=async()=>{if(!manFile||!manTarget)return;setManAssigning(true);const b64=await fileToB64(manFile);const ti=DOC_TIPOS.find(d=>d.k===manTipo)||DOC_TIPOS[0];const ext=manFile.type?.includes("pdf")?"pdf":manFile.type?.includes("png")?"png":"jpg";const doc={id:uid(),tipo:manTipo.startsWith("vuelo")?"vuelo":manTipo,nombre:`${manTarget.nombre.split(" ")[0]}_${ti.label.replace(/ /g,"_")}.${ext}`,archivo:manDesc||manFile.name,descripcion:manDesc||ti.label,data:b64,mimeType:manFile.type};if(manTarget.type==="acomp"){updClient(c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===manTarget.id?{...a,personalDocs:[...(a.personalDocs||[]),doc]}:a)}));}else{updClient(c=>({...c,personalDocs:[...(c.personalDocs||[]),doc]}));}setManFile(null);setManDesc("");setManTarget(null);setManAssigning(false);};
        const delDoc=(docId,acompId)=>{if(acompId){updClient(c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===acompId?{...a,personalDocs:(a.personalDocs||[]).filter(d=>d.id!==docId)}:a)}));}else{updClient(c=>({...c,personalDocs:(c.personalDocs||[]).filter(d=>d.id!==docId)}));}};
        const doneWithMatch=queue.filter(q=>q.status==="done"&&q.result?.matchedTarget).length;
        return(<div>
          <div style={{fontFamily:ANTON,fontSize:20,color:"#fff",letterSpacing:1,marginBottom:14,textTransform:"uppercase"}}>📄 Mis Documentos</div>
          <div style={{display:"flex",background:A.bg,borderRadius:12,padding:3,marginBottom:14,gap:3,border:`1px solid ${A.border}`}}>
            {[["view","📂 Ver"],["ia","🤖 IA Auto"],["manual","📤 Manual"]].map(([k,l])=>(<button key={k} onClick={()=>setDocsMode(k)} style={{flex:1,background:docsMode===k?A.card2:"transparent",color:docsMode===k?A.cyan:A.muted,border:"none",borderRadius:9,padding:"9px 4px",fontFamily:BF,fontSize:11,fontWeight:700,cursor:"pointer"}}>{l}</button>))}
          </div>
          {docsMode==="view"&&<div>{allMyDocs.length===0&&<div style={{background:A.card,borderRadius:16,padding:"28px",textAlign:"center",border:`1px solid ${A.border}`}}><div style={{fontSize:40,marginBottom:10}}>📄</div><div style={{fontFamily:BF,fontSize:14,color:A.muted}}>Aquí aparecerán tus documentos cuando el equipo los prepare</div></div>}{allMyDocs.map((doc,i)=>{const tiObj=DOC_TIPOS.find(t=>t.k===doc.tipo||t.k.startsWith(doc.tipo||""))||DOC_TIPOS[DOC_TIPOS.length-1];return(<div key={doc.id||i} style={{background:A.card,borderRadius:12,padding:"12px 14px",marginBottom:8,border:`1px solid ${A.border}`,display:"flex",alignItems:"center",gap:12}}><div style={{width:42,height:42,borderRadius:10,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{tiObj.icon}</div><div style={{flex:1,minWidth:0}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{doc.nombre||doc.archivo}</div><div style={{fontSize:11,color:A.muted,marginTop:2}}>{doc._acompNombre?`${doc._acompNombre} · `:""}{tiObj.label}{doc.descripcion?` · ${doc.descripcion}`:""}</div></div><div style={{display:"flex",gap:6,flexShrink:0}}>{doc.data&&<a href={`data:${doc.mimeType||"application/pdf"};base64,${doc.data}`} download={doc.nombre||"documento"} style={{background:A.cyan,border:"none",borderRadius:8,padding:"8px 14px",color:A.bg,fontSize:12,fontFamily:ANTON,letterSpacing:1,textDecoration:"none"}}>↓</a>}<button onClick={()=>delDoc(doc.id,doc._acompId)} style={{background:"transparent",border:"none",color:A.muted,fontSize:16,cursor:"pointer",lineHeight:1}}>✕</button></div></div>);})}</div>}
          {docsMode==="ia"&&<div>
            <div style={{background:A.purple+"15",border:`1px solid ${A.purple}33`,borderRadius:10,padding:"10px 14px",marginBottom:12,fontFamily:BF,fontSize:12,color:A.purple}}>🤖 Sube tus documentos y la IA los identificará y organizará automáticamente</div>
            <input ref={fileRef2} type="file" accept="application/pdf,image/*" multiple style={{display:"none"}} onChange={addFiles}/>
            <div style={{display:"flex",gap:8,marginBottom:10}}><button onClick={()=>fileRef2.current.click()} style={{flex:2,background:A.purple+"22",border:`1.5px dashed ${A.purple}`,borderRadius:10,padding:"12px",color:A.purple,fontFamily:BF,fontSize:13,cursor:"pointer",fontWeight:700}}>📁 Seleccionar PDFs / Fotos</button>{queue.filter(q=>q.status==="pending").length>0&&!processing&&<button onClick={processAll} style={{flex:1,background:A.cyan,border:"none",borderRadius:10,padding:"12px",color:A.bg,fontFamily:BF,fontSize:12,cursor:"pointer",fontWeight:700}}>🔍 Analizar</button>}</div>
            {doneWithMatch>0&&<button onClick={()=>queue.filter(x=>x.status==="done"&&x.result?.matchedTarget).forEach(item=>assignDoc(item,item.result.matchedTarget,item.result.tipo||"doc"))} style={{width:"100%",padding:"12px",background:`linear-gradient(90deg,${A.green},#26a047)`,border:"none",borderRadius:10,color:"#fff",fontFamily:BF,fontSize:13,cursor:"pointer",marginBottom:12,fontWeight:700}}>⚡ Guardar todos automáticamente ({doneWithMatch})</button>}
            {processing&&<div style={{textAlign:"center",padding:"10px",color:A.cyan,fontSize:13,marginBottom:12,fontFamily:BF}}>⏳ Analizando con Claude AI...</div>}
            {queue.length===0&&<AEmpty text="Sube billetes, seguros, vouchers u otros documentos"/>}
            {queue.map(item=>(<div key={item.id} style={{background:A.card,borderRadius:12,padding:"12px",marginBottom:8,border:`1px solid ${item.status==="done"?A.cyan+"44":item.status==="assigned"?A.green+"44":item.status==="error"?A.red+"44":A.border+"44"}`}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:item.result?10:0}}><span style={{fontSize:18}}>{item.status==="pending"?"📄":item.status==="processing"?"⏳":item.status==="done"?"✨":item.status==="assigned"?"✅":"❌"}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:700,color:A.text,fontFamily:BF,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{item.name}</div>{item.result&&<div style={{fontSize:10,color:A.cyan,fontFamily:BF}}>{item.result.tipo?.replace(/_/g," ").toUpperCase()} · {item.result.etiqueta}</div>}</div>{["pending","done","error"].includes(item.status)&&<button onClick={()=>setQueue(q=>q.filter(x=>x.id!==item.id))} style={{background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer"}}>✕</button>}</div>{item.status==="done"&&item.result&&<div>{item.result.descripcion&&<div style={{background:A.bg,borderRadius:8,padding:"8px 10px",marginBottom:8,fontSize:12,color:A.text,fontFamily:BF,border:`1px solid ${A.border}`}}>📋 {item.result.descripcion}{item.result.referencia&&<span style={{color:A.gold}}> · {item.result.referencia}</span>}</div>}<div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Guardar para:</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{allTargets2.map(target=>{const suggested=item.result.matchedTarget?.id===target.id;return(<button key={target.id} onClick={()=>assignDoc(item,target,item.result.tipo||"doc")} style={{padding:"8px 12px",borderRadius:8,cursor:"pointer",fontFamily:BF,border:`2px solid ${suggested?A.cyan:target.type==="acomp"?A.purple:A.border}`,background:suggested?A.cyan+"22":target.type==="acomp"?A.purple+"11":A.bg,color:suggested?A.cyan:target.type==="acomp"?A.purple:A.text,fontSize:11,fontWeight:suggested?800:500}}>{suggested&&"✨ "}{target.nombre.split(" ")[0]}{target.type==="acomp"&&<span style={{fontSize:9,opacity:0.7}}> acomp.</span>}</button>);})}</div></div>}{item.status==="assigned"&&<div style={{fontSize:12,color:A.green,fontWeight:700,fontFamily:BF}}>✓ Guardado en tus documentos</div>}</div>))}
          </div>}
          {docsMode==="manual"&&<div>
            <input ref={manFileRef2} type="file" accept="application/pdf,image/*" style={{display:"none"}} onChange={e=>{setManFile(e.target.files[0]||null);e.target.value="";}}/>
            <div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Tipo de documento</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>{DOC_TIPOS.map(d=>(<button key={d.k} onClick={()=>setManTipo(d.k)} style={{background:manTipo===d.k?A.cyan+"22":A.card,border:`2px solid ${manTipo===d.k?A.cyan:A.border}`,borderRadius:10,padding:"10px 8px",display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}><span style={{fontSize:18}}>{d.icon}</span><span style={{fontFamily:BF,fontSize:11,fontWeight:700,color:manTipo===d.k?A.cyan:A.text}}>{d.label}</span></button>))}</div>
            <button onClick={()=>manFileRef2.current.click()} style={{width:"100%",background:manFile?A.green+"18":A.card2,border:`1.5px dashed ${manFile?A.green:A.border}`,borderRadius:10,padding:"14px",color:manFile?A.green:A.muted,fontFamily:BF,fontSize:13,cursor:"pointer",fontWeight:700,marginBottom:12,textAlign:"center"}}>{manFile?`📎 ${manFile.name}`:"📁 Seleccionar archivo"}</button>
            <input value={manDesc} onChange={e=>setManDesc(e.target.value)} placeholder="Descripción opcional" style={{...ais,marginBottom:14}}/>
            <div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>¿Para quién es?</div>
            <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>{allTargets2.map(target=>{const sel=manTarget?.id===target.id;const photo=(target.type==="client"?client:acompanantes.find(a=>a.id===target.id))?.passportPhoto;return(<button key={target.id} onClick={()=>setManTarget(sel?null:target)} style={{background:sel?A.cyan+"22":A.card,border:`2px solid ${sel?A.cyan:target.type==="acomp"?A.purple+"55":A.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",textAlign:"left"}}><div style={{width:36,height:36,borderRadius:9,overflow:"hidden",flexShrink:0,background:sel?A.cyan+"33":A.card2,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:15,color:sel?A.cyan:A.muted}}>{photo?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:8}} alt=""/>:target.nombre[0]}</div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:sel?A.cyan:A.text}}>{target.nombre}</div>{target.type==="acomp"&&<div style={{fontFamily:BF,fontSize:10,color:A.purple}}>+ acompañante</div>}</div>{sel&&<span style={{color:A.cyan,fontSize:20}}>✓</span>}</button>);})}
            </div>
            <button onClick={assignManual} disabled={!manFile||!manTarget||manAssigning} style={{width:"100%",background:(!manFile||!manTarget)?A.card2:`linear-gradient(90deg,${A.cyan},${A.green})`,border:"none",borderRadius:12,padding:"16px",color:(!manFile||!manTarget)?A.muted:"#000",fontFamily:ANTON,fontSize:16,letterSpacing:1,cursor:(!manFile||!manTarget)?"not-allowed":"pointer",marginBottom:16}}>{manAssigning?"⏳ GUARDANDO...":"💾 GUARDAR DOCUMENTO"}</button>
          </div>}
        </div>);
      })()}
      {section==="info"&&(<div>
        <div style={{fontFamily:ANTON,fontSize:20,color:"#fff",letterSpacing:1,marginBottom:16,textTransform:"uppercase"}}>ℹ️ Info del Viaje</div>
        {trip.webUrl&&<a href={trip.webUrl} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:12,background:A.card,borderRadius:14,padding:"14px 16px",marginBottom:14,border:`1px solid ${A.cyan}33`,textDecoration:"none"}}><div style={{width:42,height:42,borderRadius:10,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>🌐</div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.cyan}}>Página del viaje</div><div style={{fontSize:11,color:A.muted,marginTop:2}}>Ver toda la información en la web</div></div><span style={{color:A.cyan,fontSize:18}}>›</span></a>}
        {(trip.info||[]).length>0&&<div style={{background:A.card,borderRadius:16,padding:"16px",marginBottom:14,border:`1px solid ${A.border}`}}><div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>Información general</div>{(trip.info||[]).map((it,i)=>(<div key={i} style={{display:"flex",gap:12,padding:"12px 0",borderBottom:i<(trip.info||[]).length-1?`1px solid ${A.border}22`:"none"}}><div style={{width:42,height:42,borderRadius:10,background:A.card2,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{it.icono}</div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.text,marginBottom:4}}>{it.titulo}</div><div style={{fontSize:12,color:A.muted,lineHeight:1.7}}>{it.texto}</div>{it.url&&<a href={it.url} target="_blank" rel="noreferrer" style={{display:"inline-block",marginTop:6,fontSize:11,color:A.cyan,wordBreak:"break-all"}}>🔗 {it.url}</a>}</div></div>))}</div>}
        {(trip.hotels||[]).length>0&&<div style={{background:A.card,borderRadius:16,padding:"16px",marginBottom:14,border:`1px solid ${A.gold}33`}}><div style={{fontFamily:BF,fontSize:9,color:A.gold,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>🏨 Alojamientos</div>{(trip.hotels||[]).map((h,i)=>(<div key={h.id||i} style={{padding:"10px 0",borderBottom:i<(trip.hotels||[]).length-1?`1px solid ${A.border}22`:"none"}}><div style={{fontFamily:BF,fontSize:14,fontWeight:700,color:A.text,marginBottom:3}}>{h.nombre}</div>{h.fechasEstancia&&<div style={{fontSize:12,color:A.gold,marginBottom:2}}>📅 {h.fechasEstancia}</div>}{h.direccion&&<div style={{fontSize:11,color:A.muted}}>📍 {h.direccion}</div>}</div>))}</div>}
        {(trip.emergencias?.tourleader||trip.emergencias?.policia)&&<div style={{background:A.red+"15",borderRadius:16,padding:"16px",border:`2px solid ${A.red}44`}}><div style={{fontFamily:BF,fontSize:9,color:A.red,letterSpacing:3,textTransform:"uppercase",marginBottom:12}}>🆘 Contactos de emergencia</div>{[{l:"Tour Leader",v:trip.emergencias.tourleader,icon:"🗺️"},{l:"Policía",v:trip.emergencias.policia,icon:"🚔"},{l:"Ambulancia",v:trip.emergencias.ambulancia,icon:"🚑"},{l:"Bomberos",v:trip.emergencias.bomberos,icon:"🚒"}].filter(x=>x.v).map(item=>(<div key={item.l} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${A.red}22`}}><div style={{width:40,height:40,borderRadius:10,background:A.red+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{item.icon}</div><div style={{flex:1}}><div style={{fontFamily:BF,fontSize:12,color:A.muted}}>{item.l}</div><div style={{fontFamily:ANTON,fontSize:18,color:A.red,letterSpacing:0.5}}>{item.v}</div></div><a href={`tel:${item.v}`} style={{background:A.red,border:"none",borderRadius:10,padding:"10px 16px",color:"#fff",fontFamily:ANTON,fontSize:12,letterSpacing:1,textDecoration:"none",flexShrink:0}}>LLAMAR</a></div>))}</div>}
        {(trip.info||[]).length===0&&!(trip.hotels||[]).length&&!trip.emergencias?.policia&&<AEmpty text="Sin información adicional publicada aún"/>}
      </div>)}
      {section==="maleta"&&(<div>
