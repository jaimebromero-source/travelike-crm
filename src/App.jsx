...
function AHome({go,trips,clients,sT,sC,cfg,sCfg,logoutAdmin,reservas,sReservas,gastosGen,sGastosGen}){
  ...
  return(<div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF}}>
    ...
    <div style={{padding:"10px 16px 0"}}>
      ...
      </button>):(<button onClick={()=>setCfgModal(true)} style={{width:"100%",background:A.card2,border:`1.5px dashed ${A.border}`,borderRadius:14,padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,color:A.muted,fontFamily:BF,fontSize:12,boxSizing:"border-box"}}><span style={{fontSize:18}}>⚡</span>Segundo botón rápido</button>)}
      </div>
    </div>
    <div style={{padding:"12px 16px"}}>
...
function QuickCfgModal({cfg,trips,sCfg,onClose}){const[label,setLabel]=useState(cfg.quickBtnLabel||"💰 Ver gastos");const[color,setColor]=useState(cfg.quickBtnColor||A.orange);const[tripId,setTripId]=useState(cfg.quickTripId||"");const[label2,setLabel2]=useState(cfg.quickBtnLabel2||"📊 Ver pagos");const[color2,setColor2]=useState(cfg.quickBtnColor2||A.cyan);const[tripId2,setTripId2]=useState(cfg.quickTripId2||"");const save=()=>{sCfg({quickTripId:tripId||null,quickBtnLabel:label,quickBtnColor:color,quickTripId2:tripId2||null,quickBtnLabel2:label2,quickBtnColor2:color2});onClose();};const COLORS=[A.orange,A.cyan,A.gold,A.green,A.red,A.purple,"#FF6B6B","#00CED1"];return(<AModal title="Botón rápido" onClose={onClose}><div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Texto del botón</div><input value={label} onChange={e=>setLabel(e.target.value)} style={{...ais,marginBottom:14}}/><div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Viaje de destino</div><select value={tripId} onChange={e=>setTripId(e.target.value)} style={{...ais,marginBottom:14}}><option value="">Sin asignar</option>{trips.map(t=><option key={t.id} value={t.id}>{t.flag} {t.name}</option>)}</select><div style={{fontFamily:BF,fontSize:10,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>Color</div><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16}}>{COLORS.map(c=><button key={c} onClick={()=>setColor(c)} style={{width:36,height:36,borderRadius:10,background:c,border:`3px solid ${color===c?"#fff":c}`,cursor:"pointer",flexShrink:0}}/>)}</div><div style={{borderTop:`1px solid ${A.border}`,marginTop:14,paddingTop:12}}><div style={{fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Botón 2 — Texto</div><input value={label2} onChange={e=>setLabel2(e.target.value)} style={{...ais,marginBottom:8}}/><div style={{fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Botón 2 — Viaje</div><select value={tripId2} onChange={e=>setTripId2(e.target.value)} style={{...ais,marginBottom:8}}><option value="">Sin asignar</option>{trips.map(t=><option key={t.id} value={t.id}>{t.flag} {t.name}</option>)}</select><div style={{fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Botón 2 — Color</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>{[A.orange,A.cyan,A.gold,A.green,A.red,A.purple,"#FF6B6B","#00CED1"].map(c=><button key={c} onClick={()=>setColor2(c)} style={{width:30,height:30,borderRadius:8,background:c,border:`3px solid ${color2===c?"#fff":c}`,cursor:"pointer",flexShrink:0}}/>)}</div></div><ARow><button onClick={onClose} style={{...ab(A.card2,A.muted),flex:1,border:`1px solid ${A.border}`}}>Cancelar</button><button onClick={save} style={{...ab(A.cyan,A.bg),flex:2}}>Guardar</button></ARow></AModal>);}
// ══════════════════════════════════════════════════════════
      ...
      <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,paddingBottom:32,fontFamily:BF}}>
        ...
        <div style={{padding:"16px"}}>
          <div style={{background:`linear-gradient(135deg,${A.cyan}18 0%,#172030 100%)`,borderRadius:20,padding:"20px",marginBottom:14,border:`1px solid ${A.cyan}33`}}>
            ...
            {pendiente>=0&&pc[pendiente]&&(<div style={{marginTop:14,background:A.bg,borderRadius:12,padding:"12px 14px",border:`1px solid ${A.gold}44`}}><div style={{fontFamily:BF,fontSize:10,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Próximo pago</div><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div><div style={{fontFamily:ANTON,fontSize:20,color:A.gold}}>{(cl.pagosImporteCustom||[])[pendiente]||pc[pendiente].importe||"—"}</div><div style={{fontSize:12,color:A.muted,fontFamily:BF}}>{pc[pendiente].label} · {pc[pendiente].fecha||"Fecha por confirmar"}</div></div><CopyBtn text={`${(cl.pagosImporteCustom||[])[pendiente]||pc[pendiente].importe||""} - ${pc[pendiente].label} - ${trip.name}`} label="Concepto"/></div></div>)}
          </div>
