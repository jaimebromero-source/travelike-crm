...
const CURRENCIES = [
  ...
  { code:"RSD",symbol:"din",name:"Dinar serbio" },
  { code:"ISK",symbol:"kr",name:"Corona islandesa" },
  ...
  { code:"AED",symbol:"د.إ",name:"Dírham emiratí" },
  { code:"SAR",symbol:"ر.س",name:"Riyal saudí" },
  { code:"QAR",symbol:"ر.ق",name:"Riyal catarí" },
  { code:"KWD",symbol:"د.ك",name:"Dinar kuwaití" },
  { code:"BHD",symbol:".د.ب",name:"Dinar bareiní" },
  { code:"OMR",symbol:"ر.ع",name:"Riyal omaní" },
  { code:"JOD",symbol:"د.ا",name:"Dinar jordano" },
  { code:"ILS",symbol:"₪",name:"Séquel israelí" },
  { code:"ZAR",symbol:"R",name:"Rand sudafricano" },
  { code:"MAD",symbol:"MAD",name:"Dírham marroquí" },
  { code:"EGP",symbol:"£",name:"Libra egipcia" },
  { code:"TND",symbol:"د.ت",name:"Dinar tunecino" },
  { code:"KES",symbol:"Ksh",name:"Chelín keniano" },
  { code:"NGN",symbol:"₦",name:"Naira nigeriana" },
  { code:"GHS",symbol:"₵",name:"Cedi ghanés" },
  { code:"ETB",symbol:"Br",name:"Birr etíope" },
  { code:"TZS",symbol:"Tsh",name:"Chelín tanzano" },
  { code:"INR",symbol:"₹",name:"Rupia india" },
  { code:"PKR",symbol:"₨",name:"Rupia pakistaní" },
  { code:"BDT",symbol:"৳",name:"Taka bangladesí" },
  { code:"LKR",symbol:"Rs",name:"Rupia de Sri Lanka" },
  { code:"NPR",symbol:"₨",name:"Rupia nepalesa" },
  { code:"CNY",symbol:"¥",name:"Yuan chino" },
  { code:"KRW",symbol:"₩",name:"Won coreano" },
  { code:"TWD",symbol:"NT$",name:"Dólar taiwanés" },
  { code:"HKD",symbol:"HK$",name:"Dólar hongkonés" },
  ...
  { code:"MMK",symbol:"K",name:"Kyat birmano" },
  { code:"KHR",symbol:"៛",name:"Riel camboyano" },
  { code:"LAK",symbol:"₭",name:"Kip laosiano" },
  { code:"BND",symbol:"B$",name:"Dólar de Brunéi" },
  { code:"MNT",symbol:"₮",name:"Tugrik mongol" },
  { code:"KZT",symbol:"₸",name:"Tenge kazajo" },
  { code:"UZS",symbol:"so'm",name:"Som uzbeko" },
  { code:"GEL",symbol:"₾",name:"Lari georgiano" },
  { code:"AMD",symbol:"֏",name:"Dram armenio" },
  { code:"AZN",symbol:"₼",name:"Manat azerbaiyano" },
  { code:"UAH",symbol:"₴",name:"Grivna ucraniana" },
  { code:"RUB",symbol:"₽",name:"Rublo ruso" },
  { code:"BYN",symbol:"Br",name:"Rublo bielorruso" },
  { code:"MDL",symbol:"L",name:"Leu moldavo" },
  { code:"MKD",symbol:"ден",name:"Denar macedonio" },
  { code:"ALL",symbol:"L",name:"Lek albanés" },
  { code:"BAM",symbol:"KM",name:"Marco bosnio" },
  { code:"CZK",symbol:"Kč",name:"Corona checa" },
  { code:"IQD",symbol:"ع.د",name:"Dinar iraquí" },
  { code:"IRR",symbol:"﷼",name:"Rial iraní" },
  { code:"AFN",symbol:"؋",name:"Afgani afgano" },
  { code:"MUR",symbol:"Rs",name:"Rupia mauriciana" },
  { code:"TZS",symbol:"Tsh",name:"Chelín tanzano" },
  { code:"XOF",symbol:"CFA",name:"Franco CFA BCEAO" },
  { code:"XAF",symbol:"CFA",name:"Franco CFA BEAC" },
  { code:"CDF",symbol:"FC",name:"Franco congoleño" },
  { code:"MZN",symbol:"MT",name:"Metical mozambiqueño" },
  { code:"ZMW",symbol:"ZK",name:"Kwacha zambiano" },
  { code:"BWP",symbol:"P",name:"Pula botsuanesa" },
  { code:"NAD",symbol:"N$",name:"Dólar namibio" },
  { code:"MGA",symbol:"Ar",name:"Ariary malgache" },
  { code:"DZD",symbol:"دج",name:"Dinar argelino" },
  { code:"LYD",symbol:"ل.د",name:"Dinar libio" },
  { code:"SDG",symbol:"ج.س",name:"Libra sudanesa" },
  { code:"SOS",symbol:"Sh",name:"Chelín somalí" },
  { code:"RWF",symbol:"Fr",name:"Franco ruandés" },
  { code:"UGX",symbol:"Sh",name:"Chelín ugandés" },
  ...
  { code:"FJD",symbol:"FJ$",name:"Dólar fiyiano" },
  { code:"PGK",symbol:"K",name:"Kina de P. Guinea" },
...
const A = {
  ...
  green:"#34C759", red:"#FF3B30", gold:"#FFD700", purple:"#BF5AF2",
  lightblue:"#4FC3F7"
};
...
const GASTO_TIPOS = [
  { k:"vuelo",       icon:"✈️",  label:"Vuelo" },
  { k:"hotel",       icon:"🏨",  label:"Hotel" },
  { k:"traslado",    icon:"🚕",  label:"Traslado" },
  { k:"guia",        icon:"🗺️",  label:"Guía" },
  { k:"seguro",      icon:"🛡️",  label:"Seguro" },
  { k:"restaurante", icon:"🍽️",  label:"Restaurante" },
  { k:"actividad",   icon:"🎟️",  label:"Actividad" },
  { k:"propina",     icon:"💸",  label:"Propina" },
...
const EDIT_SECS = [
  ...
  { k:"hotels",  icon:"🏨", label:"Hoteles" },
  { k:"maleta",  icon:"🎒", label:"Maleta" },
  { k:"emerg",   icon:"🆘", label:"SOS" },
...
const DEFAULT_SURVEY_CATS = [
  { key:"viaje", label:"Experiencia global", icon:"🌍" },
  { key:"guia",  label:"Guía / Organización", icon:"🗺️" },
  { key:"hotel", label:"Alojamientos",         icon:"🏨" }
];
...
const sendOneSignal = async (title, body, filters=[]) => {
  if (!ONESIGNAL_APP_ID) { console.warn("OneSignal App ID not set"); return { success:false, simulated:true }; }
  try {
    const payload = {
...
const mem = {};
const db = {
  async get(k) { try { const r=localStorage.getItem(k); return r?JSON.parse(r):null; } catch { return mem[k]??null; } },
  async set(k,v) { try { localStorage.setItem(k,JSON.stringify(v)); } catch {} mem[k]=v; }
};
...
function Lightbox({ src, onClose }) { return <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:10000,display:"flex",alignItems:"center",justifyContent:"center" }}><img src={src} style={{ maxWidth:"100%",maxHeight:"100%",objectFit:"contain" }} alt="" /></div>; }
...
function NumPad({ title, subtitle, onSuccess, onCancel, correctPin }) {
  const [val,setVal]=useState(""); const [err,setErr]=useState(false);
  const press = d => {
    if (val.length>=correctPin.length) return;
        ...
        <div style={{ textAlign:"center",marginBottom:20 }}>
          ...
          <div style={{ display:"flex",gap:12,justifyContent:"center",marginTop:20 }}>
            {Array.from({length:correctPin.length}).map((_,i)=>(
              <div key={i} style={{ width:14,height:14,borderRadius:"50%",background:i<val.length?A.cyan:A.card,border:`1px solid ${A.border}` }} />
            ))}
          </div>
          {err && <div style={{ color:A.red,fontSize:13,marginTop:8,fontWeight:700,fontFamily:BF }}>PIN incorrecto</div>}
        </div>
        {KEYS.map((row,ri)=>(
          <div key={ri} style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:10 }}>
            {row.map((k,ki)=>(
              <button key={ki} onClick={()=>{ if(k==="<") setVal(v=>v.slice(0,-1)); else if(k) press(k); }}
                style={{ background:k===""?"transparent":A.card,border:k===""?"none":`1px solid ${A.border}`,borderRadius:12,padding:"16px",color:A.text,fontFamily:ANTON,fontSize:24,cursor:k===""?"default":"pointer" }}>{k}</button>
            ))}
          </div>
        ))}
...
function RegistroPagoModal({ trips, clients, tid, sC, onClose }) {
  ...
  useEffect(()=>{
    ...
    if(client&&pc[pidx]) {
      const imp=(client.pagosImporteCustom||[])[pidx]||pc[pidx].importe||"";
      setMonto(imp.toString().replace(/[^\d.]/g,""));
    }
  },[cid,pidx]);
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      const adm=await db.get(SK_ADMIN);
      ...
      const tt=t.map(tr=>({ hotels:[],emergencias:emptyEmergencias(),surveyEnabled:false,surveyConfig:{categories:[...DEFAULT_SURVEY_CATS],surveyResponses:[]},maletaImprescindibles:[...DEFAULT_IMPRESCINDIBLES],maletaCats:DEFAULT_MALETA_CATS.map(mc=>({...mc,items:[...mc.items]})),gastos:[],facturas:[],currency:"EUR",...tr,pagosConfig:(tr.pagosConfig||[]).map(p=>({fechaISO:"",...p})) }));
      const cc=c.map(cl=>({ personalDocs:[],roommateId:null,acompanantes:[],tripId:null,passportExpiry:"",passportExpiryDismissed:false,notifEnabled:false,surveySubmitted:false,maletaPersonal:[],maletaMarcados:[],formaPago:"transferencia",pagosImporteCustom:[],...cl,acompanantes:(cl.acompanantes||[]).map(a=>({pagosEstado:[],personalDocs:[],...a})) }));
      setTrips(tt); setClients(cc); setCrm(cr); setCfg(cf);
      if(adm){ setAdminOk(true); }
  ...
  const logoutAdmin=async()=>{ await db.set(SK_ADMIN,null); setAdminOk(false); go("home"); };
  const loginAdmin=async()=>{ setAdminOk(true); await db.set(SK_ADMIN,true); go("ahome"); };
  ...
  if(screen==="notifprompt") return <NotifPrompt go={go} cid={nav.cid} clients={clients} sC={sC} />;
...
function PinScreen({ go, onOk, alreadyOk }) {
  ...
  const tryPin=()=>{ if(pin===ADMIN_PIN) onOk(); else { setErr(true); setPin(""); } };
  return (
    <div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",padding:"60px 28px",textAlign:"center" }}>
      <div style={{ fontFamily:ANTON,fontSize:42,letterSpacing:2,color:"#fff",marginBottom:40 }}>ADMIN ACCESO</div>
      <input type="password" value={pin} onChange={e=>{ setPin(e.target.value); setErr(false); }} placeholder="PIN" onKeyDown={e=>e.key==="Enter"&&tryPin()}
        style={{ width:"100%",padding:"22px",fontSize:32,textAlign:"center",background:A.card2,border:`2px solid ${err?A.red:A.border}`,color:A.text,borderRadius:16,fontFamily:ANTON,outline:"none",marginBottom:20,boxSizing:"border-box" }} />
      {err && <div style={{ color:A.red,fontSize:14,marginBottom:14,fontWeight:700,fontFamily:BF }}>PIN incorrecto</div>}
      <button onClick={tryPin} style={{ width:"100%",padding:"16px",border:"none",borderRadius:14,fontFamily:ANTON,fontSize:16,letterSpacing:2,cursor:"pointer",background:A.cyan,color:A.bg,textTransform:"uppercase" }}>VERIFICAR</button>
      <button onClick={()=>go("home")} style={{ marginTop:20,background:"none",border:"none",color:A.muted,fontSize:14,cursor:"pointer",fontFamily:BF }}>Volver</button>
    </div>
  );
}
    ...
    <div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF }}>
      ...
      <div style={{ padding:"12px 16px" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20 }}>
          {up.map(t=>{
            const tc=clients.filter(c=>c.tripId===t.id);
            const conf=tc.filter(c=>c.status==="confirmado"||c.status==="pagado").length;
            return (
        ...
        {hist.length>0 && (
          <div style={{ padding:"12px 0" }}>
            ...
            {hist.slice(0,3).map(t=>(
              <div key={t.id} onClick={()=>go("atrip",{tid:t.id,initTab:"menu"})} style={{ display:"flex",alignItems:"center",gap:10,padding:"7px 10px",cursor:"pointer",background:A.card,borderRadius:10,marginBottom:8,border:`1px solid ${A.border}66` }}>
                <span style={{ fontSize:18 }}>{t.flag}</span>
                <div style={{ flex:1,fontFamily:BF,fontSize:13,color:A.muted }}>{t.name}</div>
              </div>
            ))}
          </div>
        )}
...
function NotifAdmin({ trips, clients }) {
  ...
  const send=async(title,body,target,tripId)=>{
    setSending(true);
    try {
      const result = await sendOneSignal(title,body);
      const count=target==="all"?clients.filter(c=>c.notifEnabled).length:clients.filter(c=>c.tripId===tripId&&c.notifEnabled).length;
      setSent({ title,count,ok:!result.errors,simulated:result.simulated });
    } catch { setSent({ title,count:0,ok:false }); }
    setSending(false);
  ...
  return (
    <div style={{ padding:"12px 16px" }}>
      {sent && <div style={{ background:(sent.ok?A.green:A.orange)+"22",border:`2px solid ${sent.ok?A.green:A.orange}44`,borderRadius:14,padding:"14px 16px",marginBottom:14,fontFamily:BF }}><div style={{ fontFamily:ANTON,fontSize:18,color:sent.ok?A.green:A.orange }}>{sent.ok?"ENVIADO":sent.simulated?"SIMULADO":"ERROR"}</div><div style={{ fontSize:13,color:A.muted,marginTop:4 }}>«{sent.title}»{sent.ok?` — ${sent.count} viajeros`:sent.simulated?" (configura el App ID de OneSignal)":""}</div></div>}
...
// ============================================================
// ATRIP - ADMIN TRIP VIEW (from Code 1, complete)
// ============================================================
function ATrip({ go, tid, initTab, trips, clients, crm, sT, sC, sCrm }) {
  ...
  const PeopleTab=()=>{
    ...
    const [addModal,setAddModal]=useState(null);    const [newNombre,setNewNombre]=useState(""); const [newEmail,setNewEmail]=useState("");
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {tc.length===0?<AEmpty text="Sin viajeros aún" />:tc.map(c=>{
          ...
          const rawAc=(c.acompanantes||[]).filter(a=>typeof a==="string");
          const acCount=rawAc.length;
          const acompClients=rawAc.map(id=>clients.find(x=>x.id===id)).filter(Boolean);
          const roommate=c.roommateId?clients.find(x=>x.id===c.roommateId):null;
          const isInteresado=c.status==="interesado";
          return (
            <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":isInteresado?A.orange+"33":A.border+"44"}` }}>
              ...
              <div style={{ display:"flex",gap:6,marginBottom:6,flexWrap:"wrap" }}>
                {!isInteresado && <button onClick={()=>setRoomMenu(c.id)} style={{ background:rm.color+"22",border:`1.5px solid ${rm.color}55`,color:rm.color,borderRadius:9,padding:"5px 11px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:BF }}>{rm.label} ▾</button>}
              ...
              <div style={{ display:"flex",gap:6,marginBottom:6 }}>
                ...
                {!isInteresado && <button onClick={()=>setRmModal(c.id)} style={{ ...ab(roommate?A.cyan+"15":A.card2,roommate?A.cyan:A.muted),flex:1,border:`1px solid ${roommate?A.cyan+"33":A.border}`,borderRadius:8,padding:"6px",fontSize:10,fontFamily:BF }}>{roommate?roommate.nombre.split(" ")[0]:"Compañero/a"}</button>}
              ...
              {acompClients.length>0&&(
                <div style={{ marginTop:6,display:"flex",gap:4,flexWrap:"wrap" }}>
                  {acompClients.map(ac=><div key={ac.id} style={{ background:A.purple+"22",border:`1px solid ${A.purple}44`,borderRadius:8,padding:"3px 8px",fontSize:10,color:A.purple,fontFamily:BF }}>{ac.nombre.split(" ")[0]} {(ST.find(s=>s.key===ac.status)||ST[0]).emoji}</div>)}
                </div>
              )}
            </div>
          );
        ...
        {passModal && <AModal title={passModal.name} onClose={()=>setPassModal(null)}><div style={{ borderRadius:12,overflow:"hidden",marginBottom:14 }}><img src={passModal.src} style={{ width:"100%",maxHeight:280,objectFit:"cover",display:"block" }} alt="" /></div><input ref={chPassRef} type="file" accept="image/*" capture="environment" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]){ addPass(e.target.files[0],passModal.cid); setPassModal(null); } }} /><ARow><button onClick={()=>setViewPhoto(passModal.src)} style={{ ...ab(A.card2,A.text),flex:1,border:`1px solid ${A.border}` }}>Ver</button><button onClick={()=>{ chPassRef.current.value=""; chPassRef.current.click(); }} style={{ ...ab(A.cyan+"22",A.cyan),flex:1,border:`1px solid ${A.cyan}44` }}>Cambiar</button><button onClick={()=>{ updClient(passModal.cid,c=>({...c,passportPhoto:null})); setPassModal(null); }} style={{ ...ab(A.red+"22",A.red),flex:1,border:`1px solid ${A.red}44` }}>Borrar</button></ARow></AModal>}
  ...
  const PagosTab=()=>{
    const pc=trip.pagosConfig||[];
    const allPeople=[...tc,...tc.flatMap(c=>(c.acompanantes||[]).filter(a=>typeof a==="string").map(id=>{ const ac=clients.find(x=>x.id===id); return ac?{...ac,_pN:c.nombre}:null; }).filter(Boolean))];
    ...
    const [fpMenuPago,setFpMenuPago]=useState(null);
    const saveImp=()=>{ if(!editImpModal) return; updClient(editImpModal.cid,c=>{ const ai=[...(c.pagosImporteCustom||pc.map(()=>null))]; ai[editImpModal.pidx]=newImp?`${newImp}€`:null; return {...c,pagosImporteCustom:ai}; }); setEditImpModal(null); };
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
          ...
          <button onClick={()=>setRegistroPagoModal(true)} style={{ ...ab(A.green+"22",A.green),border:`1px solid ${A.green}55`,padding:"8px 12px",fontSize:12,borderRadius:8,fontFamily:BF }}>💳 Registrar Pago</button>
        </div>
        ...
        {tc.length===0?<AEmpty text="Sin viajeros" />:(
          <div style={{ background:A.card,borderRadius:14,overflow:"hidden",border:`1px solid ${A.border}` }}>
            ...
            {tc.map(c=>{
              ...
              const urgent=hasUrgent(c);
              const acompIds=(c.acompanantes||[]).filter(a=>typeof a==="string");
              const acomps=acompIds.map(id=>clients.find(x=>x.id===id)).filter(Boolean);
              const fp=FORMAS_PAGO.find(f=>f.k===(c.formaPago||"transferencia"))||FORMAS_PAGO[0];
              return (
                <React.Fragment key={c.id}>
                  <div style={{ display:"grid",gridTemplateColumns:`1fr ${pc.map(()=>"1fr").join(" ")}`,borderBottom:`1px solid ${A.border}44`,background:allPaid?A.green+"08":urgent?A.orange+"06":"transparent" }}>
                    <div style={{ padding:"10px 14px",display:"flex",alignItems:"center",gap:6 }}>
                      ...
                      <div style={{ flex:1,minWidth:0 }}><div style={{ fontFamily:ANTON,fontSize:12,color:A.text,lineHeight:1 }}>{c.nombre.split(" ")[0]}</div><button onClick={()=>setFpMenuPago(c.id)} style={{ background:"none",border:"none",cursor:"pointer",padding:0,fontSize:9,color:fp.k==="vip"?A.gold:A.cyan,fontFamily:BF,lineHeight:1.2 }}>{fp.icon} {fp.label} ▾</button></div>
                    </div>
                    {pc.map((p,i)=>{
                      ...
                      return (
                        <div key={i} style={{ borderLeft:`1px solid ${A.border}44`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,position:"relative" }}>
                          ...
                          <button onClick={()=>{ setNewImp((c.pagosImporteCustom||[])[i]?.replace(/[^\d.]/g,"")||pc[i].importe?.replace(/[^\d.]/g,"")||""); setEditImpModal({cid:c.id,pidx:i}); }}
                  ...
                  {acomps.map(ac=>{
                    const acPe=ac.pagosEstado||pc.map(()=>"pendiente");
                    return (
                      <div key={ac.id} style={{ display:"grid",gridTemplateColumns:`1fr ${pc.map(()=>"1fr").join(" ")}`,borderBottom:`1px solid ${A.border}22`,background:A.bg+"88" }}>
                        ...
                        {pc.map((p,i)=>{ const done=acPe[i]==="pagado"; const urg2=!done&&(isUrgent(p.fechaISO)||isOverdue(p.fechaISO)); return (<div key={i} style={{ borderLeft:`1px solid ${A.border}22`,display:"flex",alignItems:"center",justifyContent:"center" }}><button onClick={()=>updClient(ac.id,x=>{ const arr=[...(x.pagosEstado||pc.map(()=>"pendiente"))]; arr[i]=arr[i]==="pagado"?"pendiente":"pagado"; return {...x,pagosEstado:arr}; })} style={{ width:"100%",height:"100%",minHeight:36,background:"transparent",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}><div style={{ fontFamily:ANTON,fontSize:13,color:done?A.green:urg2?A.orange:A.muted+"88" }}>{done?"✓":urg2?"!":"-"}</div></button></div>); })}
                      </div>
                    );
                  })}
                </React.Fragment>
        ...
        {editImpModal && (
          <AModal title="Modificar importe" onClose={()=>setEditImpModal(null)}>
            <div style={{ fontFamily:BF,fontSize:13,color:A.muted,marginBottom:12 }}>
              Importe para <strong style={{ color:A.text }}>{clients.find(c=>c.id===editImpModal.cid)?.nombre}</strong> — {pc[editImpModal.pidx]?.label}
            </div>
            ...
            <ARow>
              ...
              <button onClick={saveImp} style={{ ...ab(A.gold,A.bg),flex:2 }}>Guardar importe</button>
            </ARow>
          </AModal>
        )}
        {editPcModal!==null && (
        ...
        {fpMenuPago && <FormaPagoMenu current={clients.find(c=>c.id===fpMenuPago)?.formaPago||"transferencia"} onSelect={fp=>updClient(fpMenuPago,c=>({...c,formaPago:fp}))} onClose={()=>setFpMenuPago(null)} />}
      </div>
    );
  };
  ...
  const FinanzasTab=()=>{
    ...
    useEffect(()=>{
      ...
      const t=setTimeout(async()=>{
        const eur=await convertToEUR(+gImporte,gCurrency);
        setGImporteEUR(String(eur));
        setConverting(false);
      },600);
    ...
    const totalIngresos=tc.reduce((sum,c)=>{
      const pe=c.pagosEstado||pc.map(()=>"pendiente");
      return sum+pc.reduce((s2,p,i)=>{ const imp=(c.pagosImporteCustom||[])[i]!=null?(+(c.pagosImporteCustom||[])[i].replace(/[^\d.]/g,"")||0):(+p.importe?.replace(/[^\d.]/g,"")||0); return s2+(pe[i]==="pagado"?imp:0); },0);
    },0);
    const totalEsperado=tc.reduce((sum,c)=>{
      const acomps=(c.acompanantes||[]).length;
      const perPerson=pc.reduce((s2,p,i)=>{ const imp=(c.pagosImporteCustom||[])[i]!=null?(+(c.pagosImporteCustom||[])[i].replace(/[^\d.]/g,"")||0):(+p.importe?.replace(/[^\d.]/g,"")||0); return s2+imp; },0);
      return sum+perPerson*(1+acomps);
    },0);
    const beneficio=totalIngresos-totalGastos;
    const numPersonas=tc.reduce((s,c)=>s+1+(c.acompanantes||[]).length,0);
    const costePorPersona=numPersonas>0?totalGastos/numPersonas:0;
    const pendienteTotal=totalEsperado-totalIngresos;

    const printImageAsPDF=(b64,mimeType,nombre)=>{
      const w=window.open('','_blank');
      if(!w) return;
      w.document.write(`<html><head><title>${nombre||'factura'}</title><style>body{margin:0;background:#fff;display:flex;justify-content:center;align-items:flex-start;}img{max-width:100%;height:auto;display:block;}@media print{body{margin:0;}}</style></head><body><img src="data:${mimeType};base64,${b64}" onload="window.print()"/></body></html>`);
      w.document.close();
    };
    ...
    const addFacturaTrip=async e=>{ 
      const files=Array.from(e.target.files); 
      if(!files.length) return; 
      const newFas=await Promise.all(files.map(async f=>{ const b64=await fileToB64(f); return { id:uid(),nombre:f.name,tipo:f.type,data:b64,fecha:new Date().toISOString().split("T")[0],gastoId:null }; }));
      updTrip(t=>({...t,facturas:[...(t.facturas||[]),...newFas]})); 
      e.target.value=""; 
    };
    const addFacturaGasto=async(e,gastoId)=>{ const f=e.target.files[0]; if(!f) return; const b64=await fileToB64(f); updTrip(t=>({...t,facturas:[...(t.facturas||[]),{ id:uid(),nombre:f.name,tipo:f.type,data:b64,fecha:new Date().toISOString().split("T")[0],gastoId }]})); e.target.value=""; };
    ...
    const downloadAllFacturas=()=>{ 
      ...
      all.forEach((fa,idx)=>{ 
        setTimeout(()=>{ const a=document.createElement("a"); a.href=`data:${fa.tipo};base64,${fa.data}`; a.download=fa.nombre; document.body.appendChild(a); a.click(); document.body.removeChild(a); },idx*200); 
      }); 
    };
    ...
    const scanAndRename=async(fa)=>{ 
      ...
      try {
        const parts=fa.tipo?.includes("image")?
          [{ inline_data:{ mime_type:fa.tipo,data:fa.data } },{ text:'Analiza esta factura o ticket. Extrae: proveedor, fecha, importe total con símbolo de moneda, concepto. Responde SOLO JSON: {"proveedor":"...","fecha":"...","importe":"...","moneda":"...","concepto":"..."}' }]:
          [{ text:`Analiza el documento. Extrae proveedor, fecha, importe, moneda, concepto. SOLO JSON: {"proveedor":"...","fecha":"...","importe":"...","moneda":"...","concepto":"..."}` }];
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8 }}>
          {[{l:"Cobrado",v:totalIngresos,c:A.green},{l:"Gastos",v:totalGastos,c:A.red},{l:"Beneficio",v:beneficio,c:beneficio>=0?A.cyan:A.orange}].map(item=>(
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14 }}>
          {[{l:"Total esperado",v:totalEsperado,c:A.gold},{l:"Pendiente cobro",v:pendienteTotal,c:pendienteTotal>0?A.orange:A.green}].map(item=>(
            <div key={item.l} style={{ background:item.c+"15",borderRadius:14,padding:"12px 8px",border:`1px solid ${item.c}33`,textAlign:"center" }}>
              <div style={{ fontFamily:BF,fontSize:8,color:item.c,letterSpacing:2,textTransform:"uppercase",marginBottom:4 }}>{item.l}</div>
              {unlocked?<div style={{ fontFamily:ANTON,fontSize:16,color:item.c,lineHeight:1 }}>{item.v.toLocaleString("es-ES",{minimumFractionDigits:2,maximumFractionDigits:2})}€</div>:<div style={{ fontFamily:ANTON,fontSize:16,color:item.c,lineHeight:1 }}>***</div>}
            </div>
          ))}
        </div>
        {unlocked&&numPersonas>0&&gastos.length>0&&(
          <div style={{ background:A.purple+"15",borderRadius:14,padding:"12px 16px",marginBottom:14,border:`1px solid ${A.purple}33`,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
            <div>
              <div style={{ fontFamily:BF,fontSize:9,color:A.purple,letterSpacing:2,textTransform:"uppercase",marginBottom:2 }}>COSTE NUESTRO POR PERSONA</div>
              <div style={{ fontFamily:BF,fontSize:11,color:A.muted }}>{numPersonas} persona{numPersonas>1?"s":""} · total gastos</div>
            </div>
            <div style={{ fontFamily:ANTON,fontSize:22,color:A.purple }}>{costePorPersona.toLocaleString("es-ES",{minimumFractionDigits:2,maximumFractionDigits:2})}€</div>
          </div>
        )}
          ...
          {showGastoForm&&(
            <div style={{ background:A.card2,borderRadius:14,padding:"14px",marginBottom:12,border:`1px solid ${A.orange}44` }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 90px",gap:8,marginBottom:8 }}>
                ...
                <button onClick={()=>setCurrMenu(true)} style={{ ...ais,cursor:"pointer",textAlign:"center",fontFamily:ANTON,color:A.gold,padding:"8px" }}>
                  {gCurrency} <span style={{ fontSize:10,color:A.muted }}>▾</span>
                </button>
              </div>
                      ...
                      <div style={{ padding:"10px 12px",display:"flex",gap:10,alignItems:"center" }}>
                        ...
                        <div style={{ flex:1,minWidth:0 }}>
                          ...
                          <div style={{ fontSize:11,color:A.muted,fontFamily:BF }}>
                            ...
                            {g.nota&&` · ${g.nota}`}
                            {gFacturas.length>0&&<span style={{ color:A.green }}> · 📄 {gFacturas.length} factura{gFacturas.length>1?"s":""}</span>}
                          </div>
                        </div>
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
            <div style={{ fontFamily:ANTON,fontSize:16,color:"#fff",letterSpacing:1,textTransform:"uppercase" }}>Facturas generales</div>
            <div style={{ display:"flex",gap:6 }}>
              {facturas.filter(f=>!f.gastoId).length>1&&<button onClick={downloadAllFacturas} style={{ ...ab(A.purple+"22",A.purple),border:`1px solid ${A.purple}44`,padding:"7px 12px",fontSize:11,borderRadius:8,fontFamily:BF }}>↓ Todas</button>}
              <button onClick={()=>facturaRef.current.click()} style={{ ...ab(A.gold+"22",A.gold),border:`1px solid ${A.gold}44`,padding:"7px 14px",fontSize:12,borderRadius:8,fontFamily:BF }}>+ Subir</button>
            </div>
          </div>
          <input ref={facturaRef} type="file" accept="application/pdf,image/*" multiple style={{ display:"none" }} onChange={addFacturaTrip} />
          {facturas.filter(f=>!f.gastoId).length===0?<AEmpty text="Sin facturas generales" />:facturas.filter(f=>!f.gastoId).map(fa=>(
            <div key={fa.id} style={{ background:A.card,borderRadius:10,marginBottom:8,border:`1px solid ${A.gold}33`,overflow:"hidden" }}>
              <div style={{ padding:"10px 14px",display:"flex",alignItems:"center",gap:10 }}>
                <span style={{ fontSize:22,flexShrink:0 }}>{fa.tipo?.includes("image")?"🖼️":"📄"}</span>
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontFamily:BF,fontSize:13,color:A.text,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{fa.nombre}</div>
                  <div style={{ fontSize:10,color:A.muted,fontFamily:BF }}>{fa.fecha&&new Date(fa.fecha+"T12:00:00").toLocaleDateString("es-ES")}</div>
                </div>
                <button onClick={()=>scanAndRename(fa)} disabled={scanningFA===fa.id} style={{ ...ab(A.purple+"22",A.purple),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.purple}44`,fontFamily:BF,flexShrink:0 }}>{scanningFA===fa.id?"⏳":"🤖 IA"}</button>
                {fa.tipo?.includes("image")&&fa.data&&<button onClick={()=>printImageAsPDF(fa.data,fa.tipo,fa.nombre)} style={{ ...ab(A.cyan+"22",A.cyan),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.cyan}44`,fontFamily:BF,flexShrink:0 }}>📄 PDF</button>}
                <a href={`data:${fa.tipo};base64,${fa.data}`} download={fa.nombre} style={{ ...ab(A.gold+"22",A.gold),padding:"6px 10px",fontSize:11,borderRadius:8,border:`1px solid ${A.gold}33`,fontFamily:BF,textDecoration:"none" }}>↓</a>
                <button onClick={()=>delFactura(fa.id)} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer",padding:"0 2px",lineHeight:1 }}>✕</button>
              </div>
              {scanResult?.id===fa.id&&(
                <div style={{ background:A.purple+"12",borderTop:`1px solid ${A.purple}33`,padding:"10px 14px" }}>
                  <div style={{ fontFamily:BF,fontSize:10,color:A.purple,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>🤖 Análisis IA</div>
                  <pre style={{ fontFamily:BF,fontSize:12,color:A.text,whiteSpace:"pre-wrap",margin:0,lineHeight:1.6 }}>{scanResult.text}</pre>
                  <button onClick={()=>setScanResult(null)} style={{ background:"transparent",border:"none",color:A.muted,fontSize:11,cursor:"pointer",fontFamily:BF,marginTop:6 }}>Cerrar</button>
                </div>
              )}
            </div>
          ))}
        ...
        {showPin&&<NumPad title="PIN Finanzas" subtitle="Introduce el código de acceso" correctPin={FINANZAS_PIN} onSuccess={()=>{ setUnlocked(true); setShowPin(false); }} onCancel={()=>setShowPin(false)} />}
      </div>
    );
  };
  ...
  const AIDocsTab=()=>{
    ...
    const processAll=async()=>{
      ...
      for(const item of pending){
        ...
        try{
          const b64=await fileToB64(item.file); const mimeType=item.file.type||"application/pdf";
          const prompt=`Analiza este documento de viaje.\nGrupo: ${allTargets.map(t=>t.nombre).join(", ")}.\nExtrae: 1.Pasajero 2.Tipo:vuelo/visado/seguro/doc 3.Descripción 4.Referencia 5.Cliente del grupo (nombre EXACTO o "ninguno")\nResponde SOLO JSON sin markdown: {"pasajero":"","tipo":"vuelo","descripcion":"","referencia":"","cliente":""}`;
          const parts=[ { inline_data:{ mime_type:mimeType, data:b64 } }, { text:prompt } ];
          const raw=await callGemini(parts);
          const parsed=JSON.parse(raw.replace(/```json|```/g,"").trim());
          const matchedTarget=allTargets.find(t=>t.nombre===parsed.cliente)||null;
          setQueue(q=>q.map(x=>x.id===item.id?{...x,status:"done",b64,mimeType:item.file.type||"application/octet-stream",result:{...parsed,matchedTarget}}:x));
        }catch(e){ setQueue(q=>q.map(x=>x.id===item.id?{...x,status:"error"}:x)); }
      }
      setProcessing(false);
    };
    const assignDoc=(item,target,tipo)=>{
      const doc={ id:uid(),tipo,nombre:item.result.descripcion||item.name,archivo:item.result.referencia||item.name,fileData:item.b64||null,mimeType:item.mimeType||null };
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        <button onClick={()=>setTab("menu")} style={{ background:"transparent",border:"none",color:A.cyan,fontSize:14,fontFamily:BF,cursor:"pointer",marginBottom:12 }}>← Volver al menú</button>
        <div style={{ background:A.purple+"15",border:`1px solid ${A.purple}33`,borderRadius:10,padding:"10px 14px",marginBottom:12,fontFamily:BF,fontSize:12,color:A.purple }}>
          🤖 Powered by <strong>Google Gemini</strong> — análisis automático de documentos
        </div>
        <input ref={fileRef} type="file" accept="application/pdf,image/*" multiple style={{ display:"none" }} onChange={addFiles} />
        <button onClick={()=>fileRef.current.click()} style={{ ...ab(A.purple+"22",A.purple),width:"100%",border:`1.5px dashed ${A.purple}`,borderRadius:10,marginBottom:8,fontFamily:BF }}>Seleccionar PDFs / imágenes</button>
        {queue.filter(q=>q.status==="pending").length>0&&!processing&&<button onClick={processAll} style={{ ...ab(A.cyan,A.bg),width:"100%",borderRadius:10,marginBottom:12,fontFamily:BF }}>Analizar con IA</button>}
        {processing&&<div style={{ textAlign:"center",padding:"10px",color:A.purple,fontSize:13,marginBottom:12,fontFamily:BF }}>⏳ Procesando con Gemini...</div>}
        ...
        {queue.map(item=>(
          <div key={item.id} style={{ background:A.card,borderRadius:12,padding:"12px",marginBottom:8,border:`1px solid ${item.status==="done"?A.cyan+"44":item.status==="assigned"?A.green+"44":item.status==="error"?A.red+"44":A.border+"44"}` }}>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:item.result?10:0 }}>
              ...
              <div style={{ flex:1,minWidth:0 }}><div style={{ fontSize:12,fontWeight:700,color:A.text,fontFamily:BF,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{item.name}</div></div>
              {(item.status==="pending"||item.status==="done"||item.status==="error")&&<button onClick={()=>setQueue(q=>q.filter(x=>x.id!==item.id))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer" }}>✕</button>}
            </div>
            {item.status==="done"&&item.result&&<div>{item.result.descripcion&&<div style={{ background:A.bg,borderRadius:8,padding:"8px 10px",marginBottom:8,fontSize:12,color:A.text,fontFamily:BF,border:`1px solid ${A.border}` }}>{item.result.descripcion}</div>}<div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>{allTargets.map(target=>{ const suggested=item.result.matchedTarget?.id===target.id; return(<button key={target.id} onClick={()=>assignDoc(item,target,item.result.tipo||"doc")} style={{ padding:"6px 12px",borderRadius:8,cursor:"pointer",fontFamily:BF,border:`2px solid ${suggested?A.cyan:target.type==="acomp"?A.purple:A.border}`,background:suggested?A.cyan+"22":target.type==="acomp"?A.purple+"11":A.bg,color:suggested?A.cyan:target.type==="acomp"?A.purple:A.text,fontSize:11,fontWeight:suggested?800:500 }}>{suggested&&"✨ "}{target.nombre.split(" ")[0]}{target.type==="acomp"&&<span style={{ fontSize:9,opacity:0.7 }}> acomp.</span>}</button>); })}</div></div>}
            {item.status==="assigned"&&<div style={{ fontSize:12,color:A.green,fontWeight:700,fontFamily:BF }}>✓ Asignado correctamente</div>}
          </div>
        ))}
      </div>
    );
  ...
  const EditTab=()=>{
    ...
    const [newCatIcon,setNewCatIcon]=useState("⭐"); const [newCatLabel,setNewCatLabel]=useState(""); const [newCatKey,setNewCatKey]=useState("");
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {sec==="vuelos"&&<div>
          {(trip.vuelos||[]).map((v,i)=><div key={i} style={{ background:A.card,borderRadius:10,padding:"10px 14px",marginBottom:8,border:`1px solid ${v.fileData?A.cyan+"44":A.border+"44"}`,display:"flex",alignItems:"center",gap:10 }}><div style={{ flex:1 }}><div style={{ fontSize:13,fontWeight:700,color:A.text,fontFamily:BF }}>{v.nombre}</div><div style={{ fontSize:10,color:v.fileData?A.cyan:A.muted,fontFamily:BF }}>{v.fileData?"📎 Archivo adjunto":v.archivo}</div></div>{v.fileData&&<a href={`data:${v.mimeType};base64,${v.fileData}`} download={v.nombre||"billete"} style={{ ...ab(A.cyan+"22",A.cyan),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.cyan}33`,textDecoration:"none",fontFamily:BF }}>↓</a>}<button onClick={()=>updTrip(t=>({...t,vuelos:t.vuelos.filter((_,j)=>j!==i)}))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer" }}>✕</button></div>)}
          <div style={{ background:A.card2,borderRadius:10,padding:"12px",border:`1px solid ${A.border}`,marginTop:8 }}>
            <input value={efNombre} onChange={e=>setEfNombre(e.target.value)} placeholder="Nombre / descripción del vuelo" style={{ ...ais,marginBottom:8 }} />
            <div style={{ display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginBottom:10 }}>
              <input value={efArchivo} onChange={e=>setEfArchivo(e.target.value)} placeholder="Referencia (opcional)" style={ais} />
              <label style={{ ...ab(A.purple+"22",A.purple),border:`1px solid ${A.purple}44`,borderRadius:8,padding:"8px 12px",fontSize:11,cursor:"pointer",fontFamily:BF,display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap" }}>
                📎 Foto/PDF
                <input type="file" accept="image/*,application/pdf" capture="environment" style={{ display:"none" }}
                  onChange={async e=>{ const f=e.target.files[0]; if(!f) return; const b64=await fileToB64(f); if(!efNombre.trim()) setEfNombre(f.name.replace(/\.[^.]+$/,"")); updTrip(t=>({...t,vuelos:[...(t.vuelos||[]),{id:uid(),nombre:efNombre.trim()||f.name,archivo:f.name,fileData:b64,mimeType:f.type}]})); setEfNombre(""); setEfArchivo(""); e.target.value=""; }} />
              </label>
            </div>
            <button onClick={()=>{ if(!efNombre.trim()) return; updTrip(t=>({...t,vuelos:[...(t.vuelos||[]),{id:uid(),nombre:efNombre,archivo:efArchivo}]})); setEfNombre(""); setEfArchivo(""); }} style={{ ...ab(A.cyan,A.bg),width:"100%",borderRadius:8,fontFamily:BF }}>+ Añadir sin archivo</button>
          </div>
        </div>}

        {sec==="docs"&&<div>
          {(trip.docs||[]).map((d,i)=><div key={i} style={{ background:A.card,borderRadius:10,padding:"10px 14px",marginBottom:8,border:`1px solid ${d.fileData?A.gold+"44":A.border+"44"}`,display:"flex",alignItems:"center",gap:10 }}><div style={{ flex:1 }}><div style={{ fontSize:13,fontWeight:700,color:A.text,fontFamily:BF }}>{d.nombre}</div><div style={{ fontSize:10,color:d.fileData?A.gold:A.muted,fontFamily:BF }}>{d.fileData?"📎 Archivo adjunto":d.archivo}</div></div>{d.fileData&&<a href={`data:${d.mimeType};base64,${d.fileData}`} download={d.nombre||"documento"} style={{ ...ab(A.gold+"22",A.gold),padding:"4px 8px",fontSize:10,borderRadius:6,border:`1px solid ${A.gold}33`,textDecoration:"none",fontFamily:BF }}>↓</a>}<button onClick={()=>updTrip(t=>({...t,docs:t.docs.filter((_,j)=>j!==i)}))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer" }}>✕</button></div>)}
          <div style={{ background:A.card2,borderRadius:10,padding:"12px",border:`1px solid ${A.border}`,marginTop:8 }}>
            <input value={dfNombre} onChange={e=>setDfNombre(e.target.value)} placeholder="Nombre del documento" style={{ ...ais,marginBottom:8 }} />
            <div style={{ display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginBottom:10 }}>
              <input value={dfArchivo} onChange={e=>setDfArchivo(e.target.value)} placeholder="Referencia (opcional)" style={ais} />
              <label style={{ ...ab(A.purple+"22",A.purple),border:`1px solid ${A.purple}44`,borderRadius:8,padding:"8px 12px",fontSize:11,cursor:"pointer",fontFamily:BF,display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap" }}>
                📎 Foto/PDF
                <input type="file" accept="image/*,application/pdf" capture="environment" style={{ display:"none" }}
                  onChange={async e=>{ const f=e.target.files[0]; if(!f) return; const b64=await fileToB64(f); if(!dfNombre.trim()) setDfNombre(f.name.replace(/\.[^.]+$/,"")); updTrip(t=>({...t,docs:[...(t.docs||[]),{id:uid(),nombre:dfNombre.trim()||f.name,archivo:f.name,fileData:b64,mimeType:f.type}]})); setDfNombre(""); setDfArchivo(""); e.target.value=""; }} />
              </label>
            </div>
            <button onClick={()=>{ if(!dfNombre.trim()) return; updTrip(t=>({...t,docs:[...(t.docs||[]),{id:uid(),nombre:dfNombre,archivo:dfArchivo}]})); setDfNombre(""); setDfArchivo(""); }} style={{ ...ab(A.cyan,A.bg),width:"100%",borderRadius:8,fontFamily:BF }}>+ Añadir sin archivo</button>
          </div>
        </div>}
        ...
        {sec==="hotels"&&<div>
          {hotels.map((h,i)=>(
            <div key={h.id} style={{ background:A.card,borderRadius:12,padding:"12px 14px",marginBottom:10,border:`1px solid ${A.gold}33` }}>
              ...
              <input value={h.nombre} onChange={e=>updTrip(t=>({...t,hotels:t.hotels.map(x=>x.id===h.id?{...x,nombre:e.target.value}:x)}))} placeholder="Nombre del hotel" style={{ ...ais,marginBottom:8 }} />
              <input value={h.fechasEstancia||""} onChange={e=>updTrip(t=>({...t,hotels:t.hotels.map(x=>x.id===h.id?{...x,fechasEstancia:e.target.value}:x)}))} placeholder="Fechas de estancia" style={{ ...ais,marginBottom:8 }} />
              <input value={h.direccion} onChange={e=>updTrip(t=>({...t,hotels:t.hotels.map(x=>x.id===h.id?{...x,direccion:e.target.value}:x)}))} placeholder="Dirección" style={ais} />
            </div>
          ))}
        ...
        {sec==="emerg"&&<div>
          {[{k:"policia",l:"Policía"},{k:"ambulancia",l:"Ambulancia"},{k:"bomberos",l:"Bomberos"},{k:"embajada",l:"Embajada española"}].map(item=>(
            <div key={item.k}>
              ...
              <input value={emerg[item.k]||""} onChange={e=>updTrip(t=>({...t,emergencias:{...(t.emergencias||emptyEmergencias()),[item.k]:e.target.value}}))} style={{ ...ais,marginBottom:12 }} />
            </div>
          ))}
        </div>}
            ...
            {(sc.surveyResponses||[]).map((r,i)=>(
              <div key={i} style={{ background:A.card,borderRadius:12,padding:"14px",marginBottom:10,border:`1px solid ${A.gold}33` }}>
                ...
                <div style={{ display:"flex",gap:6,flexWrap:"wrap",marginBottom:8 }}>
                  {(sc.categories||[]).map(cat=>r.ratings?.[cat.key]?(
...
// ============================================================
// CLIENT SIDE COMPONENTS (from Code 2, intact)
// ============================================================

function AcompModal({ clientId, clients, updClient, trip, onClose }) {
  const cl = clients.find(c => c.id === clientId);
  const [search, setSearch] = useState("");
  // acompanantes can be string[] (clientIds) or legacy object[]
  const rawAcomps = cl?.acompanantes || [];
  const acompIds = rawAcomps.filter(a => typeof a === "string");
  const acompClients = acompIds.map(id => clients.find(c => c.id === id)).filter(Boolean);

  const link = (targetId) => {
    if (acompIds.includes(targetId)) return;
    updClient(clientId, c => ({ ...c, acompanantes: [...acompIds, targetId] }));
  };
  const unlink = (targetId) => {
    updClient(clientId, c => ({ ...c, acompanantes: acompIds.filter(id => id !== targetId) }));
  };

  const available = clients.filter(c =>
    c.id !== clientId &&
    !acompIds.includes(c.id) &&
    (search.trim() ? matchSearch(c.nombre, search) || matchSearch(c.code || "", search) : true)
  );

  return (
    <AModal title="Acompañantes" onClose={onClose}>
      <div style={{ fontSize: 13, color: A.muted, fontFamily: BF, marginBottom: 12 }}>
        Vincula viajeros ya registrados como acompañantes de <strong style={{ color: A.text }}>{cl?.nombre}</strong>.
      </div>
      {acompClients.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontFamily: BF, fontSize: 9, color: A.muted, letterSpacing: 3, textTransform: "uppercase", marginBottom: 8 }}>Vinculados</div>
          {acompClients.map(ac => {
            const st = ST.find(s => s.key === ac.status) || ST[0];
            return (
              <div key={ac.id} style={{ background: A.bg, borderRadius: 12, padding: "10px 12px", marginBottom: 6, border: `1px solid ${A.green}44`, display: "flex", gap: 10, alignItems: "center" }}>
                <div style={{ width: 38, height: 38, borderRadius: 8, background: A.cyan + "22", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
                  {ac.passportPhoto ? <img src={ac.passportPhoto} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" /> : <span style={{ fontFamily: ANTON, fontSize: 16, color: A.cyan }}>{ac.nombre[0]?.toUpperCase()}</span>}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: BF, fontSize: 14, fontWeight: 700, color: A.text }}>{ac.nombre}</div>
                  <div style={{ fontSize: 10, color: st.color, fontFamily: BF }}>{st.emoji} {st.label} · {ac.code}</div>
                </div>
                <button onClick={() => unlink(ac.id)} style={{ background: A.red + "22", border: `1px solid ${A.red}44`, color: A.red, borderRadius: 8, padding: "4px 10px", fontSize: 11, cursor: "pointer", fontFamily: BF }}>Quitar</button>
              </div>
            );
          })}
        </div>
      )}
      <SearchBar value={search} onChange={setSearch} placeholder="Buscar viajero por nombre o código..." />
      {available.length === 0 && <AEmpty text={search ? "Sin resultados" : "No hay más viajeros disponibles"} />}
      {available.map(c => {
        const st = ST.find(s => s.key === c.status) || ST[0];
        const cTrip = c.tripId ? trip?.id === c.tripId ? null : { name: "otro viaje" } : null;
        return (
          <button key={c.id} onClick={() => link(c.id)} style={{ ...{ background: A.card2, border: `1.5px solid ${A.border}`, borderRadius: 10, padding: "10px 12px", width: "100%", marginBottom: 6, display: "flex", alignItems: "center", gap: 10, cursor: "pointer", textAlign: "left" } }}>
            <div style={{ width: 38, height: 38, borderRadius: 8, background: A.cyan + "22", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
              {c.passportPhoto ? <img src={c.passportPhoto} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" /> : <span style={{ fontFamily: ANTON, fontSize: 16, color: A.cyan }}>{c.nombre[0]?.toUpperCase()}</span>}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: BF, fontSize: 14, fontWeight: 700, color: A.text }}>{c.nombre}</div>
              <div style={{ fontSize: 10, color: st.color, fontFamily: BF }}>{st.emoji} {st.label} · {c.code}</div>
            </div>
            <span style={{ color: A.green, fontSize: 18, flexShrink: 0 }}>+</span>
          </button>
        );
      })}
    </AModal>
...
function Passport({ go, cid, clients, trips, sC, setScreen, setNav }) {
      ...
      await new Promise(resolve => {
        ...
        r.onload = async ev => {
          ...
          if (!expiry) {
            setExtracting(true);
            try {
              const b64 = dataUrl.split(",")[1]; const mt = f.type || "image/jpeg";
              const parts = [{ inline_data: { mime_type: mt, data: b64 } }, { text: 'Extrae la fecha de caducidad del pasaporte.\nResponde SOLO JSON: {"expiry":"YYYY-MM-DD"} o {"expiry":null}' }];
              const raw = await callGemini(parts);
              const parsed = JSON.parse(raw.replace(/```json|```/g, "").trim());
  ...
  const submit = async () => {
    if (photos.length === 0 || !c1) return;
    await sC(clients.map(x => x.id === cid ? { ...x, passportPhoto: photos[0], passportPhotos: photos, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false } : x));
    setNav(n => ({ ...n, cid })); setScreen("notifprompt");
  };

  const expiryDays = daysUntilExpiry(expiry);
  const expiryWarn = expiry && expiryDays !== null && expiryDays <= 365;
    ...
    <div style={{ fontFamily: BF, background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text }}>
      ...
      <div style={{ padding: "24px 20px" }}>
        <div style={{ background: A.card, borderRadius: 14, padding: 20, border: `1px solid ${A.border}`, marginBottom: 16 }}>
          ...
          <div style={{ background: A.card2, borderRadius: 12, padding: "14px", marginBottom: 16, border: `1px solid ${extracting ? A.purple + "44" : expiry ? A.green + "44" : A.border}` }}>
            <div style={{ fontFamily: BF, fontSize: 11, color: extracting ? A.purple : expiry ? A.green : A.muted, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>{extracting ? "⏳ Leyendo con IA..." : expiry ? "✓ Fecha detectada" : "Caducidad del pasaporte"}</div>
            <input type="date" value={expiry} onChange={e => setExpiry(e.target.value)} style={{ ...ais, colorScheme: "dark" }} />
            {expiryWarn && <div style={{ background: A.orange + "22", border: `1px solid ${A.orange}44`, borderRadius: 8, padding: "8px 10px", marginTop: 8, fontSize: 12, color: A.orange, fontFamily: BF }}>{expiryDays !== null && expiryDays >= 0 ? `⚠️ Caduca en ${expiryDays} días` : "⛔ Pasaporte vencido"}</div>}
          </div>
          ...
          <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12, cursor: "pointer", padding: "14px", background: c1 ? A.green + "15" : A.card2, borderRadius: 14, border: `2px solid ${c1 ? A.green : A.border}` }}>
            ...
            <div>
              <div style={{ fontFamily: BF, fontSize: 14, fontWeight: 700, color: c1 ? A.green : A.text, marginBottom: 3 }}>🔒 Uso de datos del pasaporte</div>
              <div style={{ fontFamily: BF, fontSize: 12, color: A.muted, lineHeight: 1.6 }}>Autorizo a Travelike a utilizar mi foto de pasaporte exclusivamente para los trámites del viaje. Tus datos están seguros con nosotros.</div>
            </div>
          </label>
          ...
          <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 20, cursor: "pointer", padding: "14px", background: c2 ? A.purple + "15" : A.card2, borderRadius: 14, border: `2px solid ${c2 ? A.purple : A.border}` }}>
            ...
            <div>
              <div style={{ fontFamily: BF, fontSize: 14, fontWeight: 700, color: c2 ? A.purple : A.text, marginBottom: 3 }}>📸 Fotos del viaje en redes</div>
          ...
          <button onClick={submit} disabled={photos.length === 0 || !c1} style={{ width: "100%", padding: "18px", border: "none", borderRadius: 14, fontFamily: ANTON, fontSize: 16, letterSpacing: 3, cursor: photos.length > 0 && c1 ? "pointer" : "default", background: photos.length > 0 && c1 ? `linear-gradient(90deg,${A.cyan},#0099bb)` : A.card2, color: photos.length > 0 && c1 ? A.bg : A.muted, textTransform: "uppercase" }}>CONTINUAR →</button>
        </div>
      </div>
...
function NotifPrompt({ go, cid, clients, sC }) {
  const enable = async () => { await sC(clients.map(x => x.id === cid ? { ...x, notifEnabled: true } : x)); go("client", { cid }); };
  ...
  return (
    <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 28px", fontFamily: BF, color: A.text, textAlign: "center" }}>
      ...
      <button onClick={enable} style={{ width: "100%", padding: "18px", border: "none", borderRadius: 12, fontFamily: ANTON, fontSize: 16, letterSpacing: 3, cursor: "pointer", background: `linear-gradient(90deg,${A.cyan},#0099bb)`, color: A.bg, textTransform: "uppercase", marginBottom: 14 }}>ACTIVAR</button>
...
function Client({ go, cid, clients, trips, logout, sC, sT }) {
  ...
  const passPortalRef = useRef();
  const [passLightbox, setPassLightbox] = useState(null);
  ...
  if (view === "home") {
    ...
    const passPhotos = cl.passportPhotos || (cl.passportPhoto ? [cl.passportPhoto] : []);
    const mainPhoto = passPhotos[0] || null;
    const primerNombre = cl.nombre.split(" ")[0];
      ...
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", fontFamily: BF, color: A.text, paddingBottom: 32 }}>
        <div style={{ background: "linear-gradient(135deg,#070718 0%,#0f1f3d 60%,#07070f 100%)", padding: "28px 20px 24px", borderBottom: `1px solid ${A.border}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 6 }}>
            <div onClick={() => setView("pasaporte")} style={{ width:52, height:52, borderRadius:12, overflow:"hidden", flexShrink:0, cursor:"pointer", border:mainPhoto?`2px solid ${A.cyan}99`:`2px dashed ${A.border}44`, background:A.cyan+"22", position:"relative" }}>
              {mainPhoto
                ? <img src={mainPhoto} style={{ width:"100%",height:"100%",objectFit:"cover" }} alt="" />
                : <div style={{ width:"100%",height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1 }}><span style={{ fontSize:18 }}>📷</span><span style={{ fontFamily:BF,fontSize:8,color:A.muted,letterSpacing:1 }}>FOTO</span></div>
              }
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily:BF, fontSize:11, color:A.muted, letterSpacing:2, textTransform:"uppercase", marginBottom:2 }}>¡Hola, {primerNombre}!</div>
              <div style={{ fontFamily: ANTON, fontSize: 18, color: "#fff", letterSpacing: 1, textTransform: "uppercase", lineHeight: 1.1 }}>{trip.name}</div>
              <div style={{ fontSize: 11, color: A.cyan, fontFamily: BF, letterSpacing: 2, marginTop: 2 }}>{trip.fechas}</div>
            </div>
        ...
        <div style={{ textAlign: "center", paddingTop: 4 }}><button onClick={logout} style={{ background: "none", border: "none", color: A.muted, fontSize: 13, cursor: "pointer", fontFamily: BF }}>Cerrar sesión</button></div>
      </div>
    );
  }
  ...
  if (view === "maleta") {
    ...
    const addPersonal = () => { if (!maletaNewItem.trim()) return; sC(clients.map(x => x.id === cid ? { ...x, maletaPersonal: [...(x.maletaPersonal || []), { id: uid(), texto: maletaNewItem.trim() }] } : x)); setMaletaNewItem(""); };
      ...
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text, paddingBottom: 40, fontFamily: BF }}>
        ...
        <div style={{ padding: "14px 16px" }}>
          ...
          <div style={{ background: `linear-gradient(135deg,${A.red}18 0%,${A.orange}10 100%)`, borderRadius: 16, padding: "14px", marginBottom: 14, border: `2px solid ${A.red}33` }}>
            ...
            {mImpresc.map((item, i) => {
              const key = `imp_${i}`; const checked = marcados.includes(key);
              return (
                <div key={key} onClick={() => toggle(key)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: checked ? A.green + "15" : A.bg, borderRadius: 10, marginBottom: 6, border: `1.5px solid ${checked ? A.green + "44" : A.border}`, cursor: "pointer" }}>
                  <div style={{ width: 24, height: 24, borderRadius: 6, border: `2px solid ${checked ? A.green : A.border}`, background: checked ? A.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{checked && <span style={{ color: "#fff", fontSize: 14, fontWeight: 900 }}>✓</span>}</div>
                  <span style={{ fontFamily: BF, fontSize: 14, color: checked ? A.muted : A.text, textDecoration: checked ? "line-through" : "none", flex: 1 }}>{item}</span>
                </div>
              );
            })}
          </div>
          ...
          {mCats.map((cat, ci) => {
            const catChecked = cat.items.filter((_, ii) => marcados.includes(`cat_${ci}_${ii}`)).length;
            return (
              <div key={cat.id} style={{ background: A.card, borderRadius: 16, padding: "14px", marginBottom: 10, border: `1px solid ${A.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: 22 }}>{cat.icon}</span><div style={{ fontFamily: ANTON, fontSize: 14, color: "#fff", letterSpacing: 1, textTransform: "uppercase" }}>{cat.label}</div></div>
                  <div style={{ fontFamily: BF, fontSize: 11, color: catChecked === cat.items.length && cat.items.length > 0 ? A.green : A.muted }}>{catChecked}/{cat.items.length}</div>
                </div>
                {cat.items.map((item, ii) => {
                  const key = `cat_${ci}_${ii}`; const checked = marcados.includes(key);
                  return (
                    <div key={key} onClick={() => toggle(key)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: checked ? A.green + "10" : A.bg, borderRadius: 8, marginBottom: 4, border: `1px solid ${checked ? A.green + "33" : A.border + "44"}`, cursor: "pointer" }}>
                      <div style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${checked ? A.green : A.border}`, background: checked ? A.green : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{checked && <span style={{ color: "#fff", fontSize: 12, fontWeight: 900 }}>✓</span>}</div>
                      <span style={{ fontFamily: BF, fontSize: 13, color: checked ? A.muted : A.text, textDecoration: checked ? "line-through" : "none", flex: 1 }}>{item}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ background: A.card, borderRadius: 16, padding: "14px", marginBottom: 14, border: `1px solid ${A.purple}33` }}>
            ...
            {personal.map(p => {
              const key = `per_${p.id}`; const checked = marcados.includes(key);
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: checked ? A.purple + "10" : A.bg, borderRadius: 8, marginBottom: 4, border: `1px solid ${checked ? A.purple + "33" : A.border + "44"}` }}>
                  <div onClick={() => toggle(key)} style={{ width: 20, height: 20, borderRadius: 5, border: `2px solid ${checked ? A.purple : A.border}`, background: checked ? A.purple : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer" }}>{checked && <span style={{ color: "#fff", fontSize: 12, fontWeight: 900 }}>✓</span>}</div>
                  <span onClick={() => toggle(key)} style={{ fontFamily: BF, fontSize: 13, color: checked ? A.muted : A.text, textDecoration: checked ? "line-through" : "none", flex: 1, cursor: "pointer" }}>{p.texto}</span>
                  <button onClick={() => delPersonal(p.id)} style={{ background: "transparent", border: "none", color: A.muted, fontSize: 16, cursor: "pointer", padding: "0 2px", lineHeight: 1 }}>✕</button>
                </div>
              );
            })}
  ...
  if (view === "pasaporte") {
    const passPhotos = cl.passportPhotos || (cl.passportPhoto ? [cl.passportPhoto] : []);
    const addPassPhotos = async e => {
      const files = Array.from(e.target.files);
      if (!files.length) return;
      const newPhotos = [...passPhotos];
      for (const f of files) {
        await new Promise(res => { const r = new FileReader(); r.onload = ev => { newPhotos.push(ev.target.result); res(); }; r.readAsDataURL(f); });
      }
      sC(clients.map(x => x.id === cid ? { ...x, passportPhotos: newPhotos, passportPhoto: newPhotos[0] } : x));
      e.target.value = "";
    };
    const removePassPhoto = idx => {
      const np = passPhotos.filter((_, i) => i !== idx);
      sC(clients.map(x => x.id === cid ? { ...x, passportPhotos: np, passportPhoto: np[0] || null } : x));
    };
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text, paddingBottom: 32, fontFamily: BF }}>
        <SubHeader title="Mi pasaporte 🛂" color={A.cyan} />
        <div style={{ padding: "16px" }}>
          {passPhotos.length > 0 ? (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
              {passPhotos.map((p, i) => (
                <div key={i} style={{ position: "relative", width: 140, height: 100, borderRadius: 12, overflow: "hidden", border: `2px solid ${A.cyan}44`, flexShrink: 0 }}>
                  <img src={p} onClick={() => setPassLightbox(p)} style={{ width: "100%", height: "100%", objectFit: "cover", cursor: "pointer" }} alt="" />
                  <button onClick={() => removePassPhoto(i)} style={{ position: "absolute", top: 4, right: 4, width: 24, height: 24, borderRadius: "50%", background: A.red, border: "none", color: "#fff", fontSize: 13, cursor: "pointer", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
                  {i === 0 && <div style={{ position: "absolute", bottom: 4, left: 4, background: A.cyan, borderRadius: 6, padding: "2px 6px", fontSize: 9, color: A.bg, fontFamily: BF, fontWeight: 700, letterSpacing: 1 }}>PRINCIPAL</div>}
                </div>
              ))}
              <label style={{ width: 140, height: 100, borderRadius: 12, border: `2px dashed ${A.border}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, background: A.card2, gap: 4 }}>
                <span style={{ fontSize: 22, color: A.muted }}>+</span>
                <span style={{ fontFamily: BF, fontSize: 9, color: A.muted, textTransform: "uppercase", letterSpacing: 1 }}>Añadir</span>
                <input ref={passPortalRef} type="file" accept="image/*" multiple capture="environment" style={{ display: "none" }} onChange={addPassPhotos} />
              </label>
            </div>
          ) : (
            <label style={{ display: "block", background: A.card2, border: `2px dashed ${A.border}`, borderRadius: 14, padding: "32px 20px", textAlign: "center", cursor: "pointer", marginBottom: 14 }}>
              <div style={{ fontSize: 40, marginBottom: 10 }}>📷</div>
              <div style={{ fontFamily: ANTON, fontSize: 15, color: A.cyan, letterSpacing: 1, marginBottom: 4 }}>SUBIR FOTO DEL PASAPORTE</div>
              <div style={{ fontFamily: BF, fontSize: 12, color: A.muted }}>Anverso, reverso... puedes subir varias</div>
              <input type="file" accept="image/*" multiple capture="environment" style={{ display: "none" }} onChange={addPassPhotos} />
            </label>
          )}
          {cl.passportExpiry && (
            <div style={{ background: passportWarn(cl.passportExpiry) ? A.orange + "22" : A.green + "15", border: `1px solid ${passportWarn(cl.passportExpiry) ? A.orange + "44" : A.green + "33"}`, borderRadius: 12, padding: "12px 14px", marginBottom: 14 }}>
              <div style={{ fontFamily: BF, fontSize: 10, color: passportWarn(cl.passportExpiry) ? A.orange : A.green, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Caducidad del pasaporte</div>
              <div style={{ fontFamily: ANTON, fontSize: 18, color: passportWarn(cl.passportExpiry) ? A.orange : A.green }}>
                {new Date(cl.passportExpiry + "T12:00:00").toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}
              </div>
              {passportWarn(cl.passportExpiry) && <div style={{ fontFamily: BF, fontSize: 12, color: A.orange, marginTop: 4 }}>⚠️ Menos de 6 meses de validez</div>}
            </div>
          )}
          <div style={{ background: A.card2, borderRadius: 12, padding: "12px 14px", marginBottom: 14, border: `1px solid ${A.border}`, fontSize: 12, color: A.muted, fontFamily: BF, lineHeight: 1.6 }}>
            🔒 Tus fotos se usan exclusivamente para los trámites del viaje y están protegidas según la RGPD.
          </div>
          <BigBack />
        </div>
        {passLightbox && <Lightbox src={passLightbox} onClose={() => setPassLightbox(null)} />}
      </div>
    );
  }

  if (view === "vuelos") {
    const vuelos = trip.vuelos || []; const myDocs = (cl.personalDocs || []).filter(d => d.tipo === "vuelo");
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text, paddingBottom: 32, fontFamily: BF }}>
            ...
            <div key={i} style={{ background: A.card, border: `1px solid ${A.border}`, borderRadius: 18, padding: "20px", marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: BF, fontSize: 13, color: A.muted }}>{v.archivo}</div>
                </div>
              </div>
              {v.fileData
                ? <a href={`data:${v.mimeType};base64,${v.fileData}`} download={v.nombre||"billete"} style={{ display:"block",width:"100%",padding:"14px",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"pointer",background:`linear-gradient(90deg,${A.cyan},#0099bb)`,color:A.bg,textTransform:"uppercase",textAlign:"center",textDecoration:"none",boxSizing:"border-box" }}>↓ GUARDAR EN DISPOSITIVO</a>
                : <button style={{ width:"100%",padding:"14px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"default",background:A.card2,color:A.muted,textTransform:"uppercase" }}>PRÓXIMAMENTE</button>
              }
            </div>
          ))}
          <BigBack />
            ...
            <div key={i} style={{ background: A.card, border: `1px solid ${A.border}`, borderRadius: 18, padding: "20px", marginBottom: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: BF, fontSize: 13, color: A.muted }}>{d.archivo}</div>
                </div>
              </div>
              {d.fileData
                ? <a href={`data:${d.mimeType};base64,${d.fileData}`} download={d.nombre||"documento"} style={{ display:"block",width:"100%",padding:"14px",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"pointer",background:`linear-gradient(90deg,${A.gold},#e6a800)`,color:A.bg,textTransform:"uppercase",textAlign:"center",textDecoration:"none",boxSizing:"border-box" }}>↓ GUARDAR EN DISPOSITIVO</a>
                : <button style={{ width:"100%",padding:"14px",border:"none",borderRadius:10,fontFamily:ANTON,fontSize:14,letterSpacing:3,cursor:"default",background:A.card2,color:A.muted,textTransform:"uppercase" }}>PRÓXIMAMENTE</button>
              }
            </div>
          ))}
      ...
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text, paddingBottom: 32, fontFamily: BF }}>
        ...
        <div style={{ padding: "16px" }}>
          <div style={{ background: `linear-gradient(135deg,${A.cyan}18 0%,#172030 100%)`, borderRadius: 20, padding: "20px", marginBottom: 14, border: `1px solid ${A.cyan}33` }}>
            ...
            {fp.k === "transferencia" && (
              <div>
                <div style={{ fontFamily: ANTON, fontSize: 20, color: "#fff", letterSpacing: 2, marginBottom: 4 }}>{BANK_TITULAR}</div>
                <div style={{ fontFamily: BF, fontSize: 14, color: A.muted, letterSpacing: 1, marginBottom: 12 }}>{BANK_IBAN}</div>
                <CopyBtn text={BANK_IBAN} label="Copiar IBAN" />
              </div>
            )}
          ...
          {pc.map((p, i) => {
            const done = pe[i] === "pagado";
            const urg = !done && isUrgent(p.fechaISO);
            const ovd = !done && isOverdue(p.fechaISO);
            ...
            return (
              <div key={i} style={{ background: A.card, borderRadius: 16, padding: "16px 18px", marginBottom: 10, border: `2px solid ${done ? A.green + "44" : ovd ? A.red + "44" : urg ? A.orange + "44" : A.border}`, display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: BF, fontSize: 15, fontWeight: 700, color: done ? A.green : A.text }}>{p.label}</div>
                  <div style={{ fontSize: 12, color: A.muted, fontFamily: BF }}>{p.fecha || "Fecha por confirmar"}</div>
                </div>
        ...
        <div style={{ padding: "16px" }}>
          {hotels.map(h => (
            <div key={h.id} style={{ background: A.card, borderRadius: 20, padding: "20px", marginBottom: 14, border: `2px solid ${A.gold}55` }}>
              {h.fechasEstancia && (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: A.gold + "22", border: `1px solid ${A.gold}44`, borderRadius: 30, padding: "6px 14px", marginBottom: 14 }}>
                  <span style={{ fontSize: 14 }}>📅</span>
                  <span style={{ fontFamily: BF, fontSize: 14, fontWeight: 700, color: A.gold }}>{h.fechasEstancia}</span>
                </div>
              )}
              <div style={{ fontFamily: ANTON, fontSize: 24, color: "#fff", marginBottom: 14, textTransform: "uppercase" }}>{h.nombre}</div>
              {h.direccion && (
                <div style={{ background: A.bg, borderRadius: 12, padding: "12px 14px", border: `1px solid ${A.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 20, flexShrink: 0 }}>📍</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: BF, fontSize: 15, color: A.text, fontWeight: 700 }}>{h.direccion}</div>
                  </div>
                  <CopyBtn text={h.direccion} />
                </div>
              )}
            </div>
          ))}
...
function SurveyScreen({ cl, trip, clients, sC, trips, sT, logout }) {
  ...
  const canSubmit = cats.every(c => ratings[c.key]);

  const toggleHL = k => setHighlights(h => h.includes(k) ? h.filter(x => x !== k) : [...h, k]);

  const submit = async () => {
    if (!canSubmit) return;
    const response = { ratings, highlights, mejor, mejora, date: new Date().toLocaleDateString("es-ES") };
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div style={{ background: A.card2, borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: `1px solid ${A.border}` }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {SURVEY_HLS.map(item => {
              const sel = highlights.includes(item.k);
              return (
                <button key={item.k} onClick={() => toggleHL(item.k)} style={{ padding: "9px 14px", borderRadius: 10, cursor: "pointer", fontFamily: BF, fontSize: 13, border: `2px solid ${sel ? A.gold : A.border}`, background: sel ? A.gold + "22" : A.bg, color: sel ? A.gold : A.muted, fontWeight: sel ? 700 : 400 }}>{item.l}</button>
              );
            })}
          </div>
        </div>
  ...
  return (
    <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", fontFamily: BF, color: A.text }}>
      <div style={{ background: "linear-gradient(135deg,#07070f 0%,#0f1f3d 50%,#1a0a00 100%)", padding: "56px 28px 48px", textAlign: "center", borderBottom: `1px solid ${A.border}` }}>
        ...
        <div style={{ fontFamily: ANTON, fontSize: 42, color: "#fff", letterSpacing: 2, lineHeight: 0.9, marginBottom: 12 }}>
          ¡HASTA LA<br /><span style={{ color: A.gold }}>PRÓXIMA!</span>
        </div>
        <div style={{ fontFamily: BF, fontSize: 17, color: A.muted, lineHeight: 1.7, marginTop: 16 }}>
          Ha sido un placer viajar contigo, <strong style={{ color: "#fff" }}>{cl.nombre.split(" ")[0]}</strong>.
        </div>
      </div>
