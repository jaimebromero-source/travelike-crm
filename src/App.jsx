                            ...
                            <div style={{ padding:"14px 18px",display:"flex",flexDirection:"column",gap:12 }}>
                              {grp.map(p=>(
                                <div key={p.id} style={{ display:"flex",alignItems:"center",gap:14 }}>
                                  ...
                                  <div style={{ fontFamily:ANTON,fontSize:20,color:A.text }}>{p.nombre}</div>
                                </div>
                              ))}
                            </div>
                          </div>
              ...
              <div style={{ padding:"20px 20px 120px" }}>
                ...
                {confirmed.map((c,i)=>{
                  ...
                  return allPax.map((p,pi)=>{
                    const pbday=p.birthDate?new Date(p.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"long",year:"numeric"}):"—";
                    const pexp=p.passportExpiry?new Date(p.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"long",year:"numeric"}):"—";
                    ...
                    return (
                      <div key={p.id||c.id+"-"+pi} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${p.isMain?A.border:A.purple+"55"}`,overflow:"hidden" }}>
                        <div style={{ padding:"12px 18px",borderBottom:`1px solid ${A.border}33`,display:"flex",alignItems:"center",gap:12,background:p.isMain?"transparent":A.purple+"10" }}>
                          ...
                          <div style={{ textAlign:"right" }}>
                            <div style={{ fontFamily:ANTON,fontSize:16,color:p.docNumero?A.cyan:A.red,letterSpacing:2 }}>{p.docNumero||"SIN DOC"}</div>
                            <div style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:1,textTransform:"uppercase",marginTop:2 }}>Pasaporte</div>
                          </div>
                        </div>
                        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:1,background:A.border+"33" }}>
                          <div style={{ background:A.card,padding:"14px 18px" }}>
                            <div style={{ fontFamily:BF,fontSize:10,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Fecha nacimiento</div>
                            <div style={{ fontFamily:ANTON,fontSize:18,color:A.gold,lineHeight:1.2 }}>{pbday}</div>
                          </div>
                          <div style={{ background:A.card,padding:"14px 18px" }}>
                            <div style={{ fontFamily:BF,fontSize:10,color:pexpWarn?A.orange:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:6 }}>Caducidad doc.</div>
