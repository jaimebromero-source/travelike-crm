...
const callClaude = async (b64, mime, prompt) => {
  ...
  const contentParts = [];
  if (b64 && (isImg || isPDF)) {
    contentParts.push({ type: isImg ? "image" : "document", source: { type: "base64", media_type: mime, data: b64 } });
  }
  contentParts.push({ type: "text", text: prompt });
  const resp = await fetch("https://api.anthropic.com/v1/messages", {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-api-key": import.meta.env.VITE_CLAUDE_KEY, "anthropic-version": "2023-06-01" },
