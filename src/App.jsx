        ...
        {tc.length===0?<AEmpty text="Sin viajeros aún" />:tc.map(c=>{
          ...
          return (
            <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":A.border+"44"}` }}>
              ...
              {/* Número de documento + verificación */}
              <div style={{ marginTop:8,background:c.docNumero?A.bg:A.card2,borderRadius:10,padding:"10px 12px",border:`1px solid ${c.docVerified?A.green+"55":c.docNumero?A.cyan+"44":A.border+"44"}` }}>
                <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:c.docNumero?6:0 }}>
                  <span style={{ fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",flex:1 }}>🛂 Nº DOC</span>
                  {c.docNumero&&<span style={{ fontFamily:BF,fontSize:8,color:c.docVerified?A.green:A.orange,fontWeight:700 }}>{c.docVerified?"✓ Verificado":"⚠️ Pendiente verificar"}</span>}
                </div>
                {c.docNumero?(
                  <>
                    <input value={c.docNumero||""} onChange={e=>updClient(c.id,x=>({...x,docNumero:e.target.value.toUpperCase(),docVerified:false}))} style={{ ...ais,fontFamily:ANTON,fontSize:24,letterSpacing:5,textAlign:"center",color:A.cyan,padding:"10px 14px",marginBottom:6,background:A.bg }} />
                    <label style={{ display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"8px 10px",background:c.docVerified?A.green+"18":A.card2,borderRadius:8,border:`1px solid ${c.docVerified?A.green+"44":A.border}` }}>
                      <input type="checkbox" checked={!!c.docVerified} onChange={e=>updClient(c.id,x=>({...x,docVerified:e.target.checked}))} style={{ width:18,height:18,accentColor:A.green,flexShrink:0 }} />
                      <span style={{ fontFamily:BF,fontSize:12,color:c.docVerified?A.green:A.muted,fontWeight:700 }}>He verificado foto y nº de documento extraído por IA ✓</span>
                    </label>
                  </>
                ):(
                  <input value={""} onChange={e=>updClient(c.id,x=>({...x,docNumero:e.target.value.toUpperCase()}))} placeholder="Introducir nº pasaporte/DNI..." style={{ ...ais,fontFamily:ANTON,fontSize:18,letterSpacing:3,textAlign:"center",color:A.muted,padding:"8px 14px",background:A.bg }} />
                )}
              </div>
            </div>
          );
...
function Passport({ go, cid, clients, setClients, trips, sC, logout }) {
  ...
  const [docNum, setDocNum] = useState(cl?.docNumero || "");
        ...
        r.onload = async ev => {
          ...
          if (!expiry) {
            ...
            try {
              ...
              const raw = await callClaude(b64, mt, 'Analiza este pasaporte o DNI con máxima precisión. Extrae EXACTAMENTE: 1) el número de documento (número de pasaporte o DNI, tal como aparece en el documento, carácter por carácter) y 2) la fecha de caducidad. Responde SOLO JSON sin markdown: {"expiry":"YYYY-MM-DD","docNumero":"XXXXXXXXX"} — si no puedes leer algún campo, pon null para ese campo.');
              const parsed = JSON.parse(raw.replace(/```json|```/g, "").trim());
              if (parsed.expiry) setExpiry(parsed.expiry);
              if (parsed.docNumero) setDocNum(parsed.docNumero);
            } catch {}
            setExtracting(false);
          }
          resolve();
  ...
  const submit = async () => {
    ...
    const updated = clients.map(x => x.id === cid
      ? { ...x, passportPhoto: photos[0] || null, passportPhotos: photos, rgpdConsent: true, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false, consentDate: new Date().toISOString(), consentRGPD: c0, consentPasaporte: c1, consentFoto: c2, docNumero: docNum }
      : x
    );
    setClients(updated);
    go("notifprompt", { cid });
    ...
    <div style={{ fontFamily: BF, background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text }}>
      ...
      <div style={{ padding: "24px 20px" }}>
        <div style={{ background: A.card, borderRadius: 14, padding: 20, border: `1px solid ${A.border}`, marginBottom: 16 }}>
          ...
          <div style={{ background: A.card2, borderRadius: 12, padding: "14px", marginBottom: 16, border: `1px solid ${docNum ? A.cyan + "66" : A.border}` }}>
            <div style={{ fontFamily: BF, fontSize: 11, color: docNum ? A.cyan : A.muted, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>{docNum ? "✓ Nº documento detectado" : "Nº Pasaporte / DNI"}</div>
            <input value={docNum} onChange={e => setDocNum(e.target.value.toUpperCase())} placeholder="Ej: PAX486230 / 41433980D" style={{ ...ais, fontFamily: ANTON, fontSize: 22, letterSpacing: 4, textAlign: "center", color: A.cyan }} />
            {docNum && <div style={{ textAlign: "center", marginTop: 6, fontFamily: BF, fontSize: 10, color: A.muted }}>Verifica que el número coincide exactamente con tu documento</div>}
          </div>
