    ...
    <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF}}>
      ...
      {tab==="editor"?(
        <div style={{padding:"16px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div><div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:5}}>Importe 1er pago</div><input value={pago1} onChange={e=>setPago1(e.target.value)} placeholder="500 €" style={ais}/></div>
            <div><div style={{fontFamily:BF,fontSize:9,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:5}}>Importe 2º pago</div><input value={pago2} onChange={e=>setPago2(e.target.value)} placeholder="1.500 €" style={ais}/></div>
          </div>
          <div style={{marginBottom:10}}>
...
function AHome({ go, trips, clients, sT, sC, cfg, sCfg, logoutAdmin }) {
  ...
  if(subScreen==="voucher") return <VoucherGenerator onBack={()=>setSubScreen(null)} trips={trips} />;
  if(subScreen==="presupuesto") return <PresupuestoGenerator onBack={()=>setSubScreen(null)} trips={trips} />;
    ...
    <div style={{ background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF }}>
      ...
      <div style={{ padding:"12px 16px" }}>
        ...
        {passExpireYear.length>0&&(
          <div style={{ background:A.card,borderRadius:16,border:`2px solid ${A.red}66`,overflow:"hidden",marginBottom:10,boxShadow:`0 0 18px ${A.red}22` }}>
            <div style={{ padding:"12px 14px 10px",background:`linear-gradient(90deg,${A.red}28,${A.orange}18)`,display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${A.red}33` }}>
              <span style={{ fontSize:24 }}>🚨</span>
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:ANTON,fontSize:14,color:A.red,letterSpacing:1,textTransform:"uppercase" }}>¡Pasaportes próximos a caducar!</div>
                <div style={{ fontFamily:BF,fontSize:10,color:A.muted,marginTop:1 }}>{passExpireYear.length} viajero{passExpireYear.length!==1?"s":""} con pasaporte caduca en menos de 1 año</div>
              </div>
            </div>
            {passExpireYear.map(c=>(
              <div key={c.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"9px 14px",borderTop:`1px solid ${A.border}44` }}>
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ fontFamily:BF,fontSize:11,color:A.muted }}>Caduca: {c._dateStr}</div>
                </div>
                <div style={{ fontFamily:ANTON,fontSize:14,letterSpacing:0.5,flexShrink:0,color:c._days<0?A.red:c._days<=60?A.red:c._days<=180?A.orange:A.gold }}>
                  {c._days<0?"💀 VENCIDO":c._days===0?"💀 HOY":c._days<=90?`😱 ${c._days}d`:`⚠️ ${c._days}d`}
                </div>
              </div>
            ))}
          </div>
        ...
        {[{ icon:"👥",label:"Clientes",desc:`${totalViajeros} registrados`,color:A.green,action:()=>setSubScreen("clients") },
          ...
          { icon:"📄",label:"Vouchers",desc:"Genera documentos de reserva PDF",color:A.gold,action:()=>setSubScreen("voucher") },
...
function ClientesTab({ clients, trips, sC }) {
  ...
  return (
    <div style={{ padding:"12px 16px" }}>
      ...
      {displayed.map(c=>{
        ...
        return (
          <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`1px solid ${A.border}44` }}>
            ...
            {expandedDoc===c.id && (
              <div style={{ marginTop:10,background:A.bg,borderRadius:12,border:`1px solid ${A.cyan}33`,padding:"12px" }}>
                <div style={{ fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:2,textTransform:"uppercase",marginBottom:10,fontWeight:700 }}>📋 Datos de documento — se mantienen entre viajes</div>
                <div style={{ marginBottom:8 }}>
                  ...
                  <input
                    ...
                    onBlur={e=>updC(c.id,x=>({...x,docNumero:e.target.value.toUpperCase(),docVerified:false}))}
                    placeholder="ABC123456"
                    style={{ ...ais,fontFamily:ANTON,fontSize:20,letterSpacing:3,textAlign:"center",color:A.cyan,background:A.card }}
                  />
                </div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8 }}>
                  <div>
                    <div style={{ fontFamily:BF,fontSize:9,color:A.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:4 }}>F. Nacimiento</div>
                    <input
                      key={c.id+"_cbday"}
                      type="text" inputMode="numeric" placeholder="DD-MM-AAAA" maxLength={10}
                      defaultValue={isoToDisplay(c.birthDate||"")}
                      onChange={e=>{ const raw=e.target.value.replace(/[^0-9]/g,""); let fmt=raw; if(raw.length>2)fmt=raw.slice(0,2)+"-"+raw.slice(2); if(raw.length>4)fmt=raw.slice(0,2)+"-"+raw.slice(2,4)+"-"+raw.slice(4,8); e.target.value=fmt; }}
                      onBlur={e=>updC(c.id,x=>({...x,birthDate:displayToISO(e.target.value)}))}
                      style={{ ...ais,textAlign:"center",fontFamily:ANTON,fontSize:14,color:A.gold,background:A.card,border:`1px solid ${A.gold}44` }}
                    />
                  </div>
                  <div>
                    <div style={{ fontFamily:BF,fontSize:9,color:passportWarn(c.passportExpiry)?A.orange:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:4 }}>Caducidad</div>
                    <input
                      key={c.id+"_cexp"}
                      type="text" inputMode="numeric" placeholder="DD-MM-AAAA" maxLength={10}
                      defaultValue={isoToDisplay(c.passportExpiry||"")}
                      onChange={e=>{ const raw=e.target.value.replace(/[^0-9]/g,""); let fmt=raw; if(raw.length>2)fmt=raw.slice(0,2)+"-"+raw.slice(2); if(raw.length>4)fmt=raw.slice(0,2)+"-"+raw.slice(2,4)+"-"+raw.slice(4,8); e.target.value=fmt; }}
                      onBlur={e=>updC(c.id,x=>({...x,passportExpiry:displayToISO(e.target.value),passportExpiryDismissed:false}))}
                ...
                {c.docNumero&&<div style={{ background:A.green+"10",border:`1px solid ${A.green}22`,borderRadius:8,padding:"6px 10px",fontFamily:BF,fontSize:10,color:A.green }}>✓ Datos guardados — se pre-rellenan en el siguiente viaje</div>}
              </div>
            )}
          </div>
        );
      ...
      {modal && (
        <AModal title="Nuevo cliente" onClose={()=>{ setModal(false); setPersonas([{nombre:"",email:""}]); }}>
          <div style={{ fontSize:13,color:A.muted,fontFamily:BF,marginBottom:14 }}>Puedes registrar una persona o un grupo que viajará junto.</div>
          {personas.map((p,i)=>(
            <div key={i} style={{ background:A.bg,borderRadius:12,padding:"12px",marginBottom:8,border:`1px solid ${i===0?A.cyan+"44":A.purple+"44"}` }}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                <div style={{ fontFamily:BF,fontSize:10,color:i===0?A.cyan:A.purple,letterSpacing:2,textTransform:"uppercase",fontWeight:700 }}>{i===0?"Principal":`Persona ${i+1}`}</div>
                {i>0&&<button onClick={()=>setPersonas(ps=>ps.filter((_,j)=>j!==i))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer",lineHeight:1 }}>✕</button>}
              </div>
              <input value={p.nombre} onChange={e=>setPersonas(ps=>ps.map((x,j)=>j===i?{...x,nombre:e.target.value}:x))} placeholder="Nombre completo" style={{ ...ais,marginBottom:8 }} />
              <input value={p.email} onChange={e=>setPersonas(ps=>ps.map((x,j)=>j===i?{...x,email:e.target.value}:x))} placeholder="Email (opcional)" type="email" style={ais} />
            </div>
          ))}
          <button onClick={()=>setPersonas(ps=>[...ps,{nombre:"",email:""}])} style={{ ...ab(A.purple+"22",A.purple),width:"100%",border:`1.5px dashed ${A.purple}`,borderRadius:10,marginBottom:14,fontFamily:BF }}>+ Añadir otra persona al grupo</button>
          <ARow>
            <button onClick={()=>{ setModal(false); setPersonas([{nombre:"",email:""}]); }} style={{ ...ab(A.card2,A.muted),flex:1,border:`1px solid ${A.border}` }}>Cancelar</button>
            <button onClick={addClients} style={{ ...ab(A.cyan,A.bg),flex:2 }}>{personas.filter(p=>p.nombre.trim()).length>1?`Registrar grupo (${personas.filter(p=>p.nombre.trim()).length})`:"Registrar"}</button>
          </ARow>
        </AModal>
      )}
    </div>
...
function ATrip({ go, tid, initTab, trips, clients, crm, sT, sC, sCrm }) {
  ...
  const PeopleTab=()=>{
    ...
    const buildFlightList=()=>{
      if(!confirmed.length) return "Sin confirmados";
      const lines=[`✈️ FLIGHT LIST — ${trip.name.toUpperCase()}`,`${fmt(trip.date)}`,"","Nombre | Pasaporte | F.Nacimiento | Caducidad","---"];
      const seen=new Set();
      confirmed.forEach((c,i)=>{
        if(seen.has(c.id)) return; seen.add(c.id);
        const bday=c.birthDate?new Date(c.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
        const exp=c.passportExpiry?new Date(c.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
        lines.push(`${lines.length-4}. ${c.nombre} | ${c.docNumero||"—"} | ${bday} | ${exp}`);
        // Compañeros de grupo que también están en este viaje
        if(c.groupId){
          confirmed.filter(x=>x.groupId===c.groupId&&x.id!==c.id).forEach(gm=>{
            if(seen.has(gm.id)) return; seen.add(gm.id);
            const gbday=gm.birthDate?new Date(gm.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
            const gexp=gm.passportExpiry?new Date(gm.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
            lines.push(`   + ${gm.nombre} | ${gm.docNumero||"—"} | ${gbday} | ${gexp}`);
          });
        }
        // Acompañantes legacy
        (c.acompanantes||[]).forEach(a=>{
          const abday=a.birthDate?new Date(a.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
          const aexp=a.passportExpiry?new Date(a.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
    ...
    return (
      <div style={{ padding:"0 16px" }}>
          ...
          return (
            <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":c.groupId?A.purple+"44":A.border+"44"}` }}>
              ...
              {c.groupId && (()=>{
                ...
                return gMembers.length>0?(<div style={{ background:A.purple+"15",border:`1px solid ${A.purple}33`,borderRadius:8,padding:"5px 10px",marginBottom:8,display:"flex",alignItems:"center",gap:8 }}><span style={{ fontSize:12 }}>👥</span><span style={{ fontFamily:BF,fontSize:11,color:A.purple,fontWeight:700 }}>Grupo con: {gMembers.map(x=>x.nombre.split(" ")[0]).join(", ")}</span></div>):null;
              })()}
              <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
                ...
                <button onClick={()=>{ if(window.confirm(`¿Sacar a ${c.nombre} de este viaje? Se quedará en tus clientes globales.`)) sC(clients.map(x=>x.id===c.id?{...x,tripId:null,roommateId:null}:x)); }} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer" }} title="Sacar del viaje">✕</button>
              </div>
                    ...
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                      <div>
                        ...
                        <input
                          ...
                          defaultValue={isoToDisplay(c.birthDate||"")}
                          onChange={e=>{ const raw=e.target.value.replace(/[^0-9]/g,""); let fmt=raw; if(raw.length>2) fmt=raw.slice(0,2)+"-"+raw.slice(2); if(raw.length>4) fmt=raw.slice(0,2)+"-"+raw.slice(2,4)+"-"+raw.slice(4,8); e.target.value=fmt; }}
                          onBlur={e=>updClient(c.id,x=>({...x,birthDate:displayToISO(e.target.value)}))}
                          maxLength={10}
                          style={{ ...ais,textAlign:"center",fontFamily:ANTON,fontSize:13,color:A.gold,background:A.card,border:`1px solid ${A.gold}44` }}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          maxLength={10}
                          style={{ ...ais,textAlign:"center",fontFamily:ANTON,fontSize:13,color:passportWarn(c.passportExpiry)?A.orange:A.text,background:A.card,border:`1px solid ${passportWarn(c.passportExpiry)?A.orange+"44":A.border+"44"}` }}
                        />
                      </div>
                    </div>
                  </div>
        ...
        {passModal && (
          <PassportModal
            ...
            onSave={(docNum, expiry, birthDate) => {
              updClient(passModal.cid, c => ({ ...c, docNumero: docNum, passportExpiry: displayToISO(expiry), birthDate: displayToISO(birthDate), passportExpiryDismissed: false, docVerified: false }));
              setPassModal(null);
            }}
        ...
        {listModal && (
          <div style={{ position:"fixed",inset:0,zIndex:2000,background:A.bg,display:"flex",flexDirection:"column",overflowY:"auto" }}>
            {/* HEADER */}
            <div style={{ position:"sticky",top:0,zIndex:10,background:A.bg,borderBottom:`1px solid ${A.border}`,padding:"14px 20px 0" }}>
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:14 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:ANTON,fontSize:20,color:A.text,letterSpacing:1 }}>{trip.flag} {trip.name.toUpperCase()}</div>
                  <div style={{ fontFamily:BF,fontSize:11,color:A.muted }}>{fmt(trip.date)}</div>
                </div>
              ...
              <div style={{ display:"flex",gap:6,paddingBottom:0 }}>
                {[["confirmados","🛏","Rooming",A.green,confirmed.length],["vuelo","✈️","Flight List",A.cyan,null],["interesados","👀","Interesados",A.orange,interesados.length]].map(([key,ic,lbl,col,cnt])=>(
                  <button key={key} onClick={()=>{ setListView(key); setCopied2(false); }} style={{ flex:1,background:"transparent",border:"none",borderBottom:`3px solid ${listView===key?col:"transparent"}`,color:listView===key?col:A.muted,padding:"8px 4px 10px",fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s" }}>
            ...
            {listView==="confirmados" && (
              <div style={{ padding:"20px 20px 120px" }}>
                <div style={{ fontFamily:BF,fontSize:12,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:18 }}>{confirmed.length} confirmados · {Object.entries(ROOMS).filter(([k])=>{ const byRoom={}; Object.keys(ROOMS).forEach(r=>byRoom[r]=[]); confirmed.forEach(c=>{ const r=c.room||"doble_jun"; if(byRoom[r]) byRoom[r].push(c); }); return byRoom[k]?.length>0; }).map(([k,r])=>{ const byRoom={}; Object.keys(ROOMS).forEach(rk=>byRoom[rk]=[]); confirmed.forEach(c=>{ const rr=c.room||"doble_jun"; if(byRoom[rr]) byRoom[rr].push(c); }); const n=byRoom[k].length; const habs=k==="doble_jun"||k==="doble_sep"?Math.ceil(n/2):k==="triple"?Math.ceil(n/3):k==="cuadruple"?Math.ceil(n/4):n; return `${habs} ${r.short}`; }).join(" · ")}</div>
                {(()=>{
                  const byRoom={};
                  ...
                  ["doble_jun","doble_sep"].forEach(rk=>{
                    ...
                    const roomInfo=ROOMS[rk];
                    rows.push(
                      <div key={rk+"_hdr"} style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12,marginTop:rows.length?24:0 }}>
                        <div style={{ width:4,height:22,background:roomInfo.color,borderRadius:2 }} />
                        <div style={{ fontFamily:ANTON,fontSize:15,color:roomInfo.color,letterSpacing:2,textTransform:"uppercase" }}>{roomInfo.label}</div>
                        <div style={{ fontFamily:BF,fontSize:12,color:A.muted }}>{Math.ceil(byRoom[rk].length/2)} habitaciones</div>
                      </div>
                    );
                    const seen=new Set();
                    byRoom[rk].forEach(c=>{
                      if(seen.has(c.id)) return; seen.add(c.id);
                      const partner=c.roommateId?byRoom[rk].find(x=>x.id===c.roommateId):null;
                      if(partner&&!seen.has(partner.id)) seen.add(partner.id);
                      const habNum=hab++;
                      rows.push(
                        <div key={c.id} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${roomInfo.color}33`,overflow:"hidden" }}>
                          <div style={{ background:`linear-gradient(90deg,${roomInfo.color}18,transparent)`,padding:"10px 18px",borderBottom:`1px solid ${roomInfo.color}22` }}>
                            <span style={{ fontFamily:ANTON,fontSize:13,color:roomInfo.color,letterSpacing:2 }}>HAB {habNum}</span>
                          </div>
                          <div style={{ padding:"14px 18px",display:"flex",flexDirection:"column",gap:12 }}>
                            {[c,...(partner?[partner]:[])].map((p,pi)=>(
                              <div key={p.id} style={{ display:"flex",alignItems:"center",gap:14 }}>
                                <div style={{ width:46,height:46,borderRadius:12,overflow:"hidden",flexShrink:0,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:20,color:A.cyan,border:`2px solid ${pi===0?roomInfo.color:A.border}44` }}>
                                  {p.passportPhoto?<img src={p.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:p.nombre[0]}
                                </div>
                                <div style={{ flex:1 }}>
                                  <div style={{ fontFamily:ANTON,fontSize:20,color:A.text,lineHeight:1.1 }}>{p.nombre}</div>
                                  {pi===1&&<div style={{ fontFamily:BF,fontSize:11,color:A.muted,marginTop:2 }}>Compañero/a</div>}
                                </div>
                  ...
                  ["individual","triple","cuadruple","busca"].forEach(rk=>{
                    ...
                    rows.push(
                      <div key={rk+"_hdr"} style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12,marginTop:rows.length?24:0 }}>
                        <div style={{ width:4,height:22,background:roomInfo.color,borderRadius:2 }} />
                        <div style={{ fontFamily:ANTON,fontSize:15,color:roomInfo.color,letterSpacing:2,textTransform:"uppercase" }}>{roomInfo.label}</div>
                        <div style={{ fontFamily:BF,fontSize:12,color:A.muted }}>{byRoom[rk].length} viajeros</div>
                      </div>
                    );
                    if(rk==="triple"||rk==="cuadruple"){
                      const cap=rk==="triple"?3:4;
                      const groups=[]; let g=[];
                      byRoom[rk].forEach(c=>{ g.push(c); if(g.length===cap){groups.push([...g]);g=[];} });
                      if(g.length) groups.push(g);
                      groups.forEach((grp,gi)=>{
                        rows.push(
                          <div key={rk+gi} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${roomInfo.color}33`,overflow:"hidden" }}>
                            <div style={{ background:`linear-gradient(90deg,${roomInfo.color}18,transparent)`,padding:"10px 18px",borderBottom:`1px solid ${roomInfo.color}22` }}>
                              <span style={{ fontFamily:ANTON,fontSize:13,color:roomInfo.color,letterSpacing:2 }}>HAB {hab++}</span>
                            </div>
                            <div style={{ padding:"14px 18px",display:"flex",flexDirection:"column",gap:12 }}>
                              {grp.map(p=>(
                                <div key={p.id} style={{ display:"flex",alignItems:"center",gap:14 }}>
                                  <div style={{ width:46,height:46,borderRadius:12,overflow:"hidden",flexShrink:0,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:20,color:A.cyan }}>
                                    {p.passportPhoto?<img src={p.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:p.nombre[0]}
                                  </div>
                                  <div style={{ fontFamily:ANTON,fontSize:20,color:A.text,flex:1 }}>{p.nombre}</div>
                                  <CopyBtn text={p.nombre} />
                                </div>
                              ))}
                            </div>
                      ...
                      byRoom[rk].forEach(c=>{
                        rows.push(
                          <div key={c.id} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${roomInfo.color}33`,padding:"16px 18px",display:"flex",alignItems:"center",gap:14 }}>
                            <div style={{ width:46,height:46,borderRadius:12,overflow:"hidden",flexShrink:0,background:A.cyan+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:20,color:A.cyan }}>
                              {c.passportPhoto?<img src={c.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:c.nombre[0]}
                            </div>
                            <div style={{ fontFamily:ANTON,fontSize:20,color:A.text,flex:1 }}>{c.nombre}</div>
                            <CopyBtn text={c.nombre} />
                            <span style={{ fontFamily:BF,fontSize:11,color:roomInfo.color,background:roomInfo.color+"18",padding:"4px 10px",borderRadius:8 }}>{roomInfo.short}</span>
                          </div>
                        );
                      });
                    }
            ...
            {listView==="vuelo" && (
              <div style={{ padding:"20px 20px 120px" }}>
                {(()=>{
                  ...
                  confirmed.forEach((c)=>{
                    if(seen.has(c.id)) return; seen.add(c.id);
                    // Grupo: este viajero + sus compañeros de grupo en el mismo viaje
                    const groupPax=[c,...(c.groupId?confirmed.filter(x=>x.groupId===c.groupId&&x.id!==c.id&&!seen.has(x.id)):[])]
                      .concat((c.acompanantes||[]).map(a=>({...a,isLegacyAcomp:true,isMain:false})));
                    groupPax.forEach(x=>{ if(x.id&&x.id!==c.id) seen.add(x.id); });
                    groupPax.forEach((p,pi)=>{
                      const pbday=p.birthDate?new Date(p.birthDate+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
                      const pexp=p.passportExpiry?new Date(p.passportExpiry+"T12:00:00").toLocaleDateString("es-ES",{day:"2-digit",month:"2-digit",year:"numeric"}):"—";
                      const pexpWarn=passportWarn(p.passportExpiry);
                      const isGroup=pi>0&&!p.isLegacyAcomp;
                      rows.push(
                        <div key={p.id||c.id+"-"+pi} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${pi===0?A.border:isGroup?A.purple+"55":A.purple+"55"}`,overflow:"hidden" }}>
                          <div style={{ padding:"12px 18px",borderBottom:`1px solid ${A.border}33`,display:"flex",alignItems:"center",gap:12,background:pi===0?"transparent":A.purple+"10" }}>
                            <div style={{ fontFamily:ANTON,fontSize:28,color:A.cyan,lineHeight:1,width:36,textAlign:"center" }}>{pi===0?num++:"+"}</div>
                            <div style={{ flex:1 }}>
                              <div style={{ fontFamily:ANTON,fontSize:22,color:A.text,lineHeight:1.1 }}>{p.nombre}</div>
                              {pi>0&&<div style={{ fontFamily:BF,fontSize:11,color:A.purple,marginTop:3 }}>{isGroup?"Grupo":"+ acompañante"} de {c.nombre.split(" ")[0]}</div>}
                            </div>
                            <CopyBtn text={p.nombre} />
                          </div>
                          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:1,background:A.border+"33" }}>
                            <div style={{ background:A.card,padding:"14px 12px" }}>
                              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                                <div style={{ fontFamily:BF,fontSize:9,color:A.gold,letterSpacing:2,textTransform:"uppercase" }}>Nacimiento</div>
                                <CopyBtn text={pbday} />
                              </div>
                              <div style={{ fontFamily:ANTON,fontSize:18,color:A.gold,lineHeight:1.2 }}>{pbday}</div>
                            </div>
                            <div style={{ background:A.card,padding:"14px 12px" }}>
                              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                                <div style={{ fontFamily:BF,fontSize:9,color:pexpWarn?A.orange:A.muted,letterSpacing:2,textTransform:"uppercase" }}>Caducidad</div>
                                <CopyBtn text={pexp} />
                              </div>
                              <div style={{ fontFamily:ANTON,fontSize:18,color:pexpWarn?A.orange:A.text,lineHeight:1.2 }}>{pexp}{pexpWarn?" ⚠️":""}</div>
                            </div>
                            <div style={{ background:A.card,padding:"14px 12px" }}>
                              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
                                <div style={{ fontFamily:BF,fontSize:9,color:p.docNumero?A.cyan:A.red,letterSpacing:2,textTransform:"uppercase" }}>Pasaporte</div>
                                {p.docNumero&&<CopyBtn text={p.docNumero} />}
                              </div>
                              <div style={{ fontFamily:ANTON,fontSize:18,color:p.docNumero?A.cyan:A.red,lineHeight:1.2,wordBreak:"break-all" }}>{p.docNumero||"—"}</div>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  return (<><div style={{ fontFamily:BF,fontSize:12,color:A.muted,letterSpacing:2,textTransform:"uppercase",marginBottom:18 }}>{rows.length} pasajeros totales</div>{rows}</>);
                })()}
              </div>
            )}
            ...
            {listView==="interesados" && (
              <div style={{ padding:"20px 20px 120px" }}>
                ...
                {interesados.map((c,i)=>{
                  const rm=ROOMS[c.room||"doble_jun"]||ROOMS.doble_jun;
                  return (
                    <div key={c.id} style={{ background:A.card,borderRadius:16,marginBottom:10,border:`1.5px solid ${A.orange}33`,padding:"16px 18px",display:"flex",alignItems:"center",gap:14 }}>
                      <div style={{ fontFamily:ANTON,fontSize:24,color:A.orange,width:36,textAlign:"center",lineHeight:1 }}>{i+1}</div>
                      <div style={{ width:46,height:46,borderRadius:12,overflow:"hidden",flexShrink:0,background:A.orange+"22",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:20,color:A.orange }}>
                        {c.passportPhoto?<img src={c.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:c.nombre[0]}
                      </div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:ANTON,fontSize:20,color:A.text,lineHeight:1.1 }}>{c.nombre}</div>
                        <div style={{ fontFamily:BF,fontSize:12,color:rm.color,marginTop:3 }}>{rm.label}</div>
                      </div>
                    </div>
                  );
                })}
        ...
        {addModal==="new" && <AModal title="Nuevo viajero" onClose={()=>{ setAddModal(null); setNewPersonas([{nombre:"",email:""}]); }}>
          <div style={{ fontSize:13,color:A.muted,fontFamily:BF,marginBottom:14 }}>Añade una o varias personas. Si añades más de una, viajarán juntas con el mismo código de grupo.</div>
          {newPersonas.map((p,i)=>(
            <div key={i} style={{ background:A.bg,borderRadius:12,padding:"12px",marginBottom:8,border:`1px solid ${i===0?A.cyan+"44":A.purple+"44"}` }}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                <div style={{ fontFamily:BF,fontSize:10,color:i===0?A.cyan:A.purple,letterSpacing:2,textTransform:"uppercase",fontWeight:700 }}>
                  {i===0?"Persona principal":`Persona ${i+1}`}
                </div>
                {i>0&&<button onClick={()=>setNewPersonas(ps=>ps.filter((_,j)=>j!==i))} style={{ background:"transparent",border:"none",color:A.muted,fontSize:18,cursor:"pointer",lineHeight:1 }}>✕</button>}
              </div>
              <input value={p.nombre} onChange={e=>setNewPersonas(ps=>ps.map((x,j)=>j===i?{...x,nombre:e.target.value}:x))} placeholder="Nombre completo" style={{ ...ais,marginBottom:8 }} />
              <input value={p.email} onChange={e=>setNewPersonas(ps=>ps.map((x,j)=>j===i?{...x,email:e.target.value}:x))} placeholder="Email (opcional)" type="email" style={{ ...ais }} />
            </div>
          ))}
          <button onClick={()=>setNewPersonas(ps=>[...ps,{nombre:"",email:""}])} style={{ ...ab(A.purple+"22",A.purple),width:"100%",border:`1.5px dashed ${A.purple}`,borderRadius:10,marginBottom:14,fontFamily:BF }}>+ Añadir otra persona al grupo</button>
          <ARow>
            ...
            <button onClick={()=>{
              ...
              if(!valid.length) return;
              const groupId=valid.length>1?`g${uid()}`:null;
              const nuevos=valid.map(p=>({ id:`cl${uid()}`,nombre:p.nombre.trim(),email:p.email||"",tripId:tid,code:genCode(),status:"interesado",room:"doble_jun",note:"",pagosEstado:(trip.pagosConfig||[]).map(()=>"pendiente"),pagosImporteCustom:(trip.pagosConfig||[]).map(()=>null),personalDocs:[],passportPhoto:null,passportConsent:false,photoConsent:false,firstLogin:true,passportExpiry:"",passportExpiryDismissed:false,notifEnabled:false,roommateId:null,surveySubmitted:false,acompanantes:[],maletaPersonal:[],maletaMarcados:[],formaPago:"transferencia",docNumero:"",docVerified:false,groupId }));
              sC([...clients,...nuevos]);
              setAddModal(null); setNewPersonas([{nombre:"",email:""}]);
            }} style={{ ...ab(A.cyan,A.bg),flex:2 }}>{newPersonas.filter(p=>p.nombre.trim()).length>1?`Añadir grupo (${newPersonas.filter(p=>p.nombre.trim()).length})`:newPersonas[0]?.nombre.trim()?"Añadir":"Añadir"}</button>
          </ARow>
        </AModal>}
  ...
  const AIDocsTab=()=>{
    ...
    const DOC_TIPOS=[
      { k:"vuelo_ida",   icon:"✈️",  label:"Billete IDA" },
      { k:"vuelo_vuelta",icon:"🔄",  label:"Billete VUELTA" },
      { k:"seguro",      icon:"🛡️",  label:"Seguro de viaje" },
      { k:"voucher",     icon:"🏨",  label:"Voucher hotel" },
      { k:"visado",      icon:"🛂",  label:"Visado" },
      { k:"hotel",       icon:"🏩",  label:"Reserva hotel" },
      { k:"otro",        icon:"📄",  label:"Otro documento" },
    ];
    ...
    const assignManual=async()=>{
      ...
      const ext=manFile.type?.includes("pdf")?"pdf":manFile.type?.includes("png")?"png":"jpg";
      const nombre=`${manTarget.nombre.split(" ")[0]}_${tipoInfo.label.replace(/ /g,"_")}.${ext}`;
      const doc={ id:uid(),tipo:manTipo.startsWith("vuelo")?"vuelo":manTipo,nombre,archivo:manDesc||manFile.name,descripcion:manDesc||tipoInfo.label,data:b64,mimeType:manFile.type };
      if(manTarget.type==="acomp"){ updClient(manTarget.clientId,c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===manTarget.id?{...a,personalDocs:[...(a.personalDocs||[]),doc]}:a)})); }
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        <div style={{ display:"flex",background:A.bg,borderRadius:12,padding:4,marginBottom:14,gap:4,border:`1px solid ${A.border}` }}>
          <button onClick={()=>setDocsMode("ia")} style={{ flex:1,background:docsMode==="ia"?A.purple:"transparent",color:docsMode==="ia"?"#fff":A.muted,border:"none",borderRadius:9,padding:"10px",fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s" }}>🤖 IA Automático</button>
          <button onClick={()=>setDocsMode("manual")} style={{ flex:1,background:docsMode==="manual"?A.cyan:"transparent",color:docsMode==="manual"?"#000":A.muted,border:"none",borderRadius:9,padding:"10px",fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer",transition:"all .15s" }}>📤 Subida Manual</button>
        </div>
        ...
        {/* ── MANUAL MODE ── */}
        {docsMode==="manual"&&(<>
          <div style={{ background:A.cyan+"12",border:`1px solid ${A.cyan}33`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontFamily:BF,fontSize:12,color:A.cyan }}>
            📤 Sube documentos manualmente y asígnalos a cada viajero
          </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14 }}>
            {DOC_TIPOS.map(d=>(
              <button key={d.k} onClick={()=>setManTipo(d.k)} style={{ background:manTipo===d.k?A.cyan+"22":A.card,border:`2px solid ${manTipo===d.k?A.cyan:A.border}`,borderRadius:10,padding:"10px 8px",display:"flex",alignItems:"center",gap:8,cursor:"pointer",textAlign:"left" }}>
                <span style={{ fontSize:20 }}>{d.icon}</span>
                <span style={{ fontFamily:BF,fontSize:12,fontWeight:700,color:manTipo===d.k?A.cyan:A.text }}>{d.label}</span>
              </button>
            ))}
          </div>
          ...
          <button onClick={()=>manFileRef.current.click()} style={{ width:"100%",background:manFile?A.green+"18":A.card2,border:`1.5px dashed ${manFile?A.green:A.border}`,borderRadius:10,padding:"14px",color:manFile?A.green:A.muted,fontFamily:BF,fontSize:13,cursor:"pointer",fontWeight:700,marginBottom:12,textAlign:"center" }}>
            {manFile?`📎 ${manFile.name}`:"📁 Seleccionar archivo (PDF o imagen)"}
          </button>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:6,marginBottom:16 }}>
            {allTargets.map(target=>{
              ...
              return (
                <button key={target.id} onClick={()=>setManTarget(sel?null:target)} style={{ background:sel?A.cyan+"22":A.card,border:`2px solid ${sel?A.cyan:target.type==="acomp"?A.purple+"55":A.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",textAlign:"left" }}>
                  <div style={{ width:38,height:38,borderRadius:9,overflow:"hidden",flexShrink:0,background:sel?A.cyan+"33":A.card2,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:ANTON,fontSize:17,color:sel?A.cyan:A.muted }}>
                    {(()=>{ const cl=target.type==="client"?tc.find(c=>c.id===target.id):tc.flatMap(c=>c.acompanantes||[]).find(a=>a.id===target.id); return cl?.passportPhoto?<img src={cl.passportPhoto} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:target.nombre[0]; })()}
                  </div>
                  <div style={{ flex:1 }}>
                    <div style={{ fontFamily:BF,fontSize:14,fontWeight:700,color:sel?A.cyan:A.text }}>{target.nombre}</div>
                    {target.type==="acomp"&&<div style={{ fontFamily:BF,fontSize:10,color:A.purple }}>+ acompañante</div>}
                  </div>
                  {sel&&<span style={{ color:A.cyan,fontSize:20 }}>✓</span>}
                </button>
              );
            })}
          </div>
          ...
          <button onClick={assignManual} disabled={!manFile||!manTarget||manAssigning} style={{ width:"100%",background:(!manFile||!manTarget)?A.card2:`linear-gradient(90deg,${A.cyan},${A.green})`,border:"none",borderRadius:12,padding:"16px",color:(!manFile||!manTarget)?A.muted:"#000",fontFamily:ANTON,fontSize:16,letterSpacing:1,cursor:(!manFile||!manTarget)?"not-allowed":"pointer",marginBottom:20 }}>
            {manAssigning?"⏳ ASIGNANDO...":"📤 ASIGNAR DOCUMENTO"}
          </button>

          {/* Docs ya asignados en este viaje */}
          {tc.some(c=>(c.personalDocs||[]).length>0||(c.acompanantes||[]).some(a=>(a.personalDocs||[]).length>0))&&(
            <div>
              ...
              {tc.flatMap(c=>[
                ...(c.personalDocs||[]).map(d=>({...d,_nombre:c.nombre,_cid:c.id,_isAcomp:false})),
                ...(c.acompanantes||[]).flatMap(a=>(a.personalDocs||[]).map(d=>({...d,_nombre:a.nombre,_cid:c.id,_aid:a.id,_isAcomp:true})))
              ]).map(d=>{
                const tipoInfo=DOC_TIPOS.find(t=>d.tipo&&(t.k===d.tipo||t.k.startsWith(d.tipo)))||DOC_TIPOS[DOC_TIPOS.length-1];
                return (
                  <div key={d.id} style={{ background:A.card,borderRadius:10,padding:"10px 14px",marginBottom:8,border:`1px solid ${A.cyan}22`,display:"flex",alignItems:"center",gap:10 }}>
                    <span style={{ fontSize:20 }}>{tipoInfo.icon}</span>
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontFamily:BF,fontSize:13,fontWeight:700,color:A.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{d.nombre||d.archivo}</div>
                      <div style={{ fontFamily:BF,fontSize:10,color:A.muted }}>{d._nombre}{d._isAcomp&&" (acomp.)"} · {tipoInfo.label}</div>
                    </div>
                    <button onClick={()=>{
                      if(d._isAcomp){ updClient(d._cid,c=>({...c,acompanantes:(c.acompanantes||[]).map(a=>a.id===d._aid?{...a,personalDocs:(a.personalDocs||[]).filter(x=>x.id!==d.id)}:a)})); }
                      else{ updClient(d._cid,c=>({...c,personalDocs:(c.personalDocs||[]).filter(x=>x.id!==d.id)})); }
                    }} style={{ background:"transparent",border:"none",color:A.muted,fontSize:16,cursor:"pointer",flexShrink:0 }}>✕</button>
                  </div>
                );
              })}
            </div>
  ...
  const EditTab=()=>{
    ...
    if(sec===null){
      return (
        <div style={{ padding:"12px 16px" }}>
          <button onClick={()=>{ setTab("menu"); setEditSec(null); }} style={{ background:"transparent",border:"none",color:A.cyan,fontSize:14,fontFamily:BF,cursor:"pointer",marginBottom:12 }}>← Volver al menú</button>
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {sec==="emerg"&&<div>
          {[{k:"policia",l:"Policía"},{k:"ambulancia",l:"Ambulancia"},{k:"bomberos",l:"Bomberos"},{k:"tourleader",l:"Tour Leader"}].map(item=>(
            <div key={item.k}>
    ...
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", fontFamily: BF, color: A.text, paddingBottom: 32 }}>
        ...
        <div style={{ padding: "20px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          {/* Grupo: pasaportes side-by-side si hay más de un miembro */}
          {groupMembers.length > 1 && (
            <div onClick={() => setView("pasaportes_grupo")} style={{ gridColumn: "1 / -1", background: A.card, borderRadius: 20, padding: "16px", border: `1.5px solid ${A.purple}44`, cursor: "pointer", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: A.purple+"10", pointerEvents: "none" }} />
              <div style={{ fontFamily: BF, fontSize: 10, color: A.purple, letterSpacing: 3, textTransform: "uppercase", marginBottom: 12, fontWeight: 700 }}>🛂 Pasaportes del grupo</div>
  ...
  if (view === "pasaportes_grupo") {
    return (
      <div style={{ background: A.bg, minHeight: "100vh", maxWidth: 560, margin: "0 auto", color: A.text, paddingBottom: 40, fontFamily: BF }}>
        ...
        <div style={{ padding: "16px" }}>
          <div style={{ fontFamily: BF, fontSize: 12, color: A.muted, marginBottom: 16, lineHeight: 1.6 }}>
            Aquí puedes ver el estado de los pasaportes de vuestro grupo. Cada persona debe subir el suyo desde su propio acceso.
          </div>
          {groupMembers.map((m, i) => {
            const isMe = m.id === cid;
            ...
            return (
              <div key={m.id} style={{ background: A.card, borderRadius: 20, padding: "16px", marginBottom: 14, border: `2px solid ${isMe ? A.cyan + "66" : A.purple + "44"}`, position: "relative" }}>
                {isMe && <div style={{ position: "absolute", top: 10, right: 12, background: A.cyan + "22", border: `1px solid ${A.cyan}44`, borderRadius: 20, padding: "3px 10px", fontFamily: BF, fontSize: 9, color: A.cyan, fontWeight: 700, letterSpacing: 1 }}>TÚ</div>}
                <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  {/* Foto pasaporte en grande */}
                  <div style={{ width: 90, height: 120, borderRadius: 14, overflow: "hidden", flexShrink: 0, border: `3px solid ${m.passportPhoto ? A.cyan + "88" : A.border}`, background: A.card2, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                    {m.passportPhoto
                      ? <img src={m.passportPhoto} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                      : <><div style={{ fontSize: 32, marginBottom: 6 }}>🛂</div><div style={{ fontFamily: BF, fontSize: 9, color: A.muted, textAlign: "center", lineHeight: 1.3 }}>Sin foto</div></>
                    }
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: ANTON, fontSize: 20, color: isMe ? A.cyan : A.text, letterSpacing: 0.5, marginBottom: 4 }}>{m.nombre}</div>
                    {m.passportPhoto
                      ? <div style={{ background: A.green + "18", border: `1px solid ${A.green}33`, borderRadius: 8, padding: "5px 10px", marginBottom: 8, display: "inline-flex", alignItems: "center", gap: 6 }}>
                          <span style={{ color: A.green, fontSize: 14 }}>✓</span>
                          <span style={{ fontFamily: BF, fontSize: 12, color: A.green, fontWeight: 700 }}>Pasaporte subido</span>
                        </div>
                      : <div style={{ background: A.orange + "18", border: `1px solid ${A.orange}33`, borderRadius: 8, padding: "5px 10px", marginBottom: 8, display: "inline-flex", alignItems: "center", gap: 6 }}>
                          <span style={{ color: A.orange, fontSize: 14 }}>⚠️</span>
                          <span style={{ fontFamily: BF, fontSize: 12, color: A.orange, fontWeight: 700 }}>Pendiente de subir</span>
                        </div>
                    }
