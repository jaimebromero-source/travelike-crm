...
function PassportImageViewer({ src }) {
  ...
  // Drag handlers — solo en la imagen, sin preventDefault para no robar foco a inputs
  const onImgDown = e => {
    if (!isZ) return;
    setDragging(true);
  ...
  // Rueda: solo cambia zoom cuando NO hay zoom activo; con zoom → deja que el scroll del modal fluya
  const onImgWheel = e => {
    if (!isZ) {
      e.stopPropagation();
      const n = Math.min(5, Math.max(1, zoom + (e.deltaY < 0 ? 0.25 : -0.25)));
      setZoom(n);
    }
    // si isZ → no hacemos nada, el evento llega al contenedor padre y hace scroll
  };
  ...
  return (
    <div style={{ background: "#000", flexShrink: 0 }}>
      ...
      {/* Área de imagen */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: isZ ? "0" : "12px", minHeight: 220, maxHeight: "52vh", overflow: "hidden", userSelect: "none" }}>
        <img
          src={src}
          draggable={false}
      ...
      {isZ && <div style={{ textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.3)", padding: "4px 0 6px", fontFamily: "sans-serif" }}>Arrastra para mover · rueda para scroll</div>}
    </div>
  );
}
          ...
          return (
            <div key={c.id} style={{ background:A.card,borderRadius:14,padding:"12px 14px",marginBottom:10,border:`2px solid ${urgent?A.orange+"66":A.border+"44"}` }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
                <div onClick={()=>c.passportPhoto?(setPassModal({src:c.passportPhoto,cid:c.id,name:c.nombre}),setPassDocNum(c.docNumero||""),setPassExpiry(isoToDisplay(c.passportExpiry||""))):(setUpIdx(c.id),passRef.current.value="",passRef.current.click())}
          ...
          <div style={{ position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:9999,background:"#000",display:"flex",flexDirection:"column",overflowY:"auto" }}>
            ...
            <div style={{ background:"#0a0a12",borderTop:`2px solid ${A.cyan}33`,padding:"16px",flexShrink:0 }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:8 }}>
                <button
                  onClick={()=>{ updClient(passModal.cid,c=>({...c,docNumero:passDocNum,passportExpiry:displayToISO(passExpiry),passportExpiryDismissed:false,docVerified:false})); setPassModal(null); }}
