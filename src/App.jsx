...
// Para compatibilidad — componentes que no tienen acceso a cfg usan este fallback
const GASTO_TIPOS = GASTO_TIPOS_DEFAULT;
...
function GastoTipoMenu({ current, onSelect, onClose, cats }) {
  const tipos = cats || GASTO_TIPOS_DEFAULT;
  return (
    <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10000,padding:20 }} onClick={onClose}>
...
export default function App() {
  ...
  const [cfg,setCfg]=useState({ quickTripId:null,quickBtnLabel:"💰 Ver gastos",quickBtnColor:A.orange, gastoCats:[...GASTO_TIPOS_DEFAULT] });
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(!cf){ cf={ quickTripId:null,quickBtnLabel:"💰 Ver gastos",quickBtnColor:A.orange, gastoCats:[...GASTO_TIPOS_DEFAULT] }; await db.set(SK_CFG,cf); }
  ...
  const logoutAdmin=async()=>{ await logoutSessionToken(adminToken); await db.set(SK_ADMIN,null); setAdminOk(false); setAdminToken(null); go("home"); };
  ...
  if(screen==="atrip") return <ATrip go={go} tid={nav.tid} initTab={nav.initTab} trips={trips} clients={clients} crm={crm} sT={sT} sC={sC} sCrm={sCrm} cfg={cfg} sCfg={sCfg} />;
  if(screen==="passport") return <Passport go={go} cid={nav.cid} clients={clients} setClients={setClients} trips={trips} sC={sC} />;
...
// ============================================================
function ATrip({ go, tid, initTab, trips, clients, crm, sT, sC, sCrm, cfg, sCfg }) {
  const trip=trips.find(t=>t.id===tid);
...
function Passport({ go, cid, clients, setClients, trips, sC }) {
