...
function GastosGenerales({onBack,gastosGen,setGastosGen}){
  ...
  return(
    <div style={{background:A.bg,minHeight:"100vh",maxWidth:560,margin:"0 auto",color:A.text,fontFamily:BF,paddingBottom:40}}>
      ...
      {currMenu&&<CurrencyMenu current={gCurrency} onSelect={c=>{setGCurrency(c);updTrip(t=>({...t,currency:c}));}} onClose={()=>setCurrMenu(false)}/>}
    </div>
  );
}
...
export default function App(){
  ...
  const[reservas,setReservas]=useState([]);const[gastosGen,setGastosGen]=useState([]);
  ...
  useEffect(()=>{(async()=>{
    ...
    setTrips(tt);setClients(cc);setCrm(cr);setCfg(cf);if(rp)setReservas(rp);if(gg)setGastosGen(gg);
    let admValid=false;
