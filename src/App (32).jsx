  ...
  const PeopleTab=()=>{
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {passModal && <AModal title={passModal.name} onClose={()=>setPassModal(null)}><div style={{ borderRadius:12,overflow:"hidden",marginBottom:14 }}><img src={passModal.src} style={{ width:"100%",maxHeight:280,objectFit:"cover",display:"block" }} alt="" /></div><div style={{ background:A.card2,borderRadius:10,padding:"10px 12px",marginBottom:12,border:`1px solid ${A.border}`,fontSize:12,color:A.muted,fontFamily:BF,lineHeight:1.6 }}>📱 <strong style={{ color:A.cyan }}>iOS:</strong> Pulsa "Ver imagen" y mantén pulsada para guardar en la galería.</div><input ref={chPassRef} type="file" accept="image/*" capture="environment" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]){ addPass(e.target.files[0],passModal.cid); setPassModal(null); } }} /><ARow><button onClick={()=>setViewPhoto(passModal.src)} style={{ ...ab(A.card2,A.text),flex:1,border:`1px solid ${A.border}` }}>Ver</button><a href={passModal.src} download="pasaporte.jpg" target="_blank" rel="noreferrer" style={{ ...ab(A.gold+"22",A.gold),flex:1,border:`1px solid ${A.gold}44`,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:12,padding:14,fontFamily:ANTON,fontSize:16,letterSpacing:1,textTransform:"uppercase",textDecoration:"none" }}>📲 Guardar</a><button onClick={()=>{ chPassRef.current.value=""; chPassRef.current.click(); }} style={{ ...ab(A.cyan+"22",A.cyan),flex:1,border:`1px solid ${A.cyan}44` }}>Cambiar</button><button onClick={()=>{ updClient(passModal.cid,c=>({...c,passportPhoto:null})); setPassModal(null); }} style={{ ...ab(A.red+"22",A.red),flex:1,border:`1px solid ${A.red}44` }}>Borrar</button></ARow></AModal>}
        ...
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
            ...
            <div style={{ display:"flex",gap:6 }}>
              ...
              {facturas.filter(f=>!f.gastoId&&f.data).length>0&&(
                <button onClick={downloadZip} disabled={downloadingZip} style={{ ...ab(A.green+"22",A.green),border:`1px solid ${A.green}44`,padding:"7px 12px",fontSize:11,borderRadius:8,fontFamily:BF }}>
                  {downloadingZip?"...":"🗜️ ZIP"}
                </button>
              )}
...
function Passport({ go, cid, clients, setClients, trips, sC, logout }) {
      ...
      await new Promise(resolve => {
        ...
        r.onload = async ev => {
          ...
          if (!expiry) {
            setExtracting(true);
            try {
              // ✅ callClaude en vez de callGemini
              const b64 = dataUrl.split(",")[1]; const mt = f.type || "image/jpeg";
              const raw = await callClaude(b64, mt, 'Extrae la fecha de caducidad del pasaporte. Responde SOLO JSON: {"expiry":"YYYY-MM-DD"} o {"expiry":null}');
              const parsed = JSON.parse(raw.replace(/```json|```/g, "").trim());
  ...
  const submit = async () => {
    ...
    const updated = clients.map(x => x.id === cid
      ? { ...x, passportPhoto: photos[0] || null, passportPhotos: photos, rgpdConsent: true, passportConsent: c1, photoConsent: c2, firstLogin: false, passportExpiry: expiry, passportExpiryDismissed: false, consentDate: new Date().toISOString(), consentRGPD: c0, consentPasaporte: c1, consentFoto: c2 }
      : x
    );
    setClients(updated);
    go("notifprompt", { cid });
