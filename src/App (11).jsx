...
function NotifAdmin({ trips, clients }) {
  ...
  const send=async(title,body,target,tripId)=>{
    setSending(true);
    try {
      // Construir filtros de OneSignal según el destinatario seleccionado
...
function NotifPrompt({ go, cid, clients, sC, logout }) {
  ...
  const enable = async () => {
    setLoading(true);
    const cl = clients.find(c => c.id === cid);
    // Registrar en OneSignal y vincular el dispositivo al código del cliente
      ...
      await new Promise(resolve => {
        window.OneSignalDeferred.push(async (OneSignal) => {
          try {
            ...
            // Vincular este dispositivo al código del viajero (externalId)
            await OneSignal.login(cl.code);
            // Añadir tag con el tripId para poder filtrar por viaje
