...
function PassportImageViewer({ src }) {
  ...
  const zoomOut = e => { e.stopPropagation(); const n = Math.max(1, zoom - 0.5); setZoom(n); if (n === 1) { setPan({ x: 0, y: 0 }); lastPan.current = { x: 0, y: 0 }; } };
  ...
  const onDown = e => { if (!isZ) return; e.preventDefault(); setDragging(true); dragStart.current = { x: e.clientX - lastPan.current.x, y: e.clientY - lastPan.current.y }; };
  const onMove = e => { if (!dragging) return; const nx = e.clientX - dragStart.current.x; const ny = e.clientY - dragStart.current.y; lastPan.current = { x: nx, y: ny }; setPan({ x: nx, y: ny }); };
  const onUp = () => setDragging(false);
  const onWheel = e => { e.stopPropagation(); const n = Math.min(5, Math.max(1, zoom + (e.deltaY < 0 ? 0.25 : -0.25))); setZoom(n); if (n === 1) setPan({ x: 0, y: 0 }); };

  return (
    <div style={{ background: "#000", flexShrink: 0 }}>
      <div style={{ display: "flex", gap: 8, padding: "8px 12px", alignItems: "center", background: "rgba(0,0,0,0.6)" }}>
      ...
      <div onWheel={onWheel} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}
        style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: isZ ? "0" : "12px", minHeight: 220, maxHeight: "52vh", overflow: "hidden", touchAction: "none", userSelect: "none", cursor: isZ ? (dragging ? "grabbing" : "grab") : "default" }}>
        <img src={src} draggable={false}
          style={{ maxWidth: isZ ? "none" : "100%", maxHeight: isZ ? "none" : "50vh", objectFit: "contain", borderRadius: isZ ? 0 : 8, boxShadow: "0 0 40px rgba(0,200,255,0.15)", transform: `rotate(${rot}deg) scale(${zoom}) translate(${pan.x / zoom}px,${pan.y / zoom}px)`, transformOrigin: "center center", transition: dragging ? "none" : "transform 0.15s ease" }}
          alt="Pasaporte" />
      </div>
      {isZ && <div style={{ textAlign: "center", fontSize: 10, color: "rgba(255,255,255,0.3)", padding: "4px 0 6px", fontFamily: "sans-serif" }}>Arrastra para mover</div>}
    </div>
  );
}
  ...
  const PeopleTab=()=>{
    ...
    return (
      <div style={{ padding:"0 16px" }}>
        ...
        {passModal && (
          <div style={{ position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:9999,background:"#000",display:"flex",flexDirection:"column",overflowY:"auto" }}>
            {/* Top bar */}
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px",background:"rgba(0,0,0,0.85)",backdropFilter:"blur(8px)",borderBottom:`1px solid ${A.border}22`,flexShrink:0,position:"sticky",top:0,zIndex:10 }}>
              <div style={{ fontFamily:ANTON,fontSize:18,color:"#fff",letterSpacing:1 }}>{passModal.name}</div>
              <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                <a href={passModal.src} download="pasaporte.jpg" target="_blank" rel="noreferrer"
            ...
            {/* FULL IMAGE — zoom + rotate + pan */}
            <PassportImageViewer src={passModal.src} />
            {/* Editable fields — right below the photo */}
            <div style={{ background:"#0a0a12",borderTop:`2px solid ${A.cyan}33`,padding:"16px",flexShrink:0 }}>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14 }}>
                <div>
                  <div style={{ fontFamily:BF,fontSize:9,color:A.cyan,letterSpacing:2.5,textTransform:"uppercase",marginBottom:6 }}>Nº Documento</div>
                  <input
                    value={passDocNum}
                    onChange={e=>setPassDocNum(e.target.value.toUpperCase())}
                    placeholder="ABC123456"
                    autoComplete="off"
                    style={{ ...ais,fontFamily:ANTON,fontSize:20,letterSpacing:3,textAlign:"center",color:A.cyan,background:"#000",border:`2px solid ${A.cyan}44`,borderRadius:10,padding:"14px 8px" }}
                  />
                </div>
                <div>
                  <div style={{ fontFamily:BF,fontSize:9,color:A.orange,letterSpacing:2.5,textTransform:"uppercase",marginBottom:6 }}>Caducidad</div>
                  <input
                    value={passExpiry}
                    ...
                    type="text"
                    autoComplete="off"
                    style={{ ...ais,fontFamily:ANTON,fontSize:18,textAlign:"center",color:passportWarn(passExpiry)?A.orange:A.text,background:"#000",border:`2px solid ${passportWarn(passExpiry)?A.orange:A.border}55`,borderRadius:10,padding:"14px 8px" }}
                  />
                </div>
              </div>
              {/* Actions */}
              <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:8 }}>
                <button
                  onClick={()=>{ updClient(passModal.cid,c=>({...c,docNumero:passDocNum,passportExpiry:passExpiry,passportExpiryDismissed:false,docVerified:false})); setPassModal(null); }}
                  style={{ background:`linear-gradient(90deg,${A.cyan},#00a8cc)`,border:"none",borderRadius:10,padding:"14px",color:A.bg,fontFamily:ANTON,fontSize:15,letterSpacing:1,cursor:"pointer",textTransform:"uppercase" }}>
                  💾 Guardar
                </button>
                <button
                  onClick={()=>{ chPassRef.current.value=""; chPassRef.current.click(); }}
                  style={{ background:A.purple+"22",border:`1px solid ${A.purple}44`,borderRadius:10,padding:"14px",color:A.purple,fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer" }}>
                  📷 Cambiar
                </button>
                <button
                  onClick={()=>{ updClient(passModal.cid,c=>({...c,passportPhoto:null,docNumero:"",passportExpiry:"",docVerified:false})); setPassModal(null); }}
                  style={{ background:A.red+"22",border:`1px solid ${A.red}44`,borderRadius:10,padding:"14px",color:A.red,fontFamily:BF,fontSize:12,fontWeight:700,cursor:"pointer" }}>
                  🗑 Borrar
                </button>
              </div>
            </div>
